...
def gen_regular_order_on_triangular_lattice(conf_name='Tetrahedra', latt_choice=2):
    ...
    if conf_name=='FM':
        nat = sites.shape[-2]
        sp_lat = np.zeros((1,1,nat,3),float)
        sp_lat[...,2] = 1   
...
def gen_regular_order_on_honeycomb_lattice(conf_name='FM',Neel_vec=np.array([0,0,1]),nx=2,ny=2):
    ...
    if conf_name in ['FM','AFM','Neel','Zigzag','Stripy','super-Neel']:
        ...
        if conf_name in ['FM','AFM','Neel']: magmom = np.zeros((1,1,2),float)
        else: magmom = np.zeros((2,2,2),float)
...
def regular_order(lat_type='triangular',conf_name='Tetrahedra',Neel_vec=np.array([0,0,1]),nnx=1,nny=1,latt_choice=2):
    if lat_type=='triangular': sp_lat_uc = gen_regular_order_on_triangular_lattice(conf_name,latt_choice=latt_choice)
    if lat_type=='honeycomb':  sp_lat_uc = gen_regular_order_on_honeycomb_lattice(conf_name,Neel_vec=Neel_vec)
    if lat_type=='square':     sp_lat_uc = gen_regular_order_on_square_lattice(conf_name,Neel_vec=Neel_vec)
    if lat_type=='Lieb':       sp_lat_uc = gen_regular_order_on_Lieb_lattice(conf_name,Neel_vec=Neel_vec)
    if lat_type=='kagome':     sp_lat_uc = gen_regular_order_on_kagome_lattice(conf_name,Neel_vec=Neel_vec)
