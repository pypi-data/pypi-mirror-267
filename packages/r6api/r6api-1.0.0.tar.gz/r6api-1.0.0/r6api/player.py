...
class Player:
    ...
    def set_timespan_dates(self, start_date: str = None, end_date: str = None) -> None:
    ...
    async def load_summaries(self, gamemodes: List[str] = ["all","ranked","unranked","casual"], team_roles: List[str] = ['all', 'Attacker', 'Defender']) -> None:
        """Loads the player's current summaries.
        ...
        data_gamemodes = data.get('platforms', {}).get(self._platform_group, {}).get("gameModes", [])
        ...
        for gamemode in data_gamemodes:
            ...
            for role in roles:
                if gamemode == "ranked":
                    self.ranked_summary.setdefault(role[0], {})
                elif gamemode == "unranked":
                    self.unranked_summary.setdefault(role[0], {})
                elif gamemode == "casual":
                    self.casual_summary.setdefault(role[0], {})
                elif gamemode == "all":
                    self.all_summary.setdefault(role[0], {})
