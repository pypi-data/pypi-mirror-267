...
class LogApi:
    ...
    def send(
        ...
        message_send_executor.submit(
            internal_send, message, flags_value, image_path
        )
    ...
    def send_log_event(
        ...
        if not BM.has(flags, LogMessageFlags.SILENCE):
            message: str | Callable[[dict[str, Any]], str] = event_description.message # type: ignore
            if callable(message):
