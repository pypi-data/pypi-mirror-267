...
if sys.platform.startswith("linux") and TORCH==True:
    ...
    package_data={'': ['*.yaml', '*.txt', '*.bin', '*.pcd', '*.png', '*.gif', '*.ui', 'fireworks_explosion.pyc']},
    classifiers=[
    ...
    install_requires=['numpy', 'open3d', 'PyQt5', 'PyOpenGL', 'pyqtgraph', 'python-lzf',
                      'matplotlib', 'opencv-python', 'opencv-contrib-python',
                      'pyyaml', 'vtk', 'scipy', 'tabulate', 'pyquaternion'],  # 安装所需要的库
    entry_points={
        'console_scripts': [
            'wata.help=wata.console:explain',
            'wata.show_kitti=wata.console:show_kitti',
            'wata.lxq.yanhua=wata.console:fireworks',
            'wata=wata.console:wata_console',
        ],
    },  # 设置命令行工具(可不使用就可以注释掉)
