...
def overlap(target, source):
    ...
    for k, v in source.items():
        if k not in target or not v:  # not in (None, ''):
            target[k] = v
            target.setdefault(k, v)
    return target
...
def walk(
    ...
    def ignore(*path):
        # foo = 1
        foo = [str(_) for _ in deep_chain(path)]
        path = f"/{'/'.join(foo)}"
        # foo = 1
        # ---------------------------------------
        # check key filters
    ...
    if isinstance(container, dict):
        ...
        if include_struct or not ignore(root):
            yield buid_key(
                root,
    ...
    # EXCLUDES
    recursive, matches, same_klass = 0, 0, 0
    to_exclude = set()
    valid_items = dict()
    view_items = dict()
    for key, item in func():
        view_items[key] = item
        if ignore(root, key):
            # print(f"ingnoring: root: {root} key: {key}")
            continue
        ...
        # continue
        # ---------------------------------------
        # print(f"ok: root: {root} key: {key}")

        # yield the item found at this recursion level
        ...
        if recursive == 0 and matches == 1 and same_klass == 1:
        ...
        # RECURSIVE exploring
        for key, item in view_items.items():
