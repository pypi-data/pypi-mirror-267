...
from gailbot.shared.exception.transcribeException import APIKeyError, InternetConnectionError, AudioFileError, \
    GetUttResultError
...
class WatsonCore:
    ...
    def valid_region_api(apikey, region):
        ...
        """
        if region not in WATSON_CONFIG.regions_uris:
    ...
    def transcribe(self,
        ...
        Parameters
        ----------
        audio_path (str) : path to audio file that will be transcribed
        base_language_model (str) :  specifies the base_language_model
        language_customization_id (str) : ID of the custom acoustic model
        acoustic_customization_id (str) : ID of the custom language model.
        """
        ...
        try:
            if get_size(audio_path) >= WATSON_CONFIG.max_file_size_bytes:
                audio_path = MediaHandler.compress_to_opus(audio_path, payload_workspace)
                logger.info(f"compressed audio_path is {audio_path}")
        except Exception as e:
    ...
    def _prepare_utterance(self, audio_name: str, closure: Dict[str, Any]) -> List:
        """ 
         output the response data from Watson STT, convert the raw data to
        ...
        data: Dict[Tuple[Any, Any], Dict[str, Any]] = dict()
