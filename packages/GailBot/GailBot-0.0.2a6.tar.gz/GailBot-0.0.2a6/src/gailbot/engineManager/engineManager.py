...
# @Last Modified time: 2024-04-07 13:28:44
import os
import platform
...
class EngineManager:
    ...
    default_data = (
        {"engine": "whisper", "language": "English", "detect_speakers": False}
        if platform.machine() == "x86_64"
        else {"engine": "whisperX", "language": "en"}
    )
    ...
    def _load_engine(self, name: str, data: Dict[str, str]) -> EngineProvider | None:
        for clazz in EngineManager.engine_provider:
            engine = clazz.load(name, data)
            if engine:
