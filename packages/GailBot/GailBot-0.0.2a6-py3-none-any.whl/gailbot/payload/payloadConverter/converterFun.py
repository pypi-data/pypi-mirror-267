...
# @Last Modified time: 2024-04-09 15:21:52
# @Descriptions
...
def _convert_audio(path, tgt_parent_directory) -> List[str]:
    ...
    command = ["ffmpeg", "-i", path, "-acodec", "pcm_s16le", tgt]
...
def _convert_mxf(path, tgt_parent_directory) -> List[str]:
    ...
    for i in range(num_channels):
        ...
        skip_channel = any(np.array_equal(channel_audio, sound_data[:, j]) for j in range(i))
        if not skip_channel:
