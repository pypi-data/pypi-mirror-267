...
class RandomSplitter(DatasetSplitter):
    ...
    def split(self, dataset: Dataset) -> Tuple[pd.DataFrame, pd.DataFrame]:
        for _ in range(self.retrys):
            ...
            split_class_counts = set([self._count_unique_classes_in_split(data_files_df, annotations_df, split) for split in self.names])
            if len(split_class_counts) == 1:
                return data_files_df, annotations_df
            self.seed += 1

        raise ValueError(f"Unable to generate splits where all object classes are in all splits. Retrys: {self.retrys}")
