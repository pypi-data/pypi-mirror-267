...
__all__ = ["upload_file_to_s3", "upload_file_to_s3_in_memory",
           "upload_file_to_s3_io", "download_file_from_s3", "list_files_in_s3"]
...
def upload_file_to_s3(client: HLClient, pathname: str,
                      mimetype: Optional[str]=None,
                      multipart_filesize: Optional[str]=None) -> dict:
...
def upload_file_to_s3_in_memory(client: HLClient,bytes: bytes, img_name: str,
                                mimetype: Optional[str]=None) -> dict:
...
def guess_mimetype(file_name: str) -> Optional[str]:
    ...
    mimetype: Optional[str] = custom_mimetype_lookup.get(file_extention,
                                          mimetypes.MimeTypes().guess_type(str(file_name))[0]
                                          )
    return mimetype
...
def upload_large_file_threaded_mem(client: HLClient, filepath: str, *, part_size: int = C_1MB * 100, n_workers: int = 4) -> Tuple[bool, Optional[str]]:
    thread_client = HLClient.from_credential(api_token=client.api_token,
    ...
    def upload_worker(part_n: int, data: IO):
        with data:
            ...
            result = thread_client.presignedUploadPartUrl(
                return_type=PresignedUrlType,
                uploadId=upload_id, key=key, partNumber=part_n
            )
    ...
    def complete_file_multipart_upload(upload_id: str, key: str, parts: List[Dict]):
        complete_upload_client = HLClient.from_credential(api_token=client.api_token,
                                                 endpoint_url=client.endpoint_url)
...
def upload_file_to_s3_io(client: HLClient, fp: IO, file_name: str, file_size: int,
                         mimetype: Optional[str]=None) -> dict:
    ...
    if mimetype is None:
        mimetype = guess_mimetype(file_name)
    if mimetype is None:
        raise ValueError(f"Unable to guess mimetype from {file_name}. "
...
def download_file_from_s3(
    ...
    assert Path(output_path).parent.exists(), (
        f"Parent directory of output_path must exist, got: {output_path}")
