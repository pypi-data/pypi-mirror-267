...
from highlighter.gql_client import (
    ...
    ENV_HL_WEB_GRAPHQL_ENDPOINT, KEY_API_TOKEN, KEY_ENDPOINT_URL,
    ENV_AWS_ACCESS_KEY_ID, ENV_AWS_SECRET_ACCESS_KEY, KEY_CLOUD,
...
ENDPOINT_URL_OPTION = click.option(
    ...
    required=True,
    help=(
        "Endpoint url of the profile, "
        "https://<client-account>.highlighter.ai/graphql"
    ),
)
...
def create(path, name, api_token, endpoint_url):
    ...
    if Path(path).exists():
        raise ValueError(
            f"{path} alread exists. Use 'highlighter config update ...'")
