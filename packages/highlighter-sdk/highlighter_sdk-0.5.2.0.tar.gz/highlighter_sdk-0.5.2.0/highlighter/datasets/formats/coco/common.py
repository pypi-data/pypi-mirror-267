...
def segmentation_to_bbox(seg):
    ...
    if len(seg) > 1:
        warnings.warn(
                "segmentation_to_bbox currently only supports "
                "single polygon, not multipolygons."
                )
...
def shapely_poly_to_segmentation(poly):
    ...
    if _poly_has_interiors(poly):
        raise NotImplementedError((
            "Coco dataset spec does not support interior "
           f"polygons. ie: Polys with holes, got {poly.wkt}"
        ))
