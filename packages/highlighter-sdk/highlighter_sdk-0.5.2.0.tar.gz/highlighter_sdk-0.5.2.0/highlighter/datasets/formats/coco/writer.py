...
def validate_categories(
    ...
    if not all([a == b for a, b in zip(
        sorted(unique_ids), range(0, len(unique_ids)))]):
        raise ValueError(
                "category.id are not consecutive and starting at 1 got: "
                f"{categories}")
...
class CocoWriter(IWriter):
    ...
    def write(
        ...
        unique_object_classes = dataset.annotations_df[dataset.annotations_df.attribute_id == self.category_attribute_id].value.unique()
        ...
        if self.categories is not None:
            ...
            self.object_class_value_to_cat_id = {object_class: i for
                    i, object_class in enumerate(unique_object_classes)
                    }


            self.coco_categories = [{"id": i,  "name": object_class} for
                    i, object_class in enumerate(unique_object_classes)]
        ...
        def to_coco_annotation(grp):
            ...
            object_class_attr = data.pop(
                    OBJECT_CLASS_ATTRIBUTE_UUID,
                    data.pop(
                        str(OBJECT_CLASS_ATTRIBUTE_UUID),
                        data.pop(OBJECT_CLASS_ATTRIBUTE_UUID.label,
                                 None)))

            if object_class_attr is None:
                print((
                    f"entity: {entity_id}, No OBJECT_CLASS"
                    ))
            ...
            location_attr = data.pop(
                    PIXEL_LOCATION_ATTRIBUTE_UUID,
                    data.pop(
                        str(PIXEL_LOCATION_ATTRIBUTE_UUID),
                        data.pop(PIXEL_LOCATION_ATTRIBUTE_UUID.label,
                                 None)))
        ...
        for split in dataset.data_files_df.split.unique():
            ...
            split_annos_df = dataset.annotations_df[dataset.annotations_df.data_file_id.isin(split_data_files_df.data_file_id)]
