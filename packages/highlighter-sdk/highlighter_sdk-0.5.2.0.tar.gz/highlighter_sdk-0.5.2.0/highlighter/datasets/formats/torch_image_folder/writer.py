...
def _create_full_data_file_folder_dataset(dataset, use_entity_id_as_class,
                                      attribute_of_interest, root_dir,
                                      data_file_dir, use_symlinks):
...
def _create_crop_folder_dataset(dataset, attribute_of_interest, data_file_dir, root_dir,
    ...
    def make_crop(grp, orig_file_name, pil_data_file, root_dir):
        ...
        save_path = root_dir / attr_record["value"] / f"{orig_file_name.stem}-{l}-{t}-{r}-{b}{orig_file_name.suffix}"

        if not save_path.exists():

            crop = crop_rect_from_poly(pil_data_file,
                                       poly,
                                       crop_args)
...
class TorchImageFolderWriter(IWriter):
    ...
    def __init__(
        ...
        if (
               ((attribute_of_interest is not None) and use_entity_id_as_class) or
        ...
        if isinstance(crop_args, dict):
            ...
            raise ValueError(f"invalid parameter 'crop_args' expected a CropArgs object or None, got {crop_args}")
    ...
    def write(self, dataset):
        ...
        if self.crop_args is None:
            _create_full_data_file_folder_dataset(dataset, self.use_entity_id_as_class,
                                              self.attribute_of_interest, self.root_dir,
                                              self.data_file_dir, self.use_symlinks)
        else:
            _create_crop_folder_dataset(dataset, self.attribute_of_interest, self.data_file_dir, self.root_dir,
                                        self.crop_args)
