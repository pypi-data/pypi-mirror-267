...
class AwsS3Writer(IWriter):
    ...
    def write(
        ...
        id_to_new_filename = {i: prepend_dataset_id(f) for i, f in zip(dataset.idf.data_file_id, dataset.idf.filename)}
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            tmp_dataset.cloud_files_info = [
                S3Files(bucket_name=self.bucket_name,
                       prefix=str(prefix),
                       files=[files_s3_prefix.name])
            ]
            ...
            manifest = {
                "files": [files_s3_prefix.name],
                "records": [records_s3_prefix.name]
            }
