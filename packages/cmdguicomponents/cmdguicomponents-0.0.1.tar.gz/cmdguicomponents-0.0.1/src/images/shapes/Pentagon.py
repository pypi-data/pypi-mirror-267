...
class CMDPentagon:
            ...
            for y in range(self.radius, -self.radius - 1, -1):
                for x in range(-self.radius, self.radius + 1):
                    if self.hollow and self.pointOnEdge(x, y):
                        print("██", end="")
                    elif not self.hollow and self.pointInPentagon(x, y):
                        print("██", end="")
                    else:
                        print("  ", end="")
                print()
    ...
    def pointInPentagon(self, x, y):
        vertices = [
            (self.radius * math.cos(math.radians(angle)), self.radius * math.sin(math.radians(angle)))
            for angle in range(-18, 360 - 18, 72)
        ]
        return self.isPointInPolygon(x, y, vertices)

    def pointOnEdge(self, x, y):
        return self.pointInPentagon(x, y) and (not self.pointInPentagon(x-1, y) or not self.pointInPentagon(x+1, y) or not self.pointInPentagon(x, y-1) or not self.pointInPentagon(x, y+1))
