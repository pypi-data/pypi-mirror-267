...
class CMDHendecagon:
            ...
            for y in range(self.radius, -self.radius - 1, -1):
                for x in range(-self.radius, self.radius + 1):
                    if self.hollow and self.pointOnEdge(x, y):
                        print("██", end="")
                    elif not self.hollow and self.pointInHendecagon(x, y):
                        print("██", end="")
                    else:
                        print("  ", end="")
                print()
    ...
    def pointOnEdge(self, x, y):
        return self.pointInHendecagon(x, y) and (not self.pointInHendecagon(x-1, y) or not self.pointInHendecagon(x+1, y) or not self.pointInHendecagon(x, y-1) or not self.pointInHendecagon(x, y+1))
