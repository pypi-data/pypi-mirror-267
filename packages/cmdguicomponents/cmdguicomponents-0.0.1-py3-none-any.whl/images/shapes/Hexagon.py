...
class CMDHexagon:
            ...
            for y in range(self.radius, -self.radius - 1, -1):
                for x in range(-self.radius, self.radius + 1):
                    if self.hollow and self.pointOnEdge(x, y):
                        print("██", end="")
                    elif not self.hollow and self.pointInHexagon(x, y):
                        print("██", end="")
                    else:
                        print("  ", end="")
                print()
    ...
    def pointOnEdge(self, x, y):
        return self.pointInHexagon(x, y) and (not self.pointInHexagon(x-1, y) or not self.pointInHexagon(x+1, y) or not self.pointInHexagon(x, y-1) or not self.pointInHexagon(x, y+1))
