...
# Loggers
# ---------------------------------------------------------
from agptools.containers import walk, myassign, rebuild, SEP
...
class AsyncCrawler(iCrawler):
    ...
    async def run(self):
        ...
        while remain := self.remain_tasks():
            ...
            self.progress.update(remain=remain)

        await self._stop_pool()
        await self.syncmodel.save()
    ...
    async def dispatch(self, task, data, *args, **kw):
        ...
        # check if we need to do domething from time to time
        t1 = time.time()
        if t1 > self.t1:
