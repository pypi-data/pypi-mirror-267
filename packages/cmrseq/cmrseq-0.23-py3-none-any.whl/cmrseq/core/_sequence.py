...
class Sequence:
    ...
    **Gridding, Moments and K-space**
    ...
    def validate(self) -> None:
        ...
        :raises ValueError: If any combination of contained acquisition blocks have temporal overlap
        """
        for block in self._blocks:
            block.validate(system_specs=self._system_specs)

        self._validate_acquisition_overlap()

    def _validate_acquisition_overlap(self):
        """ Checks for any temporal overlap of contained acquisition blocks """
        start_ends = [(block.tmin.m_as("ms"), block.tmax.m_as("ms"))
                               for block in self._blocks if isinstance(block, ADC)]
        if start_ends:
            start_ends = np.array(start_ends)
            sort_idcs = np.argsort(start_ends[:, 0])
            gaps = start_ends[:, 0][sort_idcs][1:] - start_ends[:, 1][sort_idcs][:1]
            if not np.all(np.greater(gaps, 0.)):
                raise ValueError("Detected overlapping Acquisition intervals:"
                                 f"\n{start_ends[:, 0][sort_idcs]}\n{start_ends[:, 1][sort_idcs]}")
    ...
    def get_block(self, block_name: Union[str, Iterable[str]] = None,
        ...
        if block_name is not None:
            ...
            matched_block_types = [block for name, block in self._block_lookup.items()
                                   if (isinstance(block, typedef) ^ invert_pattern)]
            return matched_block_types
        else:
