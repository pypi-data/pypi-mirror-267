...
def get_analysis(dataset):
    ...
    if 'hjd_ori' in list(data.keys()):
        data.sort_values('hjd_ori',inplace=True)
        xd=data['hjd_ori']-2400000.5
    elif 'mjd_ori' in list(data.keys()):
        xd=data['mjd']
    else:
        raise Exception('No time data found in lightcurve data.')
    ...
    ts_timeseries = TimeSeries(time=ts_time, meta={'object path': None, 'object_name': None, 'frange': [[2, 60, 1500000]]})
