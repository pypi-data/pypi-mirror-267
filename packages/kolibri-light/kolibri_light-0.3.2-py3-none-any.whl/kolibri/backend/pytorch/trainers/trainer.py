...
from kolibri.backend.pytorch.data import Corpus, Dictionary, _len_dataset
from kolibri.backend.pytorch.datasets import DataLoader
from kolibri.backend.pytorch.samplers import FlairSampler
from kolibri.backend.pytorch.trainers.plugins import (
    AnnealingPlugin,
    CheckpointPlugin,
...
from kolibri.backend.pytorch.training_utils import identify_dynamic_embeddings, log_line, store_embeddings
from kolibri.backend.pytorch.nn import Model
...
class ModelTrainer(Pluggable):
    ...
    def __init__(self, model: Model, corpus: Corpus) -> None:
        """Initialize a model trainer.

        Args:
            model: The model that you want to train. The model should inherit from kolibri.backend.pytorch.nn.Model  # noqa: E501
    ...
    def train_custom(
        ...
        **kwargs,
        ...
        """Trains any class that implements the kolibri.backend.pytorch.nn.Model interface.
        ...
        Returns:
            ...
            additional information to this dictionary, such as the :class:`kolibri.backend.pytorch.trainers.plugins.MetricHistoryPlugin`
        """
        ...
        # Sanity conversion: if kolibri.backend.pytorch.device was set as a string, convert to torch.device
        if isinstance(kolibri.backend.pytorch.device, str):
            kolibri.backend.pytorch.device = torch.device(kolibri.backend.pytorch.device)
        ...
        with contextlib.ExitStack() as context_stack:
            ...
            scaler = torch.cuda.amp.GradScaler(enabled=use_amp and kolibri.backend.pytorch.device.type != "cpu")
            ...
            log.info(f" - compute on device: {kolibri.backend.pytorch.device}")
    ...
    def _initialize_model_card(self, **training_parameters):
        ...
        model_card = {
            "flair_version": kolibri.backend.pytorch.__version__,
