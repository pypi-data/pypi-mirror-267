...
from kolibri.backend.pytorch.data import (
    Corpus,
    FlairDataset,
    MultiCorpus,
    Relation,
...
from kolibri.backend.pytorch.datasets.base import find_train_dev_test_files
from kolibri.backend.pytorch.file_utils import cached_path, unpack_file
...
class JsonlCorpus(MultiFileJsonlCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        :param name: name of the Corpus see kolibri.backend.pytorch.data.Corpus
        """
...
class JsonlDataset(FlairDataset):
    def __init__(
        ...
        with path_to_json_file.open(encoding=encoding) as jsonl_fp:
            for line in tqdm.tqdm(jsonl_fp):
                ...
                if isinstance(current_line, dict):
                    ...
                    raw_text = [token[text_column_name] for token in current_line]
                    current_labels = {idx: token[label_column_name] for idx, token in enumerate(current_line)}
                    current_metadatas = {idx: token.get(self.metadata_column_name, {}) for idx, token in enumerate(current_line)}
                    current_sentence = Sentence(raw_text)

                self._add_labels_to_sentence(current_sentence, current_labels)
                self._add_metadatas_to_sentence(current_sentence, current_metadatas)

                self.sentences.append(current_sentence)

    def _add_labels_to_sentence(self, sentence: Sentence, labels: Dict[int, str]):
        # Add tags for each annotated span
        for idx, label in labels.items():
            sentence[idx].add_label(self.label_type, label)

    def _add_metadatas_to_sentence(self, sentence: Sentence, metadatas: Dict[int, Dict[str, str]]):
        # Add metadatas for sentence
        for idx, metadata_dict in metadatas.items():
            for metadata_key, metadata_value in metadata_dict.items():
                sentence[idx].add_metadata(metadata_key, metadata_value)
