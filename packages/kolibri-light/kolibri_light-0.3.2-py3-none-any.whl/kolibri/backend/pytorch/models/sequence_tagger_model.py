...
import kolibri.backend.pytorch.nn
from kolibri.backend.pytorch.data import Dictionary, Label, Sentence, Span, get_spans_from_bio
from kolibri.backend.pytorch.datasets import DataLoader, FlairDatapointDataset
from kolibri.backend.pytorch.embeddings import TokenEmbeddings
from kolibri.backend.pytorch.file_utils import cached_path, hf_download, unzip_file
from kolibri.backend.pytorch.models.sequence_tagger_utils.crf import CRF
from kolibri.backend.pytorch.models.sequence_tagger_utils.viterbi import ViterbiDecoder, ViterbiLoss
from kolibri.backend.pytorch.training_utils import store_embeddings
from kolibri.backend.pytorch.nn import Classifier
log = logging.getLogger("kolibri")
...
class SequenceTagger(Classifier[Sentence]):
    def __init__(
        ...
        if word_dropout > 0.0:
            self.word_dropout = kolibri.backend.pytorch.nn.WordDropout(word_dropout)

        if locked_dropout > 0.0:
            self.locked_dropout = kolibri.backend.pytorch.nn.LockedDropout(locked_dropout)
        ...
        self.to(kolibri.backend.pytorch.device)
    ...
    def _init_loss_weights(self, loss_weights: Dict[str, float]) -> torch.Tensor:
        ...
        return torch.tensor(weight_list).to(kolibri.backend.pytorch.device)
    ...
    def forward_loss(self, sentences: List[Sentence]) -> Tuple[torch.Tensor, int]:
        ...
        if len(sentences) == 0:
            return torch.tensor(0.0, dtype=torch.float, device=kolibri.backend.pytorch.device, requires_grad=True), 0
    ...
    def _calculate_loss(self, scores: torch.Tensor, labels: torch.LongTensor) -> Tuple[torch.Tensor, int]:
        if labels.size(0) == 0:
            return torch.tensor(0.0, requires_grad=True, device=kolibri.backend.pytorch.device), 1
    ...
    def _make_padded_tensor_for_batch(self, sentences: List[Sentence]) -> Tuple[torch.LongTensor, torch.Tensor]:
        ...
        pre_allocated_zero_tensor = torch.zeros(
            ...
            device=kolibri.backend.pytorch.device,
        )
        all_embs = []
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            if not isinstance(sentences, list) and not isinstance(sentences, kolibri.backend.pytorch.data.Dataset):
                sentences = [sentences]
            ...
            overall_loss = torch.zeros(1, device=kolibri.backend.pytorch.device)
            label_count = 0
    ...
    def _fetch_model(model_name) -> str:
        ...
        # special handling for the taggers by the @redewiegergabe project (TODO: move to model hub)
            ...
            model_file = kolibri.cache_root / cache_dir / "indirect" / "final-model.pt"
            if not model_file.exists():
                ...
                unzip_file(
                    kolibri.cache_root / cache_dir / "indirect.zip",
                    kolibri.cache_root / cache_dir,
                )
            model_path = str(kolibri.cache_root / cache_dir / "indirect" / "final-model.pt")

        elif model_name == "de-historic-direct":
            model_file = kolibri.cache_root / cache_dir / "direct" / "final-model.pt"
            if not model_file.exists():
                cached_path(
                ...
                unzip_file(
                    kolibri.cache_root / cache_dir / "direct.zip",
                    kolibri.cache_root / cache_dir,
                )
            model_path = str(kolibri.cache_root / cache_dir / "direct" / "final-model.pt")

        elif model_name == "de-historic-reported":
            model_file = kolibri.cache_root / cache_dir / "reported" / "final-model.pt"
            if not model_file.exists():
                cached_path(
                ...
                unzip_file(
                    kolibri.cache_root / cache_dir / "reported.zip",
                    kolibri.cache_root / cache_dir,
                )
            model_path = str(kolibri.cache_root / cache_dir / "reported" / "final-model.pt")

        elif model_name == "de-historic-free-indirect":
            model_file = kolibri.cache_root / cache_dir / "freeIndirect" / "final-model.pt"
            if not model_file.exists():
                cached_path(
                ...
                unzip_file(
                    kolibri.cache_root / cache_dir / "freeIndirect.zip",
                    kolibri.cache_root / cache_dir,
                )
            model_path = str(kolibri.cache_root / cache_dir / "freeIndirect" / "final-model.pt")
...
from kolibri.backend.pytorch.models import SequenceTagger
