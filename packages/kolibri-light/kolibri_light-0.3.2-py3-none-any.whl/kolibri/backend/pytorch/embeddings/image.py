...
from kolibri.backend.pytorch.data import Image
from kolibri.backend.pytorch.embeddings.base import Embeddings, register_embeddings
    ...
    def _add_embeddings_internal(self, images: List[Image]):
        for image in images:
            if image.imageURL in self.url2tensor_dict:
                ...
                image.set_embedding(self.name, torch.zeros(self.__embedding_length, device=kolibri.backend.pytorch.device))
...
class ConvTransformNetworkImageEmbeddings(ImageEmbeddings):
    ...
    def forward(self, x):
        ...
        if self.use_transformer:
            ...
            y = y.type(torch.float32).to(kolibri.backend.pytorch.device)
