...
from kolibri.backend.pytorch import device
...
class WordEmbeddings(TokenEmbeddings):
    ...
    def __init__(
        ...
        if fine_tune and force_cpu and device.type != "cpu":
    ...
    def get_vec(self, word: str) -> torch.Tensor:
        ...
        word_embedding = torch.tensor(word_embedding.tolist(), device=device, dtype=torch.float)
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]):
        for sentence in sentences:
            ...
            for i, c in enumerate(tokens_sorted_by_length):
                tokens_mask[i, : chars2_length[i]] = torch.tensor(c, dtype=torch.long, device=device)
...
class OneHotEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        one_hot_sentences_tensor = torch.tensor(one_hot_sentences, dtype=torch.long).to(device)
...
class HashEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]):
        ...
        hash_sentences = torch.tensor(context_idxs, dtype=torch.long).to(device)
...
class MuseCrosslingualEmbeddings(TokenEmbeddings):
    ...
    def get_cached_vec(self, language_code: str, word: str) -> torch.Tensor:
        ...
        word_embedding = torch.tensor(word_embedding, device=device, dtype=torch.float)
        return word_embedding
