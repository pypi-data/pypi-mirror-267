...
from kolibri.backend.pytorch.data import Sentence, Token, log
from kolibri.backend.pytorch.embeddings.base import (
    DocumentEmbeddings,
    Embeddings,
    TokenEmbeddings,
    register_embeddings,
...
def combine_strided_tensors(
    ...
    if hidden_states.dim() == 2:
        sentence_hidden_states = torch.zeros(
            (sentence_count, token_count), device=kolibri.backend.pytorch.device, dtype=hidden_states.dtype
        )
    else:
        sentence_hidden_states = torch.zeros(
            (sentence_count, token_count, hidden_states.shape[2]), device=kolibri.backend.pytorch.device, dtype=hidden_states.dtype
        )
...
class TransformerBaseEmbeddings(Embeddings[Sentence]):
    ...
    def __build_transformer_model_inputs(
        ...
        if self.feature_extractor is not None:
            ...
            image_encodings = image_encodings.to(kolibri.backend.pytorch.device)
...
class TransformerOnnxEmbeddings(TransformerBaseEmbeddings):
    ...
    def _forward_tensors(self, tensors) -> Dict[str, torch.Tensor]:
        ...
        result = {}
        if self.document_embedding:
            result["document_embeddings"] = torch.tensor(embeddings[0], device=kolibri.backend.pytorch.device)
        if self.token_embedding:
            result["token_embeddings"] = torch.tensor(embeddings[-1], device=kolibri.backend.pytorch.device)
    ...
    def export_from_embedding(
        ...
        if providers is None:
            if kolibri.backend.pytorch.device.type == "cuda":
                providers = [
                    (
...
class TransformerJitEmbeddings(TransformerBaseEmbeddings):
    def __init__(self, jit_model: Union[bytes, ScriptModule], param_names: List[str], **kwargs) -> None:
        ...
        if isinstance(jit_model, bytes):
            ...
            buffer.seek(0)
            self.jit_model: ScriptModule = torch.jit.load(buffer, map_location=kolibri.backend.pytorch.device)
        else:
...
class TransformerEmbeddings(TransformerBaseEmbeddings):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        transformer_model = transformer_model.to(kolibri.backend.pytorch.device)
        ...
        if layers == "all":
            ...
            hidden_states = transformer_model(torch.tensor([1], device=kolibri.backend.pytorch.device).unsqueeze(0))[-1]
