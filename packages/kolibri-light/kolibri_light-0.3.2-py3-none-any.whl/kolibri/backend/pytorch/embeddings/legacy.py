...
from kolibri.backend.pytorch.data import Sentence, Token
from kolibri.backend.pytorch.embeddings.document import DocumentEmbeddings
from kolibri.backend.pytorch.embeddings.token import StackedEmbeddings, TokenEmbeddings
from kolibri.backend.pytorch.file_utils import cached_path
...
class ELMoEmbeddings(TokenEmbeddings):
    ...
    def __setstate__(self, state):
        ...
        if re.fullmatch(r"cuda:[0-9]+", str(kolibri.backend.pytorch.device)):
            cuda_device = int(str(kolibri.backend.pytorch.device).split(":")[-1])
        elif str(kolibri.backend.pytorch.device) == "cpu":
            cuda_device = -1
        else:
            cuda_device = 0
        ...
        self.ee.elmo_bilm.to(device=kolibri.backend.pytorch.device)
        self.ee.elmo_bilm._elmo_lstm._states = tuple(
            [state.to(kolibri.backend.pytorch.device) for state in self.ee.elmo_bilm._elmo_lstm._states]
        )
    ...
    def embed(self, sentences: Union[List[Sentence], Sentence]):
        ...
        if not everything_embedded:
            ...
            for sentence in sentences:
                ...
                word_embeddings = torch.cat(word_embeddings, dim=0).to(kolibri.backend.pytorch.device)
...
class DocumentLSTMEmbeddings(DocumentEmbeddings):
    ...
    def __init__(
        ...
        if locked_dropout > 0.0:
            from kolibri.backend.pytorch.nn import LockedDropout, WordDropout
        ...
        for _i, sentence in enumerate(sentences):
            ...
            for _add in range(longest_token_sequence_in_batch - len(sentence.tokens)):
                word_embeddings.append(
                    torch.zeros(self.length_of_all_token_embeddings, dtype=torch.float).unsqueeze(0).to(kolibri.backend.pytorch.device)
                )

            word_embeddings_tensor = torch.cat(word_embeddings, 0).to(kolibri.backend.pytorch.device)
