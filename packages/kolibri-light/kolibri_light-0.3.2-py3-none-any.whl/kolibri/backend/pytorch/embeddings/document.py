...
from kolibri.backend.pytorch.embeddings.token import FlairEmbeddings, StackedEmbeddings, TokenEmbeddings
    ...
    def embed(self, sentences: Union[List[Sentence], Sentence]):
        ...
        for sentence in sentences:
            word_embeddings = torch.cat([token.get_embedding().unsqueeze(0) for token in sentence.tokens], dim=0).to(
                kolibri.backend.pytorch.device
            )
...
class SentenceTransformerDocumentEmbeddings(DocumentEmbeddings):
    def __init__(
        ...
        self.model = SentenceTransformer(
            model, cache_folder=str(kolibri.cache_root / "embeddings" / "sentence-transformer")
        )
