...
from kolibri.backend.pytorch.file_utils import Tqdm
from kolibri.backend.pytorch.tokenization import SegtokTokenizer, SpaceTokenizer, Tokenizer
...
class Label:
    ...
    def __str__(self) -> str:
        return f"{self.data_point.unlabeled_identifier}{kolibri.backend.pytorch._arrow}{self._value}{self.metadata_str} ({round(self._score, 4)})"
...
class DataPoint:
    ...
    def get_embedding(self, names: Optional[List[str]] = None) -> torch.Tensor:
        ...
        if names and len(names) == 1:
            if names[0] in self._embeddings:
                return self._embeddings[names[0]].to(kolibri.backend.pytorch.device)
            else:
                return torch.tensor([], device=kolibri.backend.pytorch.device)
    ...
    def get_each_embedding(self, embedding_names: Optional[List[str]] = None) -> List[torch.Tensor]:
        ...
        for embed_name in sorted(self._embeddings.keys()):
            if embedding_names and embed_name not in embedding_names:
                continue
            embed = self._embeddings[embed_name].to(kolibri.backend.pytorch.device)
    ...
    def _printout_labels(self, main_label=None, add_score: bool = True, add_metadata: bool = True):
        ...
        sent_sep = kolibri.backend.pytorch._arrow if add_score else "/"
        for key in keys:
...
class Sentence(DataPoint):
    ...
    def __init__(
        ...
        Args:
            ...
            use_tokenizer: Specify a custom tokenizer to split the text into tokens. The Default is
                :class:`kolibri.backend.pytorch.tokenization.SegTokTokenizer`. If `use_tokenizer` is set to False,
                :class:`kolibri.backend.pytorch.tokenization.SpaceTokenizer` will be used instead. The tokenizer will be ignored,
    ...
    def to_tagged_string(self, main_label=None) -> str:
        ...
        if len(label_append) > 0:
            output += f"{kolibri.backend.pytorch._arrow}[" + ", ".join(label_append) + "]"
