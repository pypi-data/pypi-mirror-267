...
def cached_path(url_or_filename: str, cache_dir: Union[str, Path]) -> Path:
    ...
    dataset_cache = kolibri.cache_root / cache_dir if kolibri.cache_root not in cache_dir.parents else cache_dir
...
def hf_download(model_name: str) -> str:
    ...
    try:
        return hf_hub_download(
            ...
            revision=revision,
            library_name="kolibri",
            library_version=kolibri.backend.pytorch.__version__,
            cache_dir=kolibri.cache_root / "models" / model_folder,
        )
    except HTTPError:
        # output information
        logger.error("-" * 80)
        ...
        logger.error("-" * 80)
        Path(kolibri.cache_root / "models" / model_folder).rmdir()  # remove folder again if not valid
        raise
