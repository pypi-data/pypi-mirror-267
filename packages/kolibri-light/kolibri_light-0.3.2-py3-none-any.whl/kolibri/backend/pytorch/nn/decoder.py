...
from kolibri.backend.pytorch.data import Dictionary, Sentence
from kolibri.backend.pytorch.embeddings import Embeddings
from kolibri.backend.pytorch.nn.distance import (
    CosineDistance,
    EuclideanDistance,
    HyperbolicDistance,
    LogitCosineDistance,
...
class LabelVerbalizerDecoder(torch.nn.Module):
    """A class for decoding labels using the idea of siamese networks / bi-encoders. This can be used for all classification tasks in kolibri.backend.pytorch.

    Args:
        label_encoder (kolibri.backend.pytorch.embeddings.TokenEmbeddings):
            The label encoder used to encode the labels into an embedding.
        label_dictionary (kolibri.backend.pytorch.data.Dictionary):
    ...
    def verbalize_labels(label_dictionary: Dictionary) -> List[Sentence]:
        ...
        Args:
            label_dictionary (kolibri.backend.pytorch.data.Dictionary): The label dictionary to verbalize.
