...
from kolibri.backend.pytorch.class_utils import get_non_abstract_subclasses
from kolibri.backend.pytorch.data import DT, DT2, Corpus, Dictionary, Sentence, _iter_dataset
from kolibri.backend.pytorch.datasets import DataLoader, FlairDatapointDataset
from kolibri.backend.pytorch.embeddings import Embeddings
from kolibri.backend.pytorch.embeddings.base import load_embeddings
from kolibri.backend.pytorch.file_utils import Tqdm, load_torch_state
from kolibri.backend.pytorch.training_utils import Result, store_embeddings
...
class Model(torch.nn.Module, typing.Generic[DT], ABC):
    ...
    def load(cls, model_path: Union[str, Path, Dict[str, Any]]) -> "Model":
        ...
        if inspect.isabstract(cls):
            ...
            for model_cls in subclasses:
                # if str(model_cls) == "<class 'kolibri.backend.pytorch.models.pairwise_classification_model.TextPairClassifier'>": continue
                try:
            ...
            model.eval()
            model.to(kolibri.backend.pytorch.device)
...
class Classifier(Model[DT], typing.Generic[DT], ReduceTransformerVocabMixin, ABC):
    ...
    The classifier inherits from kolibri.backend.pytorch.nn.Model and adds unified functionality for both, single- and multi-label
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            # loss calculation
            eval_loss = torch.zeros(1, device=kolibri.backend.pytorch.device)
            average_over = 0
...
class DefaultClassifier(Classifier[DT], typing.Generic[DT, DT2], ABC):
    ...
    It inherits from kolibri.backend.pytorch.nn.Classifier and thus from kolibri.backend.pytorch.nn.Model. All features shared by all classifiers are
    ...
    def __init__(
        ...
        if loss_weights is not None:
            ...
            self.loss_weights: Optional[torch.Tensor] = torch.FloatTensor(weight_list).to(kolibri.backend.pytorch.device)
        else:
                ...
                if len(data_points) > 0:
                    ...
                    if return_loss:
                        ...
                        if has_unknown_label:
                            ...
                            scores = torch.index_select(scores, 0, torch.tensor(filtered_indices, device=kolibri.backend.pytorch.device))
    ...
    def __str__(self) -> str:
        return (
            super(kolibri.backend.pytorch.nn.Model, self).__str__().rstrip(")")
