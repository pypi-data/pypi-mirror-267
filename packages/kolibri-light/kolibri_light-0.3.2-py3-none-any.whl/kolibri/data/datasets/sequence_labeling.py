...
import kolibri
from kolibri.data.data_base import (
    Corpus,
    FlairDataset,
    MultiCorpus,
    Sentence,
...
from kolibri.data.datasets.base import find_train_dev_test_files
from kolibri.utils.file_utils import cached_path, unpack_file
...
class JsonlCorpus(MultiFileJsonlCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        :param name: name of the Corpus see kolibri.data.Corpus
        """
...
class JsonlDataset(FlairDataset):
    def __init__(
        ...
        metadata_column_name: str = "metadata",
        label_type: str = "ner",
        labeling_level: str = "tokens"  # other options "span"
    ) -> None:
        ...
        self.labeling_level = labeling_level
        self.path_to_json_file = path_to_json_file

        self.sentences: List[Sentence] = []
        self._process_file(path_to_json_file, encoding)


    def _process_file(self, path_to_json_file, encoding):
        file_extension = path_to_json_file.suffix

        with self.path_to_json_file.open(encoding=encoding) as file:
    ...
    def _process_line(self, line):
        ...
        self._process_item(current_line)
    def _process_item(self, item):
        raw_text = item[self.text_column_name]
        if self.label_column_name not in item:
            print(item)
        else:
            current_labels = item[self.label_column_name]
            current_metadatas = item.get(self.metadata_column_name, [])
    ...
    def _add_labels_to_sentence(self, raw_text: str, sentence: Sentence, labels: List[List[Any]]):
        ...
        if self.labeling_level=="tokens":
            for idx, label in enumerate(labels):
                sentence[idx].add_label(self.label_type, label)
        else:
            for label in labels:
                self._add_label_to_sentence_span(raw_text, sentence, label[0], label[1], label[2])

    def _add_label_to_sentence_span(self, text: str, sentence: Sentence, start: int, end: int, label: str):
        ...
        :param sentence: Tokenized kolibri Sentence.
...
class ColumnDataset(FlairDataset):
    """
    A dataset class that loads data from a column-formatted file.
    """
    ...
    def __init__(
        ...
        self.path_to_column_file = Path(path_to_column_file)
        self.column_delimiter_pattern = re.compile(column_delimiter)
        ...
        self.text_column = self._get_text_column(column_name_map)
        self.head_id_column = self._get_head_column(column_name_map)

        self._identify_span_columns(column_name_map, skip_first_line)

        if self.in_memory:
            self.sentences, self.total_sentence_count = self._load_sentences()
        else:
            self.sentences = self._load_sentences_generator()
            self.total_sentence_count = None

    def _get_text_column(self, column_name_map: Dict[int, str]) -> int:
        """Returns the column index for the text column."""
        return next((col for col, name in column_name_map.items() if name == "text"), 0)

    def _get_head_column(self, column_name_map: Dict[int, str]) -> Optional[int]:
        """Returns the column index for the head column (if present)."""
        head_names = ["head", "head_id"]
        return next((col for col, name in column_name_map.items() if name in head_names), None)

    def _load_sentences(self) -> Tuple[List[Sentence], int]:
        """Loads all sentences from the file into memory."""
        sentences = []
        with self.path_to_column_file.open(encoding=self.encoding) as file:
            if self.skip_first_line:
                file.readline()

            previous_sentence = None
            for lines in self._sentence_generator(file):
                ...
                if self.banned_sentences and any(d in sentence.to_plain_string() for d in self.banned_sentences):
                    continue
    ...
    def _load_sentences_generator(self):
        """Generates sentences from the file one by one."""
        with self.path_to_column_file.open(encoding=self.encoding) as file:
            if self.skip_first_line:
                file.readline()
    ...
    def _sentence_generator(self, file):
        """Generates sentence lines from the file."""
        lines = []
        for line in file:
            if not line.isspace():
                lines.append(line)

            # if sentence ends, yield lines and reset
            if len(lines) > 0 and self._line_completes_sentence(line):
                yield lines
                lines = []

    def _identify_span_columns(self, column_name_map: Dict[int, str], skip_first_line: bool):
        """Identifies span-level and word-level tag columns."""
        ...
        with self.path_to_column_file.open(encoding=self.encoding) as file:
            ...
            for _ in range(5):
                lines = next(self._sentence_generator(file), None)
                if not lines:
                    break

                for line in lines:
                    fields = self.column_delimiter_pattern.split(line.rstrip())
    ...
    def _convert_lines_to_sentence(
        self, lines: List[str], word_level_tag_columns: Dict[int, str], span_level_tag_columns: Dict[int, str]
    ) -> Optional[Sentence]:
        """Converts a list of lines into a Sentence object."""
        ...
        for span_column in span_level_tag_columns:
            try:
                bioes_tags = [self.column_delimiter_pattern.split(line.rstrip())[span_column] for line in filtered_lines]
                ...
                logger.warning(f"Error processing span column {span_column}: {e}")
    ...
    def _parse_token(self, line: str, column_name_map: Dict[int, str], last_token: Optional[Token] = None) -> Token:
        """Parses a single line into a Token object."""
        # get fields from line
        fields: List[str] = self.column_delimiter_pattern.split(line.rstrip())
    ...
    def _remap_label(self, tag: str) -> str:
        """Remaps a label using the label_name_map if provided."""
    ...
    def _line_completes_sentence(self, line: str) -> bool:
        """Checks if a line completes a sentence."""
        return line.isspace() or line == ""

    def is_in_memory(self) -> bool:
        """Returns whether the dataset is loaded in memory."""
        return self.in_memory

    def __len__(self) -> int:
        """Returns the number of sentences in the dataset."""
        if self.total_sentence_count is None:
            raise RuntimeError("Cannot determine length of dataset when not in-memory")
    ...
    def __getitem__(self, index: int) -> Sentence:
        ...
        # else skip to position in file where sentence begins
            ...
            try:
                sentence = next(self._load_sentences_generator())
...
class ONTONOTES(MultiFileColumnCorpus):
    ...
    def _ensure_data_processed(cls, base_path, language: str, version: str):
        ...
        base_path = kolibri.cache_root / "datasets" if not base_path else Path(base_path)
        ...
        if not processed_split_path.exists():
            logger.info(f"OntoNotes splits for {version}/{language} have not been generated yet, generating it now.")

            for split in ["train", "development", "test"]:
                logger.info(f"Generating {split} split for {version}/{language}")
...
class CONLL_03(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        if not data_folder.exists():
            logger.warning("-" * 100)
            logger.warning(f'WARNING: CoNLL-03 dataset not found at "{data_folder}".')
            logger.warning(
...
class CONLL_2000(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        data_file = kolibri.cache_root / "datasets" / dataset_name / "train.txt"
        if not data_file.is_file():
            ...
            with gzip.open(kolibri.cache_root / "datasets" / dataset_name / "train.txt.gz", "rb") as f_in, open(
                kolibri.cache_root / "datasets" / dataset_name / "train.txt",
                "wb",
            ) as f_out:
                shutil.copyfileobj(f_in, f_out)
            with gzip.open(kolibri.cache_root / "datasets" / dataset_name / "test.txt.gz", "rb") as f_in, open(
                kolibri.cache_root / "datasets" / dataset_name / "test.txt",
                "wb",
            ) as f_out:
                shutil.copyfileobj(f_in, f_out)
...
class FEWNERD(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        base_path = kolibri.cache_root / "datasets"
    ...
    def _download(self, setting):
        ...
        logger.info(f"FewNERD ({setting}) dataset not found, downloading.")
        ...
        if setting not in os.listdir(self.data_folder):
            ...
            logger.info("FewNERD dataset has not been extracted yet, extracting it now. This might take a while.")
...
class NER_BASQUE(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        data_path = kolibri.cache_root / "datasets" / dataset_name
        ...
        if not data_file.is_file():
            ...
            with tarfile.open(
                kolibri.cache_root / "datasets" / dataset_name / "eiec_v1.0.tgz",
                "r:gz",
            ) as f_in:
                corpus_files = (
            ...
            with (data_folder / (file + ".txt")).open(encoding="utf-8") as fin:
                for line in fin:
                    ...
                    if line.startswith("Answer_to_Question_ID"):
                        answers += 1
            logger.info(f"File {file} has {questions} questions and {answers} answers.")
...
class NER_ENGLISH_WEBPAGES(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        # default dataset folder is the cache root
        if not base_path:
            base_path = Path(kolibri.cache_root) / "datasets"
        data_folder = base_path / dataset_name
        import tarfile
    ...
    def __init__(
        ...
        **corpusargs,
        ...
        if version not in supported_versions:
            logger.error(f"The specified version '{version}' is not in the list of supported version!")
            logger.error(f"Supported versions are '{supported_versions}'!")
            raise Exception
        ...
        for language in languages:
            ...
            if language not in language_to_code.values():
                logger.error(f"Language '{language}' is not in list of supported languages!")
                logger.error(f"Supported are '{language_to_code.values()}'!")
                logger.error("Instantiate this Corpus for instance like so 'corpus = NER_MASAKHANE(languages='luo')'")
                raise Exception
            ...
            logger.info(f"Reading data for language {language}@{version}")
            corp = ColumnCorpus(
...
class NER_MULTI_CONER_V2(MultiFileColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        if not data_folder.exists():
            logger.warning("-" * 100)
            logger.warning(f'WARNING: MultiCoNerV2 dataset not found at "{data_folder}".')
            logger.warning('Instructions for obtaining the data can be found here: https://multiconer.github.io/dataset"')
            logger.warning("-" * 100)
    ...
    def __init__(
        ...
        **corpusargs,
        ...
        for language in languages:
            ...
            # initialize comlumncorpus and add it to list
            logger.info(f"Reading data for language {language}")
            corp = ColumnCorpus(
                data_folder=language_folder,
                column_format=columns,
...
class NER_MULTI_WIKINER(MultiCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        for language in languages:
            ...
            # initialize comlumncorpus and add it to list
            logger.info(f"Read data for language {language}")
            corp = ColumnCorpus(
                data_folder=language_folder,
                column_format=columns,
                in_memory=in_memory,
    ...
    def _download_wikiner(self, language_code: str, dataset_name: str):
        ...
        data_file = kolibri.cache_root / "datasets" / dataset_name / f"aij-wikiner-{lc}-wp3.train"
        if not data_file.is_file():
            cached_path(
            ...
            bz_file = bz2.BZ2File(
                kolibri.cache_root / "datasets" / dataset_name / f"aij-wikiner-{lc}-wp3.bz2",
                "rb",
            )
            with bz_file as f, open(
                kolibri.cache_root / "datasets" / dataset_name / f"aij-wikiner-{lc}-wp3.train",
                "w",
                encoding="utf-8",
            ) as out:
                for lineb in f:
...
class NER_UKRAINIAN(ColumnCorpus):
    def __init__(
        ...
        **corpusargs,
        ...
        # download data if necessary
        conll_path = "https://raw.githubusercontent.com/lang-uk/kolibri-ner/master/fixed-split"
        test_file = "test.iob"
    ...
    def __init__(
        ...
        **corpusargs,
        ...
        if language not in supported_languages:
            logger.error(f"Language '{language}' is not in list of supported languages!")
            logger.error(f"Supported are '{supported_languages}'!")
            raise Exception
        ...
        **corpusargs,
        ...
        for domain in domains:
            if domain not in supported_domains:
                logger.error(f"Domain '{domain}' is not in list of supported domains!")
                logger.error(f"Supported are '{supported_domains}'!")
                raise Exception
        ...
        **corpusargs,
        ...
        for language in languages:
            if language not in supported_languages:
                logger.error(f"Language '{language}' is not in list of supported languages!")
                logger.error(f"Supported are '{supported_languages}'!")
                logger.error("Instantiate this Corpus for instance like so 'corpus = MASAKHA_POS(languages='bam')'")
                raise Exception
