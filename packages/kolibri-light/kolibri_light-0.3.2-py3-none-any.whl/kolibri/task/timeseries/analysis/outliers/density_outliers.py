...
def get_anomalies_density(ts, window_size = 15, distance_coef = 3, n_neighbors = 3,
    distance_func = absolute_difference_distance):
    ...
    for seg in segments:
        ...
        for grp in groups:
            ...
            series = segment_df[ts.target].values
            timestamps = segment_df[ts.timestamp].values
            ...
            if series_std:
                ...
                outliers_per_group[grp] = outliers
            else:
                outliers_per_group[grp] = []
