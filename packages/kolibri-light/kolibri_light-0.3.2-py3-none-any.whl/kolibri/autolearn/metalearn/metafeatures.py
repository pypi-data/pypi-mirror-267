...
# These should be invoked with the same transformations of data and y with which
...
def landmark_worst_node_learner(data, y):
    ...
    for attribute_idx in range(data.shape[1]):
        ...
        for train, test in kf:
            node = sklearn.tree.DecisionTreeClassifier(criterion="entropy",
                ...
                min_samples_leaf=1)
            node.fit(data[train][:,attribute_idx].reshape((-1, 1)), y[train],
                     check_input=False)
            predictions = node.predict(data[test][:,attribute_idx].reshape((-1, 1)))
