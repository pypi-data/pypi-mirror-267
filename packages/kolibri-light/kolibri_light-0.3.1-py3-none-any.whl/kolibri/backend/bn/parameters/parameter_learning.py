...
from kolibri.backend.bn.bnlearn import _filter_df, to_bayesiannetwork
from kolibri.backend.bn.bnlearn import structure_scores
import warnings
...
def fit(model, df, methodtype='bayes', scoretype='bdeu', smooth=None, n_jobs=-1, verbose=3):
    ...
    if 'BayesianNetwork' not in str(type(model)):
        ...
        model = to_bayesiannetwork(adjmat, verbose=config['verbose'])
    ...
    out['structure_scores'] = structure_scores(out, df)

    return(out)


def fit_update(model, df, n_jobs=-1, verbose=3):
    """Learn the parameters given the DAG and data.
    ...
    config = {}
    config['verbose'] = verbose
    config['method'] = model.get('config')['method']
    config['n_jobs'] = n_jobs
    adjmat = model['adjmat']
    ...
    if config['verbose']>=3: print('[bnlearn] >Parameter learning')
    # Extract model
    if isinstance(model, dict):
        model = model['model']
    ...
    # Learn on CPDs
    model.fit_update(df)  # estimator as None makes it maximum likelihood estimator according pgmpy docs.
