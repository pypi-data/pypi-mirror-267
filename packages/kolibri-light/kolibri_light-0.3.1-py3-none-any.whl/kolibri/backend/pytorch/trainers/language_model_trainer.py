...
from kolibri.backend.pytorch.optim import SGDW, ReduceLRWDOnPlateau
from kolibri.backend.pytorch.training_utils import add_file_handler
...
class LanguageModelTrainer:
        ...
        try:
            ...
            for epoch in range(self.epoch, max_epochs):
                ...
                for curr_split, train_slice in enumerate(training_generator, self.split):
                    ...
                    total_loss = torch.zeros(1, device=kolibri.backend.pytorch.device)
    ...
    def _get_batch(source, i, sequence_length):
        ...
        data = data.to(kolibri.backend.pytorch.device)
        target = target.to(kolibri.backend.pytorch.device)
