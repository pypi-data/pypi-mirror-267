...
from kolibri.backend.pytorch.data import Corpus, Dictionary, Sentence, _iter_dataset
from kolibri.backend.pytorch.embeddings.base import TokenEmbeddings, load_embeddings, register_embeddings
from kolibri.backend.pytorch.embeddings.transformer import (
    TransformerEmbeddings,
    TransformerOnnxWordEmbeddings,
)
from kolibri.backend.pytorch.file_utils import cached_path, extract_single_zip_file, instance_lru_cache
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Arguments propagated to :meth:`kolibri.backend.pytorch.embeddings.transformer.TransformerEmbeddings.__init__`
        """
...
class WordEmbeddings(TokenEmbeddings):
    ...
    def __init__(
        ...
        if fine_tune and force_cpu and kolibri.backend.pytorch.device.type != "cpu":
    ...
    def get_vec(self, word: str) -> torch.Tensor:
        ...
        word_embedding = torch.tensor(word_embedding.tolist(), device=kolibri.backend.pytorch.device, dtype=torch.float)
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        if self.force_cpu:
            embeddings = embeddings.to(kolibri.backend.pytorch.device)
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]):
        for sentence in sentences:
            ...
            for i, c in enumerate(tokens_sorted_by_length):
                tokens_mask[i, : chars2_length[i]] = torch.tensor(c, dtype=torch.long, device=kolibri.backend.pytorch.device)
...
class FlairEmbeddings(TokenEmbeddings):
    ...
    def __init__(
        ...
        from kolibri.backend.pytorch.models import LanguageModel
...
class PooledFlairEmbeddings(TokenEmbeddings):
    ...
    def __setstate__(self, d: Dict[str, Any]):
        ...
        if kolibri.backend.pytorch.device.type != "cpu":
...
class OneHotEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        one_hot_sentences_tensor = torch.tensor(one_hot_sentences, dtype=torch.long).to(kolibri.backend.pytorch.device)
...
class HashEmbeddings(TokenEmbeddings):
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]):
        ...
        hash_sentences = torch.tensor(context_idxs, dtype=torch.long).to(kolibri.backend.pytorch.device)
...
class MuseCrosslingualEmbeddings(TokenEmbeddings):
    ...
    def get_cached_vec(self, language_code: str, word: str) -> torch.Tensor:
        ...
        word_embedding = torch.tensor(word_embedding, device=kolibri.backend.pytorch.device, dtype=torch.float)
        return word_embedding
...
class BPEmbSerializable(BPEmb):
    ...
    def __setstate__(self, state):
        ...
        self.cache_dir: Path = kolibri.cache_root / "embeddings"
...
class BytePairEmbeddings(TokenEmbeddings):
    def __init__(
        ...
        **kwargs,
        ...
        if not cache_dir:
            cache_dir = kolibri.cache_root / "embeddings"
        if language:
