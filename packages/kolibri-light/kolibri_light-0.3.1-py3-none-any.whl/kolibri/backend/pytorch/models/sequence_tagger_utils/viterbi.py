...
from kolibri.backend.pytorch.data import Dictionary, Label, List, Sentence
...
class ViterbiLoss(torch.nn.Module):
    ...
    def forward(self, features_tuple: tuple, targets: torch.Tensor) -> torch.Tensor:
        ...
        targets_matrix_indices = torch.tensor(targets_matrix_indices, dtype=torch.long).unsqueeze(2).to(kolibri.backend.pytorch.device)
        ...
        scores_upto_t = torch.zeros(batch_size, self.tagset_size, device=kolibri.backend.pytorch.device, dtype=features.dtype)
...
class ViterbiDecoder:
    ...
    def decode(
        ...
        scores_upto_t = torch.zeros(batch_size, seq_len + 1, self.tagset_size, dtype=features.dtype).to(kolibri.backend.pytorch.device)
        # Create a tensor to hold back-pointers
        # i.e., indices of the previous_tag that corresponds to maximum accumulated score at current tag
        # Let pads be the <end> tag index, since that was the last tag in the decoded sequence
        backpointers = (
            torch.ones((batch_size, seq_len + 1, self.tagset_size), dtype=torch.long, device=kolibri.backend.pytorch.device)
            * self.stop_tag
        )
        ...
        decoded = torch.zeros((batch_size, backpointers.size(1)), dtype=torch.long, device=kolibri.backend.pytorch.device)
        pointer = torch.ones((batch_size, 1), dtype=torch.long, device=kolibri.backend.pytorch.device) * self.stop_tag
        ...
        # Sanity check
        assert torch.equal(
            decoded[:, 0], torch.ones((batch_size), dtype=torch.long, device=kolibri.backend.pytorch.device) * self.start_tag
        )
