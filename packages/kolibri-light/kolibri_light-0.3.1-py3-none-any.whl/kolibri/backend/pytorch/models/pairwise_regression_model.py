...
import kolibri.backend.pytorch.embeddings
import kolibri.backend.pytorch.nn
from kolibri.backend.pytorch.data import Corpus, Dictionary, Sentence, TextPair, _iter_dataset
from kolibri.backend.pytorch.datasets import DataLoader, FlairDatapointDataset
from kolibri.backend.pytorch.nn.model import ReduceTransformerVocabMixin
from kolibri.backend.pytorch.training_utils import MetricRegression, Result, store_embeddings
from kolibri.backend.pytorch.nn import Model
from kolibri.backend.pytorch.embeddings import DocumentEmbeddings, TransformerDocumentEmbeddings
class TextPairRegressor(Model[TextPair], ReduceTransformerVocabMixin):
    ...
    def __init__(
        ...
        if not self.embed_separately:
            ...
            if isinstance(
                self.embeddings,
                kolibri.TransformerDocumentEmbeddings,
            ):
    ...
    def _prepare_target_tensor(self, pairs: List[TextPair]):
        ...
        return torch.cat(target_values, 0).to(kolibri.backend.pytorch.device)
