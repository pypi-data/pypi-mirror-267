...
from kolibri.backend.pytorch.data import Dictionary
from kolibri.backend.pytorch.nn.recurrent import create_recurrent_layer
...
class LanguageModel(nn.Module):
    ...
    def get_representation(
        ...
        for chunk in chunks:
            ...
            t = torch.tensor(sequences_as_char_indices, dtype=torch.long).to(device=kolibri.backend.pytorch.device, non_blocking=True)
            batches.append(t)
    ...
    def load_language_model(cls, model_file: Union[Path, str], has_decoder=True):
        state = torch.load(str(model_file), map_location=kolibri.backend.pytorch.device)
    ...
    def generate_text(
        ...
        with torch.no_grad():
            ...
            if len(prefix) > 1:
                ...
                input = torch.cat(char_tensors).to(kolibri.backend.pytorch.device)

                prediction, _, hidden = self.forward(input, hidden)

            input = torch.tensor(self.dictionary.get_idx_for_item(prefix[-1])).unsqueeze(0).unsqueeze(0)

            log_prob = torch.zeros(1, device=kolibri.backend.pytorch.device)

            for _i in range(number_of_characters):
                input = input.to(kolibri.backend.pytorch.device)
    ...
    def calculate_perplexity(self, text: str) -> float:
        ...
        targets = targets.to(kolibri.backend.pytorch.device)
