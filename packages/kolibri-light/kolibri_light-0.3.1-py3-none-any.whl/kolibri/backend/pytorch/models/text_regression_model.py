...
from kolibri.backend.pytorch.embeddings import DocumentEmbeddings
log = logging.getLogger("kolibri")


class TextRegressor(Model[Sentence], ReduceTransformerVocabMixin):
    def __init__(
        self,
        ...
        self.document_embeddings: DocumentEmbeddings = document_embeddings
    ...
    def _prepare_tensors(self, sentences: List[Sentence]) -> Tuple[torch.Tensor]:
        ...
        text_embedding_tensor = torch.cat(text_embedding_list, 0).to(kolibri.backend.pytorch.device)
    ...
    def _labels_to_tensor(self, sentences: List[Sentence]):
        ...
        vec = torch.cat(indices, 0).to(kolibri.backend.pytorch.device)
