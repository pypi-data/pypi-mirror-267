...
def cross_corr_plot(ts, n_segments = 10, maxlags = 21, segments = None, group=None):
    """
    ...
    if ts.group_cols !=[]:
        if group is None:
            group=np.random.choice(ts.groups, size=1)[0]
    ...
    for i, (segment_1, segment_2) in enumerate(segment_pairs):
        df_segment_1 = ts[group:, segment_1, :][segment_1]
        df_segment_2 = ts[group:, segment_2, :][segment_2]
        fig, axx = utils.create_mpl_ax(ax[i])
        target_1 = df_segment_1[ts.target]
        target_2 = df_segment_2[ts.target]
...
def sample_acf_plot(ts , n_segments = 10, lags = 21, segments = None, group=None):
    """
    ...
    if ts.group_cols !=[]:
        if group is None:
            group=rnd_state.choice(ts.groups, size=1)[0]
    ...
    for i, name in enumerate(sorted(np.random.choice(segments, size=k, replace=False))):
        df_slice = ts[group:, name, :][name]
        plot_acf(x=df_slice[ts.target].values, ax=ax[i], lags=lags)
        ax[i].set_title(name)
    plt.show()


def sample_pacf_plot(ts, n_segments = 10, lags = 21, segments = None, group=None):
    """
    ...
    for i, name in enumerate(sorted(np.random.choice(segments, size=k, replace=False))):
        df_slice = ts[group:, name, :][name]
        plot_pacf(x=df_slice[ts.target].values, ax=ax[i], lags=lags)
        ax[i].set_title(name)
    plt.show()


def distribution_plot( ts, n_segments = 10, segments = None, shift = 30, window= 30, freq: str = "1M", n_rows: int = 10,):
    ...
    df_full.loc[:, "mean"] = (
        df_full.groupby(ts.segment).target.shift(shift).transform(lambda s: s.rolling(window).mean())
    )
    df_full.loc[:, "std"] = df_full.groupby(ts.segment).target.shift(shift).transform(lambda s: s.rolling(window).std())
    df_full = df_full.dropna()
    df_full.loc[:, "z"] = (df_full[ts.target] - df_full["mean"]) / df_full["std"]
    ...
    for period, df_slice in grouped_data:
        if period not in groups:
            continue
        sns.boxplot(data=df_slice.sort_values(by=ts.segment), y="z", x=ts.segment, ax=ax[i], fliersize=False)
