...
import kolibri.utils
from kolibri.task.text.entities import EntityExtractor
from kdmt.file import write_json_to_file
...
class EntitySynonymMapper(EntityExtractor):
    ...
    def process(self, message, **kwargs):
        ...
        message.entities = updated_entities
    ...
    def load(cls,
        ...
        if os.path.isfile(entity_synonyms_file):
            synonyms = kolibri.utils.file.read_json_file(entity_synonyms_file)
        else:
            synonyms = None
    ...
    def add_entities_if_synonyms(self, entity_a, entity_b):
        if entity_b is not None:
            original = kolibri.utils.as_text_type(entity_a)
            replacement = kolibri.utils.as_text_type(entity_b)
