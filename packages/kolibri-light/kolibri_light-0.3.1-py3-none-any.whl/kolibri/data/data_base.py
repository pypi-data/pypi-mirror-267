...
from kolibri.tokenizers import SegtokTokenizer, WordTokenizer, Tokenizer
...
def _iter_dataset(dataset: Optional[Dataset]) -> typing.Iterable:
    if dataset is None:
        return []
    from kolibri.data.datasets import DataLoader
...
class Dictionary:
    ...
    def load(cls, name: str):
        from kolibri.utils.file_utils import cached_path
...
class Label:
    ...
    def __str__(self) -> str:
        return f"{self.data_point.unlabeled_identifier}{kolibri._arrow}{self._value}{self.metadata_str} ({round(self._score, 4)})"
...
class DataPoint:
    """This is the parent class of all data points in Kolibri.
    ...
    def _printout_labels(self, main_label=None, add_score: bool = True, add_metadata: bool = True):
        ...
        sent_sep = kolibri._arrow if add_score else "/"
        for key in keys:
...
class Sentence(DataPoint):
    ...
    def __init__(
        ...
        Args:
            ...
            use_tokenizer: Specify a custom tokenizer to split the text into tokens. The Default is
                :class:`kolibri.tokenizers.SegTokTokenizer`. If `use_tokenizer` is set to False,
                :class:`kolibri.tokenizers.SpaceTokenizer` will be used instead. The tokenizer will be ignored,
        ...
        if isinstance(use_tokenizer, Tokenizer):
            ...
            tokenizer = SegtokTokenizer() if use_tokenizer else WordTokenizer()
    ...
    def to_tagged_string(self, main_label=None) -> str:
        ...
        if len(label_append) > 0:
            output += f"{kolibri._arrow}[" + ", ".join(label_append) + "]"
