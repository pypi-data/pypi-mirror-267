...
class TextFile(Dataset):
    ...
    ...                      preprocess=lower)
    >>> from kolibri.data.streams import DataStream
    ...
    def __init__(self, files, dictionary=None, bos_token='<S>', eos_token='</S>',
                 unk_token='<UNK>', preprocess=None,
                 encoding=None):
    ...
    def open(self):
        return chain(*[iter_(open_gziped(f, encoding=self.encoding))
                       for f in self.files])
...
class CsvFile(Dataset):
    r"""Reads csv files and numberizes them given a dictionary.
    ...
    def __init__(self, files, delimiter=',', quotechar='"', preprocess=None,
                 encoding=None):
        self.files=files
    ...
    def open(self):
        return chain(*[iter_(csv.DictReader(open_gziped(f, encoding=self.encoding), delimiter=self.delimiter, quotechar=self.quotechar))
                       for f in self.files])
