...
class CollectionMixin(object):
    ...
    def base_classifications(self, value=None):
        if value:
            self.collection["base_classifications"] = value
        return self.collection["base_classifications"]
...
class MainConfigTemplate(YAMLConfigTemplate):
    ...
    def render(
        ...
        return MainConfig(self.content["projectname"])
...
class CollectionConfigTemplate(YAMLConfigTemplate, CollectionMixin):
    ...
    def render(self, overwrite=False):
        ...
        for t in self.synergy_analyzer.targets:
        ...
        self.process_proposals(
            ...
            else cc.collection["attributes"]["optional"],
        )

        if cc.content and cc.base_classifications():
            self.base_classifications(value=cc.base_classifications())
        else:
            # add an empty set to give a hint on how fill it out
            self.base_classifications(
                value=[{"id": None, "url": None, "value": None}]
            )
