...
class Chc:
    ...
    @classmethod
    def Search(cls, query: str, basedon: str, limit=30) -> dict:
        """
        Search based on ['track', 'artist', 'playlist']
        ...
        r = requests.get(url, headers=__headers__, params=params).json()
        print(r)
        ...
        else: raise cls.OperationError("Invalid basedon, Available: ['track', 'artist', 'playlist']")

    # Get Details By ['track', 'artist', 'playlist']
    @classmethod
    def GetDetails(cls, seedid: str, basedon: str) -> dict:
        """
        Get Details based on ['track', 'artist', 'playlist']
        ...
        - limit:int Range(1, 100)
        """
        if basedon not in ['track', 'artist', 'playlist']: raise cls.OperationError("Invalid basedon, Available: ['track', 'artist', 'playlist']")
        url = f'https://www.chosic.com/api/tools/{basedon}s/{seedid}'
        r = requests.get(url, headers=__headers__).json()
        return r

    # Get Similar Songs By ['track', 'artist', 'playlist', 'genre']
    @classmethod
    def GetRecommendation(cls, seedid: str, basedon: str, limit=30) -> dict:
        """
        Get recommendation based on ['track', 'artist', 'playlist', 'genre']
