...
def instagram(ctx, inputfile, opacity, dpi, background, ranges, radius):
    ...
    if ranges[1] == 1:
        ctx.invoke(topng, inputfile=inputfile, ranges=(1, 1), transparent=True, dpi=dpi)
        ctx.invoke(blur, inputimage=f'main.png', outputimage=f'main_b.png', opacity=opacity, radius=radius)
        ctx.invoke(stack, image=f'main.png', background=f'main_b.png', outputfile=f'main_bs.png', position=(-2, -2))
        ctx.invoke(addbg, image=f'main_bs.png', background=background, outputfile=f'./downloads/main_f.png')
