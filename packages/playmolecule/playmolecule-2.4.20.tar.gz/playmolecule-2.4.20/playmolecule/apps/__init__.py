...
def _docs_from_manifest(manifest, appname, appdir):
    ...
    appfiles = _get_app_files(appdir)
    tutorials = [key for key in appfiles if key.startswith("files/tutorials/")]
    if len(tutorials):
        docs.append("")
        docs.append("Notes")
        docs.append("-----")
        ...
        for fname in tutorials:
            docs.append(f"    - {appfiles[fname].path}")
    ...
    for version in app_versions:
        ...
        try:
            app_info[version] = {
                ...
                "docs": _docs_from_manifest(
                    manifest, appname, app_versions[version]["appdir"]
                ),
