...
class ClinvarXMLSource(LazyFileSource):
    ...
    def get_CLNSIG__mstring(self, interpretations):
        ...
        for interpretation in interpretations:
            result = [
                i.strip()
                for i in re.split(
                    "[,;]", interpretation.find(DESCRIPTION).text
                )
            ]
        return result
    ...
    def should_parse_varchive(self, varchive):
        ...
        hgvs_g = self.create_csra(interpreted_record, self.get_assembly())
    ...
    def var_archive_parse(self, variation_archive):  # pragma: no mccabe
        ...
        csra = self.create_csra(interpreted_record, self.get_assembly())
        grch38_csra = self.create_csra(interpreted_record, GRCH_38)
        hgvs_g = csra
