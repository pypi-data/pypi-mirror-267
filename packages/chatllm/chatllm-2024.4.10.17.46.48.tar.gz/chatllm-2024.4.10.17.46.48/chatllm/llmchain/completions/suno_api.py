...
def song_info(df):
    ...
    """
    df['🎵音乐链接'] = df['id'].map(
        lambda x: f"『请1-2两分钟后点击』[🎧音频](https://cdn1.suno.ai/{x}.mp3) [▶️视频](https://cdn1.suno.ai/{x}.mp4)"
    )
    df['专辑图'] = df['image_url'].map(lambda x: f"![🖼]({x})")

    df = df[["created_at", "model_name", "🎵音乐链接", "专辑图"]]
...
{df.to_markdown(index=False)}.replace('|:-', '|-').replace('-:|', '-|')
    """
