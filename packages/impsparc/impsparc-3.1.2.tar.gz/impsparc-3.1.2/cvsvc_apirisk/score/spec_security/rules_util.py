        ...
        for (r, n, apinode) in nodes:
            ...
            for i, m in enumerate(self.matchSet):
                ...
                if 'request_ref_list' in m.rule.idenStr:
                        flag = False
                        for components_api in request_ref_list:
                            if components_api + '/' in n.pathstr:
                                flag = True
                        if not flag:
                            allmatch = False
                        continue
                elif m.rule.idenStr == 'check_additionalProperties':
                        if '/additionalProperties' in n.pathstr:
                            allmatch = False
                        continue
                elif m.isNodeMatch(r, IdMatchType.APILOCAL) :
                    if debug: 
    ...
    def performRefMatches(self, refdef, nodes, vios, linum_mapping, request_ref_list, debug=False):
        ...
        for refnode in nodes:
            ...
            for i, m in enumerate(self.matchSet):
                ...
                if m.rule.idenStr == 'request_ref_list':
                    flag = False
                    for components_api in request_ref_list:
                        if components_api +'/' in refnode.pathstr:
                            flag = True
                    if not flag:
                        allmatch = False
                    continue
                elif 'check_additionalProperties' in m.rule.idenStr:
                        if '/additionalProperties' in refnode.pathstr:
                            allmatch = False
                        continue
                elif m.isNodeMatch(refnode.rulestr, IdMatchType.APILOCAL) :
                    if debug: 
