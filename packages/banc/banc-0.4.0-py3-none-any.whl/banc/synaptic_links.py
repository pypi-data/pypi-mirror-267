...
def to_ng_annotations(synapses, input_order='xyz', input_units=(1, 1, 1),
    ...
    if isinstance(synapses, pd.DataFrame):
        if 'pre_pt_position' in synapses.columns and 'post_pt_position' in synapses.columns:
            synapses = np.hstack([np.vstack(synapses.pre_pt_position.values),
                                  np.vstack(synapses.post_pt_position.values)])
        elif 'pt_position' in synapses.columns:
            synapses = np.vstack(synapses.pt_position.values)
        else:
            raise ValueError("DataFrame must have columns 'pre_pt_position' and 'post_pt_position' or 'pt_position'")
    ...
    if synapses.shape[1] == 3:
        annotations = [pt_anno(synapses[i, :].tolist())
                       for i in range(synapses.shape[0])]
    elif synapses.shape[1] == 6:
