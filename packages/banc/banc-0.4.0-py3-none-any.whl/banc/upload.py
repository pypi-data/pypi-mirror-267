...
def add_soma_pt(neuron, table_name='somas_v1', fake=True):
    """
    Add a point to the soma table

    Parameters
    ----------
    neuron: 3-length iterable (point coordinate) OR int (segment ID)
    """
    client = auth.get_caveclient()
    if isinstance(neuron, int):
        ts = client.chunkedgraph.get_root_timestamps(neuron)
        if len(ts) == 0:
            raise ValueError('Int arguments must be valid segment IDs')
        segid = neuron
        ts = ts[0]
        mm = auth.get_meshmanager()
        mesh = mm.mesh(seg_id=neuron)
        pt = (mesh.centroid / (4, 4, 45)).round().astype(int)
        pt_segid = lookup.segid_from_pt_cv(pt)
        if pt_segid != segid:
            raise ValueError(f'Centroid {pt} is not in segment {segid}, please'
                             ' specify a point instead.')
    elif len(neuron) == 3 and all(isinstance(x, (int, np.integer)) for x in neuron):
        pt = np.array(neuron)
        segid = lookup.segid_from_pt_cv(pt)
        if not segid:
            raise ValueError(f'Point {pt} is not in the segmentation.')

    existing_annotations = client.materialize.live_live_query(
        table_name,
        datetime.now(timezone.utc),
        filter_equal_dict={table_name: {'pt_root_id': segid}}
    )
    if not existing_annotations.empty:
        raise ValueError('This segment already has '
                         f'{existing_annotations.pt_position.values[0]}'
                         f' in table "{table_name}".')

    stage = client.annotation.stage_annotations(table_name)
    stage.add(pt)
    if fake:
        print('Would upload soma:')
        print(stage.annotation_dataframe)
    else:
        print(f'Uploading point {pt} to table "{table_name}"')
        return client.annotation.upload_staged_annotations(stage)
