...
class GalileoPromptCallback(BaseCallbackHandler):
    ...
    def on_llm_end(
        ...
        if response.llm_output:
            # OpenAI format.
            usage_data = response.llm_output.get("token_usage", dict())
        ...
        if usage_data:
            self.rows[run_id].query_input_tokens = (
                usage_data.get("prompt_tokens") or usage_data.get("prompt_token_count") or 0
            )
            self.rows[run_id].query_output_tokens = (
                usage_data.get("completion_tokens") or usage_data.get("candidates_token_count") or 0
            )
