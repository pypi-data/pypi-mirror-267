...
def resp2generator(api_resp: Iterable, request_id: str, usage: Usage):
    tool_calls = None
    acc_chunks = []
    ...
    def gen():
        acc = []
        for chunk in itertools.chain(acc_chunks, api_resp):
            choices = chunk.choices
            if choices:
                choice = choices[0]
...
def call_llm(messages: List[str], model: str, request_id: str,
             ...
             do_search=False, search_query=None, tools: List = [],
    ...
    with logger.contextualize(request_id=request_id):
        ...
        if stream:
            ...
            tool_calls, content = resp2generator(api_resp, request_id, usage)
        ...
        resp = LLMResp(content=content, details=details, usage=usage, perf=None)
        return resp
