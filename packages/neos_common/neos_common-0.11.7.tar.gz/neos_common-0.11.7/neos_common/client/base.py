        ...
        async with httpx.AsyncClient(event_hooks={"request": [log_request]}, verify=verify) as client:
            try:
                ...
                raise error.ServiceTimeoutError(
                    title=f"Timeout connecting to {self.service_name} service.",
                    details=str(e),
                ) from e
            except httpx.ConnectError as e:
                raise error.ServiceConnectionError(
                    title=f"Error connecting to {self.service_name} service.",
                    details=str(e),
                ) from e
    ...
    def process_response(self, response: httpx.Response) -> dict:
        try:
            ...
            raise self.unhandled_error_class(
                title=f"Invalid {self.service_name} api JSON format response.",
                status=response.status_code,
                code=None,
                details=f"{exc.msg}: {exc.pos}",
            ) from exc
        if response.status_code >= BAD_REQUEST_CODE:
            ...
            try:
                ...
                exc = self.unhandled_error_class(
                    title=f"Unhandled {self.service_name} api error response.",
                    status=response.status_code,
                    code=data["type"],
                    details=details,
                )
                ...
                exc = self.unhandled_error_class(
                    title=f"Unhandled {self.service_name} api response.",
                    status=response.status_code,
                    code=None,
                    details=None,
                )
                if response.status_code == RATE_LIMIT_CODE:
                    exc = self.unhandled_error_class(
                        title=f"{self.service_name} api rate limit error.",
                        status=response.status_code,
                        code=None,
                        details=None,
                    )
            ...
            if response.status_code == REQUEST_VALIDATION_CODE:
                exc = self.handled_error_class(
                    title=f"{self.service_name} request validation error.",
