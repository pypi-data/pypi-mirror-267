...
class Creater:
    ...
    def deal_src_cmakelist(self, d):
        dp = self.name + '/CMakeLists.txt'
        # do not modify user defines cmake
        if os.path.exists(self.name):
            return
        src = os.path.join(d, 'changeme')
        util.copy_dir(src, self.name)
        os.remove(dp)
        srcfile = os.path.join(d, 'changeme/CMakeLists.txt')
        fr = open(srcfile, 'r')
        content = fr.read()
        fr.close()
        wc = str.replace(content, "changeme", self.name)
        ...
        fw.write(wc)
        fw.close()
        shared_dp = self.name + '/shared.cc'
        shared_src = os.path.join(d, 'changeme/shared.cc')
        os.remove(shared_dp)
        fs = open(shared_src, 'r')
        shared_content = fs.read()
        fr.close()
        shared_new = str.replace(shared_content, "changeme", self.name)
        sw = open(shared_dp, 'w')
        sw.write(shared_new)
        sw.close()
    ...
    def deal_cmake(self, d):
        ...
        util.mkdir('cmake')
        cxx_config = os.path.join(cm, 'changeme_cxx_config.cmake')
        cxx_config_dst = os.path.join('cmake', self.name + '_cxx_config.cmake')
        util.copy_file(cxx_config, cxx_config_dst)
        deps = os.path.join(cm, 'changeme_deps.cmake')
        deps_dst = os.path.join('cmake', self.name + '_deps.cmake')
        util.copy_file(deps, deps_dst)
        test = os.path.join(cm, 'changeme_test.cmake')
        test_dst = os.path.join('cmake', self.name + '_test.cmake')
        util.copy_file(test, test_dst)
    ...
    def create_tests_dir(self, d):
        src = os.path.join(d, 'tests')
        if self.test:
            if not os.path.exists('tests'):
                util.copy_dir(src, 'tests')
...
###########
# turbo
#gottingen/turbo@v0.9.86 -DCARBIN_BUILD_TEST=OFF -DCARBIN_BUILD_BENCHMARK=OFF -DCARBIN_BUILD_EXAMPLES=OFF -DBUILD_SHARED_LIBRARY=OFF -DBUILD_STATIC_LIBRARY=ON -DCMAKE_BUILD_TYPE=release
        '''
        f.write(content)
        f.close()
