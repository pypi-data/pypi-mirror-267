...
def select_closest(ttemdata,
    ...
    if isinstance(welllog, (str, pathlib.PurePath)):
        ori_well = ProcessWell(welllog)
        ori_well = ori_well.upscale(10)
...
def corr_well_filter(matched_ttem, matched_well, corr_thershold=0.3, correxport=False):
    ...
    for name, group in ttem_group:
        tmp_df = pd.merge_asof(group.sort_values('Elevation_Cell'),
                               well_group.get_group(name).sort_values('Elevation_top'), left_on='Elevation_Cell',
                               right_on='Elevation_top', direction='nearest')
