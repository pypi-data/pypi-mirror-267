...
from qbraid_core.services.quantum.proxy import (
    SUPPORTED_QJOB_LIBS,
    _check_proxy_braket,
...
def test_proxy_state_no_packages_installed(device_lib):
    ...
    with patch("qbraid_core.services.quantum.proxy.distribution", side_effect=PackageNotFoundError):
...
def test_braket_proxy_package_not_installed(dependency):
    ...
    with patch("qbraid_core.services.quantum.proxy.distribution", side_effect=side_effect):
...
def test_braket_proxy_native_botocore():
    ...
    with patch("qbraid_core.services.quantum.proxy.distribution", side_effect=mock_distribution):
        result = _check_proxy_braket()
