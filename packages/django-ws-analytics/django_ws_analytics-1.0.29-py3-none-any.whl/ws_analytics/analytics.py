...
class Analytics:
    ...
    def get_current_online_devices(self):
        ...
        base = AnalyticsDevice.objects.filter(expire_date__gt=timezone.now())
    ...
    def calculate_average_session_duration(self):
        ...
        sessions = (AnalyticsSession.objects.filter(expire_date__gte=self.start_of_last_month).
                    prefetch_related('analyticsrequest'))

        total_avg_duration = sessions.annotate(duration=duration_annotation).aggregate(average_duration=Avg('duration'))
        ...
        for single_day in range(7):
            ...
            daily_avg_duration = sessions.filter(
