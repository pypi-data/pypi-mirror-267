...
class Analytics:
    ...
    def get_current_online_devices(self):
        ...
        count = self.base_devices.filter(expire_date__gt=timezone.now()).count()
        return count
    ...
    def get_requests_data(self):
        ...
        daily_counts_last_week = (self.base_requests.filter(timestamp__gte=self.start_of_last_week,
                                                            timestamp__lt=self.start_of_current_week)
    ...
    def get_daily_new_users_last_month(self):
        ...
        new_users = (
            ...
            .filter(creation_date__gte=self.last_30d, creation_date__lte=timezone.now())
    ...
    def get_sessions_per_hour(self):
        ...
        averages_per_hour_last_month = {
            entry['hour']: entry['total'] / total_sessions_last_month * hours_count_last_month
    ...
    def calculate_average_requests_per_session(self):
        ...
        self.todays_avg_requests = self.base_requests.filter(
        ...
        return {
            ...
            'todays_average_requests_per_session': self.todays_avg_requests,
    ...
    def get_avg_duration(self, queryset):
    ...
    def calculate_average_session_duration(self):
        ...
        # self.today's sessions for average duration
        self.today_sessions = AnalyticsSession.objects.filter(
            analyticsrequest__timestamp__date=self.today
        )
        self.today_avg_duration = self.get_avg_duration(self.today_sessions)
        ...
        for single_day in range(7):
            ...
            last_week_daily_avg_duration[day] = self.get_avg_duration(daily_sessions)

        # Weekly average durations for last month
        last_month_weekly_avg_duration = AnalyticsSession.objects.filter(
            analyticsrequest__timestamp__gte=self.start_of_last_month
        ).annotate(
            week=TruncWeek('analyticsrequest__timestamp')
        ).values('week').annotate(average_duration=self.get_avg_duration(AnalyticsSession.objects)).order_by('week')

        return {
            'today_avg_duration': self.today_avg_duration,
    ...
    def aggregate_session_data_total_avg(self):
        ...
        def device_attribute_aggregation(attribute):
            self.today_filter = {'analyticsrequest__timestamp__date': self.today}
            ...
            # Aggregate counts
            self.today_count = AnalyticsSession.objects.filter(**self.today_filter).values(attribute).annotate(
                total=Count('id'))
            ...
            self.today_avg_duration = self.get_avg_duration(AnalyticsSession.objects.filter(**self.today_filter))
            last_week_avg_duration = self.get_avg_duration(AnalyticsSession.objects.filter(**last_week_filter))
            last_month_avg_duration = self.get_avg_duration(AnalyticsSession.objects.filter(**last_month_filter))

            return {
                'today_count': list(self.today_count),
    ...
    def calculate_bounce_rate(self):
        ...
        sessions_last_month = (
            AnalyticsSession.objects.filter(analyticsrequest__timestamp__gte=self.start_of_last_month)
    ...
    def aggregate_request_data(self, attribute, aggregate_func=Count):
        ...
        # self.today's data
        self.todays_data = requests_last_month.filter(timestamp__date=self.today).values(attribute).annotate(
    ...
    def analyze_request_data(self):
        ...
        countries = self.aggregate_request_data('country')
    ...
    # # how many active users have sent an application, self.today, last week daily, last month weekly
    # def get_unique_submissions_by_users():
    #     last_month_queryset = AnalyticsRequest.objects.filter(timestamp__gte=self.start_of_last_month,
    #                                                method="POST")
    #
    ...
    #     submission_requests_last_day = (last_month_queryset.filter(timestamp__day=self.today).values('session_device').
    ...
    def get_retention_data_new_devices(self):
        ...
        self.get_retention_data(total_queryset_device)
    ...
    def get_retention_data_all_active_devices(self):
        ...
        total_queryset_device = AnalyticsDevice.objects.filter(
            id__in=device_to_last_session.keys()
        ).annotate(
            creation_date=Case(
                *[When(id=device_id, then=Value(session_date)) for device_id, session_date in
                  device_to_last_session.items()],
                output_field=DateField()
            ),
            cohort_week=TruncWeek('creation_date')  # This remains the same
        )
