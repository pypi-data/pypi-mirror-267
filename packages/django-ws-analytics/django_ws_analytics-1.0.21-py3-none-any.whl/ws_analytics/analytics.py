...
class Analytics:
    ...
    def get_device_usage_based_on_requests(self):
        ...
        def aggregate_for_attribute_based_on_requests(attribute_path):
            ...
            last_month_data = requests_last_month.values(name=attribute_path).annotate(total=Count(attribute_path)).order_by(
                '-total')
            last_week_data = requests_last_week.values(name=attribute_path).annotate(total=Count(attribute_path)).order_by(
                '-total')
            day_of_week_data = (requests_last_month.annotate(day_of_week=TruncDay('timestamp')).values('day_of_week',
                                                                                                       name=attribute_path).
                                annotate(total=Count(attribute_path)).order_by('day_of_week', '-total'))
    ...
    def calculate_average_session_duration(self):
        ...
        ).values('week').annotate(average_duration=Avg('duration')).order_by('week')
