...
class PipelineClassification:
    ...
    def report(self):
        ...
        return (
            pd.DataFrame(classification_report(self.y_test, self.pred, output_dict=True))
            .transpose()
            .reset_index()
        )
    ...
    def lgbm(
        ...
        # predict
        self.pred = self.lgb_model.predict(self.x_test, num_iteration=self.lgb_model.best_iteration_)
        # report
        report = None
        if report_output:
            report = self.report()
        return self.lgb_model, report
