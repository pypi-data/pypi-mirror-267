...
class GasMonitor(Base):
    ...
    def get_optimal_gas_price(self, network_name, minutes, stats=None, values=None):
        ...
        try:
            ...
            raise DapiError(message=error_message, status_code=response.status_code if response else None,
                            response=response.text if response else None, exception=json_error)
        except HTTPError as http_error:
            # Handle HTTP errors
            error_message = f"HTTP error occurred: {http_error}"
            raise DapiError(message=error_message, status_code=response.status_code if response else None,
                            response=response.text if response else None, exception=http_error)
