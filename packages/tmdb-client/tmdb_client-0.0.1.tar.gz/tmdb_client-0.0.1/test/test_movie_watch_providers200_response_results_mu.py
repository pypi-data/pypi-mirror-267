# coding: utf-8

"""
    TMDB API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from tmdb_client.models.movie_watch_providers200_response_results_mu import MovieWatchProviders200ResponseResultsMU

class TestMovieWatchProviders200ResponseResultsMU(unittest.TestCase):
    """MovieWatchProviders200ResponseResultsMU unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MovieWatchProviders200ResponseResultsMU:
        """Test MovieWatchProviders200ResponseResultsMU
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MovieWatchProviders200ResponseResultsMU`
        """
        model = MovieWatchProviders200ResponseResultsMU()
        if include_optional:
            return MovieWatchProviders200ResponseResultsMU(
                link = 'https://www.themoviedb.org/movie/550-fight-club/watch?locale=MU',
                buy = [
                    tmdb_client.models.movie_watch_providers_200_response_results_mu_buy_inner.movie_watch_providers_200_response_results_MU_buy_inner(
                        logo_path = '/peURlLlr8jggOwK53fJ5wdQl05y.jpg', 
                        provider_id = 2, 
                        provider_name = 'Apple TV', 
                        display_priority = 15, )
                    ],
                rent = [
                    tmdb_client.models.movie_watch_providers_200_response_results_mu_buy_inner.movie_watch_providers_200_response_results_MU_buy_inner(
                        logo_path = '/peURlLlr8jggOwK53fJ5wdQl05y.jpg', 
                        provider_id = 2, 
                        provider_name = 'Apple TV', 
                        display_priority = 15, )
                    ]
            )
        else:
            return MovieWatchProviders200ResponseResultsMU(
        )
        """

    def testMovieWatchProviders200ResponseResultsMU(self):
        """Test MovieWatchProviders200ResponseResultsMU"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
