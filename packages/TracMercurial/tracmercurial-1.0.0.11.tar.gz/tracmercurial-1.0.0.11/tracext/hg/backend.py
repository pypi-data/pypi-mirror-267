...
_stat_float_times = os.stat_float_times() \
                    if hasattr(os, 'stat_float_times') else None
...
try:
    ...
    try:
        ...
        demandimport.enable()
    except ImportError as hg_import_error:
        demandimport = None
...
if _stat_float_times is not None:
...
def checked_encode(u, encodings, check):
    """Convert `unicode` to `bytes` trying several encodings until a
    condition is met.
    ...
    :return: the first converted `bytes` if `check(s)` is `True`,
...
def get_branchtags(repo):
    ...
    bt = {}
    for bn, heads in iteritems(repo.branchmap()):
...
def render_source_prop(repos, context, name, value):
    try:
        ...
        href = context.href.changeset(to_unicode(ctx.hex()), repos.reponame)
...
class CsetPropertyRenderer(Component):
    ...
    def match_property(self, name, mode):
        if (
            mode == 'revprop' and
            name.startswith('hg-') and
            name[3:] in ('Parents', 'Children', 'Tags', 'Branch', 'Bookmarks',
                         'source')
        ):
            return 4
        return 0
...
class HgExtPropertyRenderer(Component):
    ...
    def match_property(self, name, mode):
        if (
            mode == 'revprop' and
            name in ('hg-transplant_source', 'hg-rebase_source',
                     'hg-amend_source', 'hg-convert_revision')
        ):
            return 4
        return 0
...
class HgDefaultPropertyRenderer(Component):
    ...
    def match_property(self, name, mode):
        if mode == 'revprop' and name.startswith('hg-'):
            return 2
        return 0
...
class MercurialConnector(Component):
    ...
    def __init__(self):
        ...
        if hg_import_error:
            ...
            self._version = unicode(get_version() or b'', 'utf-8')
            ...
            m = re.match(r'(\d+)\.(\d+)(?:\.(\d+))?', self._version)
            if m:
...
class MercurialRepository(Repository):
    ...
    def normalize_rev(self, rev):
        ...
        return to_unicode(self.changectx(rev).hex())
    ...
    def short_rev(self, rev):
        ...
        if not rev:
            return len(repo) - 1
        if isinstance(rev, integer_types):
            return rev
        rev = to_unicode(rev)
        if not rev.startswith("'"):  # "'11:11'" can be a tag name?
            rev = rev.split(':', 1)[0]
            if rev == '-1':
                return nullrev
            if rev.isdigit():
    ...
    def _display(self, ctx):
        ...
        `show_rev` options.
        """
        nodestr = to_unicode(ctx.hex() if self._node_fmt == 'hex' else
                             bytes(ctx))
        if self._show_rev:
    ...
    def get_quickjump_entries(self, rev):
        ...
        for tag_, n in tagslist:
            tags.setdefault(n, []).append(self.to_u(tag_))
        def taginfo(ctx):
            t = tags.get(ctx.node())
            if t:
                return ' (%s)' % ', '.join(t)
        ...
        def quickjump_entries(ctx_name_pairs):
            for ctx, name in sorted(ctx_name_pairs, reverse=True,
                                    key=lambda arg: arg[0].rev()):
                nodestr = self._display(ctx)
                name = self.to_u(name) if name else nodestr
                yield name + taginfo(ctx), '/', nodestr
        # bookmarks
    ...
    def get_oldest_rev(self):
        repo = self.repo
        n = len(self.repo)
        return to_unicode(repo[0 if n > 0 else -1].hex())

    def get_youngest_rev(self):
        return to_unicode(self.changectx().hex())
    ...
    def previous_rev(self, rev, path=''):  # FIXME: path ignored for now
        for p in self.changectx(rev).parents():
            if p:
                return to_unicode(p.hex())  # always follow first parent
        ...
        if path: # might be a file
            ...
            if fc: # it is a file
                for c in fc.children():
                    return to_unicode(c.hex())
                else:
                    return None
        # it might be a directory (not supported for now) FIXME
        for c in ctx.children():
            return to_unicode(c.hex())  # always follow first child

    def parent_revs(self, rev):
        return [to_unicode(p.hex()) for p in self.changectx(rev).parents()]
    ...
    def get_changes(self, old_path, old_rev, new_path, new_rev,
        ...
        if new_node.isdir:
            ...
            for str_path in new_node.manifest:
                ...
                if str_new_path and \
                        not str_path.startswith(str_new_path + b'/'):
                    continue
            ...
            for str_path in old_node.manifest:
                # changes out of scope
                if str_old_path and \
                        not str_path.startswith(str_old_path + b'/'):
                    continue
...
class MercurialNode(Node):
    ...
    def __init__(self, repos, path, changectx,
        ...
        if path == '' or path == '/':
            ...
            if str_path is None:
                ...
                str_path = checked_encode(path + '/', repos.encoding,
                                          has_dir_node)
                if str_path is None:
                    raise NoSuchNode(path, to_unicode(changectx.hex()))
                lo = idx[0]
        ...
        if str_path == b'':
            ...
            if not dirctx:
                ...
                dirctxs = self._find_dirctx(changectx.rev(), [str_dir,],
                                            {str_dir: str_entries})
                for dirctx in itervalues(dirctxs):
                    break
                else:
                    raise RuntimeError('dirctxs is empty')
        ...
        if dirctx is not None:
            ...
            rev = created_rev = to_unicode(dirctx.rev())
    ...
    def get_entries(self):
        ...
        if self.str_path:
            str_dir = self.str_path + b'/'
            for str_file in self.manifest:
                if str_file.startswith(str_dir):
                    add_entry(str_file, str_file.find(b'/', len(str_dir)))
        else:
            for str_file in self.manifest:
                add_entry(str_file, str_file.find(b'/'))
        ...
        for str_entry in str_entries:
            yield self.subnode(str_entry.rstrip(b'/'), dirctxs.get(str_entry))
    ...
    def get_history(self, limit=None):
        ...
        opts = {'rev': [self.changectx.hex() + b':0']}
        if self.isfile and self.repos.version_info < (2, 1, 1):
            opts['follow'] = True
        if arity(walkchangerevs) == 4:
            iterator = self._get_history_1_4(repo, pats, opts, limit)
        else:
            iterator = self._get_history_1_3(repo, pats, opts, limit)
        to_u = self.repos.to_u
        for path, rev, chg in iterator:
            yield to_u(path), to_unicode(rev), chg
    ...
    def _get_history_1_4(self, repo, pats, opts, limit):
        ...
        if hasattr(cmdutil, 'walkchangerevs'):
            def iter_ctx(repo, pats, opts, prep):
                matcher = match(repo[None], pats, opts)
                return walkchangerevs(repo, matcher, opts, prep)
        else:
            def iter_ctx(repo, pats, opts, prep):
                wopts = logcmdutil.walkopts(pats=pats, opts=opts,
                                            revspec=opts['rev'])
                revs, matcher = logcmdutil.makewalker(repo, wopts)
                return walkchangerevs(repo, revs, matcher, prep)

        # keep one lookahead entry so that we can detect renames
        path = self.path
        entry = None
        count = 0
        for ctx in iter_ctx(repo, pats, opts, prep):
...
class MercurialChangeset(Changeset):
    ...
    def get_properties(self):
        ...
        if len(parents) > 1:
            properties['hg-Parents'] = (self.repos, [to_unicode(p.hex())
                                                     for p in parents if p])
        children = self.ctx.children()
        if len(children) > 1:
            properties['hg-Children'] = (self.repos, [to_unicode(c.hex())
                                                      for c in children])
        if self.branch:
        ...
        if phasestr is not None:
            properties['hg-Phase'] = (self.repos, to_unicode(phasestr))
        for k, v in iteritems(self.ctx.extra()):
            k = to_u(k)
            if k != 'branch':
                properties['hg-' + k] = (self.repos, to_u(v))
        return properties
    ...
    def get_tags(self):
        ...
        to_u = self.repos.to_u
        return [to_u(t) for t in self.ctx.tags()]

    def get_bookmarks(self):
        """Yield bookmarks associated with this changeset."""
        to_u = self.repos.to_u
        return [to_u(b) for b in get_bookmarks(self.ctx)]
