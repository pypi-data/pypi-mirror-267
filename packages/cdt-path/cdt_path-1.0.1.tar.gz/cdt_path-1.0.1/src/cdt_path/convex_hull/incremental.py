import numpy as np
from ..utils import LogLeft,ToLeft,Simplify_With_X_i
...
def Incremental_Sorted_Simplified(P):
	...
	i+=1
	while i<l:
		L,i_xmax=_Sorted_Increment(L,P[i],i_xmax)
		i+=1
		
	return L
	
	
def Convex_Hull_Sorted_Increment(A,p,a1=None):
	if a1==None:
	...
	while LogLeft(p,A[a1],A[a1+1])!=1:
		a1+=1
		if a1+1==la: a1=-1
		
	while LogLeft(p,A[a2-1],A[a2])!=1:
		a2-=1
...
def _Sorted_Increment(A,p,a1):
	a2=a1-len(A)
	if ToLeft(p,A[a2],A[a2+1])>0:
		a1-=1
	else:
		a2+=1
