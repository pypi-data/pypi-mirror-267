...
class FugluBlockWelcome(AbstractBackend):
    ...
    def _update_hit_count_rest(self, suspect: Suspect, listing: BlockWelcomeEntry):
        ...
        if restapi_endpoint:
            ...
            data = {
                "lasthit": datetime.datetime.utcnow().isoformat(timespec='milliseconds') + 'Z',  # utc timestamp without +00:00
                "hitcount": 1
            }
            endpoint = f'{restapi_endpoint}/{listing.list_id}'
            try:
                restapi.put(endpoint, data)
                self.logger.debug(f'{suspect.id} updated hitcount of listing {listing.list_id} via rest')
            except RESTAPIError as e:
                self.logger.warning(f'{suspect.id} failed to update hitcount of listing {listing.list_id} via rest due to {str(e)}')
...
class BlockWelcomeList(PrependerPlugin):
    ...
    def _run_backends(self, suspect: Suspect, stage: str):
        ...
        for backend_name in self.backends:
            ...
            try:
                backend.evaluate(suspect, stage)
            except RESTAPIError as e:
                suspect.set_tag('restapi.error', e)
            except Exception as e:
    ...
    def pluginlist(self, suspect: Suspect, pluginlist):
        self._run_backends(suspect, STAGE_PREPENDER)
        return pluginlist
...
class BlockWelcomeMilter(BMPRCPTMixin, BMPEOHMixin, BasicMilterPlugin, BlockWelcomeList):
    ...
    async def examine_rcpt(self, sess: tp.Union[asm.MilterSession, sm.MilterSession], recipient: bytes) -> tp.Union[bytes, tp.Tuple[bytes, str]]:
        return self._examine(sess, recipient, stage=asm.RCPT)

    async def examine_eoh(self, sess: tp.Union[asm.MilterSession, sm.MilterSession]) -> tp.Union[bytes, tp.Tuple[bytes, str]]:
        return self._examine(sess, sess.to_address, stage=asm.EOH)

    def _examine(self, sess: tp.Union[asm.MilterSession, sm.MilterSession], recipient: bytes, stage: str) -> tp.Union[bytes, tp.Tuple[bytes, str]]:
        ...
        message = None
        try:
            self._run_backends(suspect, stage)
    ...
    def _store_redis(self, suspect, rediskey, increment):
        ...
        while attempts:
            ...
            try:
                ...
                # don't retry here...
                self.logger.error(f'{suspect.id} failed to register autolist for {rediskey} due to {str(e)}')
                attempts = 0
            except (redis.exceptions.ConnectionError, redis.exceptions.ResponseError) as e:
                # retry possible for connection errors
                msg = f'{suspect.id} (retry={bool(attempts)}) failed to register autolist for {rediskey} due to {str(e)}, resetting connection'
