...
from fuglu.shared import Suspect, ScannerPlugin, DELETE, DUNNO, string_to_actioncode, FileList, \
    get_outgoing_helo, actioncode_to_string, get_default_cache, _SuspectTemplate, SuspectFilter
...
"""
    ...
    def examine(self, suspect):
        ...
        try:
            enabledarchivetypes = self.config.getlist(self.section, 'enabledarchivetypes')
    ...
    def walk(self, suspect, checkarchivenames, checkarchivecontent, sendbounce, blockedfiletemplate):
        ...
        for attObj in suspect.att_mgr.get_objectlist():
            ...
            if attObj.is_inline or attObj.is_attachment or not attObj.filename_generated:
                ...
                self.logger.debug(
                    f'{suspect.id} Skip message object: {att_name} (attachment: {attObj.is_attachment}, inline: {attObj.is_inline}, auto-name: {attObj.filename_generated}')
                continue
    ...
    def lint(self):
        ...
        fc = FunkyConsole()
        allok = self.check_config() and self.lint_magic() and self.lint_sql(fc) and self.lint_archivetypes(fc)
        return allok
...
class FileHashCheck(ScannerPlugin):
    ...
    def examine(self, suspect):
        ...
        for attobj in suspect.att_mgr.get_objectlist(level=0):
            ...
            try:
                virusname = self._check_hash(suspect, myhash)
        ...
        if self.redis_pool is not None:
            ...
            while attempts:
                ...
                try:
                    ...
                    msg = f"{suspect.id} problem getting virusname for hash {myhash} due to {self.__class__.__name__}: {str(e)}"
                    self.logger.warning(msg) if attempts else self.logger.error(msg)
                except (socket.timeout, redis.exceptions.TimeoutError):
                    self.logger.warning(f"{suspect.id} Socket timeout in check_hash for hash {myhash}")
        return virusname
...
class FileHashRedis(object):
    def __init__(self, redis_pool, ttl, logger, logprefix='(n/a)'):
    ...
    def insert(self, myhash, messagebytes, age=0):
        ...
        try:
            ...
            self.logger.info(f"{self.logprefix} Socket timeout in insert_redis")
...
class FileHashFeeder(FileHashCheck):
    ...
    def _run(self, suspect):
        ...
        for attobj in suspect.att_mgr.get_objectlist(level=0):
            ...
            if self.config.get(self.section, 'redis_conn'):
                self._insert_redis(myhash, messagebytes, suspect)
            if self.config.get(self.section, 'kafkahosts'):
                try:
    ...
    def _insert_redis(self, myhash, messagebytes, suspect):
        self._init_redis()
        redisbackend = FileHashRedis(self.redis_pool, self.delta_expiration, self.logger, suspect.id)
