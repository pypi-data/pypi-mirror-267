  ...
  PRIMARY KEY (`email`)
    ...
    def __init__(self, config, section=None):
        ...
        self.requiredvars = {
            ...
            'redis_password': {
                'default': '',
                'description': 'password to connect to redis database. leave empty for no password (deprecated, set password in redis_conn)',
            },
    ...
    def examine_rcpt(self, sess: tp.Union[asm.MilterSession, sm.MilterSession], recipient: bytes) -> tp.Union[bytes, tp.Tuple[bytes, str]]:
        ...
        if relays is not None:
            ...
            for relay in test_relays:
                ...
                try:
                    ...
                    action = self._problemcode()
                    self.logger.error(f'{sess.id} {action} failed to query skiplist for {to_address} due to {e.__class__.__name__}: {str(e)}')
                    return action, 'Internal Server Error'
                ...
                if to_address == testaddress:
                    ...
                    try:
                        self.cache.skiplist(domain, relay, servercachetime, SMTPTestResult.STAGE_CONNECT, 'call-ahead loop detected', sess.id)
                    except Exception as e:
        ...
        if recverificationsupport:
            ...
            if addrstate == SMTPTestResult.ADDRESS_TEMPFAIL:
                ...
                try:
                    self.cache.skiplist(domain, relay, servercachetime, result.stage, f'tempfail: {msg}', sess.id)
                except Exception as e:
                    self.logger.warning(f'{sess.id} failed to add skiplist entry for {domain} due to {e.__class__.__name__}: {str(e)}')
            ...
            if not (relayaccessdenied and defer_on_relayaccessdenied):
                ...
                try:
                    self.cache.put_address(to_address, cachetime, positive, msg, sess.id)
                except Exception as e:
            ...
            try:
                self.cache.skiplist(domain, relay, servercachetime, result.stage, blreason, sess.id)
            except Exception as e:
                self.logger.warning(f'{sess.id} failed to add skiplist entry for {domain} due to {e.__class__.__name__}: {str(e)}')
...
class TargetListSingleton(object):
    """
...
class ProxySMTP(smtplib.SMTP):
    ...
    def _get_socket(self, host, port, timeout):
        if self.proxy_url or socks is None:
            # Patched SMTP._get_socket
            return socks.create_connection(
                (host, port),
                timeout,
    ...
    def get_relays(self, domain, domainconfig=None):
        ...
        if srctype == 'sql':
            ...
            try:
                ...
                self.logger.error(f'{self.miltersession.id} Error reading targetlist from {val}: {e.__class__.__name__}: {str(e)}')
                relays = []
        elif srctype == 'tag':
            keylist = val.split(':')
            ...
            self.logger.error(f'{self.miltersession.id} unknown relay lookup type {srctype} in {serverconfig}')
...
class CallAheadCacheInterface(object):
    ...
    def skiplist(self, domain, relay, expires, failstage=SMTPTestResult.STAGE_RCPT_TO, reason='unknown', fugluid=None):
    ...
    def put_address(self, address, expires, positiveEntry=True, message=None, fugluid=None):
...
class MySQLCache(CallAheadCacheInterface):
    ...
    def skiplist(self, domain, relay, seconds, failstage='rcpt_to', reason='unknown', fugluid=None):
        """Put a domain/relay combination on the recipient verification skiplist for a certain amount of time"""
        conn = get_session(self.conn)

        statement = text("""INSERT INTO ca_skiplist (domain,relay,expiry_ts,check_stage,reason)
    ...
    def is_skiplisted(self, domain, relay):
        ...
        if not conn:
            return False
        statement = text("SELECT reason FROM ca_skiplist WHERE domain=:domain and relay=:relay and expiry_ts>now()")
    ...
    def unskiplist(self, relayordomain):
        """remove a server from the skiplist/history"""
        conn = get_session(self.conn)
        statement = text("DELETE FROM ca_skiplist WHERE domain=:removeme or relay=:removeme")
    ...
    def get_skiplist(self):
        ...
        if not conn:
            return None
        statement = text("SELECT domain,relay,reason,expiry_ts FROM ca_skiplist WHERE expiry_ts>now() ORDER BY domain")
        values = {}
    ...
    def wipe_address(self, address):
        ...
        if not conn:
            return None
        statement = text("DELETE FROM ca_addresscache WHERE email=:email")
    ...
    def cleanup(self):
        ...
        statement = text("DELETE FROM ca_addresscache WHERE positive=:pos and expiry_ts<(now() -interval :keeptime day)")
        ...
        res = conn.execute(text("DELETE FROM ca_skiplist where expiry_ts<now()"))
    ...
    def wipe_domain(self, domain, positive=None):
        ...
        statement = text(f'DELETE FROM ca_addresscache WHERE domain=:domain {posstatement}')
    ...
    def put_address(self, address, seconds, positiveEntry=True, message=None, fugluid=None):
        """put address into the cache"""
        conn = get_session(self.conn)
        if not conn:
            return None
        statement = text("""INSERT INTO ca_addresscache (email,domain,expiry_ts,positive,message) VALUES (:email,:domain,now()+interval :interval second,:positive,:message)
    ...
    def get_address(self, address):
        ...
        if not conn:
            return None
        statement = text("SELECT positive,message FROM ca_addresscache WHERE email=:email and expiry_ts>now()")
        values = {'email': address}
        res = conn.execute(statement, values)
        first = res.first()
        conn.remove()
    ...
    def get_all_addresses(self, domain):
        ...
        if not conn:
            return None
        statement = text("SELECT email,positive FROM ca_addresscache WHERE domain=:domain and expiry_ts>now() ORDER BY email")
        values = {'domain': domain}
        result = conn.execute(statement, values)
    ...
    def get_total_counts(self):
        conn = get_session(self.conn)
        statement = text("SELECT count(*) FROM ca_addresscache WHERE expiry_ts>now() and positive=1")
        result = conn.execute(statement)
        poscount = result.fetchone()[0]
        statement = text("SELECT count(*) FROM ca_addresscache WHERE expiry_ts>now() and positive=0")
        result = conn.execute(statement)
...
class RedisCache(CallAheadCacheInterface):
    ...
    def get_skiplist(self):
        ...
        for name in names:
            ...
            ts = utcnow() + timedelta(seconds=ttl)
    ...
    def put_address(self, address, expires, positiveEntry=True, message=None, fugluid=None):
        ...
        redisconn = self.redis_pool.get_conn()
        expires = max(expires, redisconn.ttl(name))
        self._update(name, values.data, expires)
    ...
    def wipe_address(self, address):
        ...
        name = self._keys(f'addr-{address.lower()}')
        redisconn = self.redis_pool.get_conn()
...
class MemoryCache(CallAheadCacheInterface):
    ...
    def _get(self, key) -> tp.Dict:
        obj, exptime = self.cache.get(key, ({}, 0))
        if obj is not None:
            now = time.time()
            if now < exptime:
                obj = None
        ...
        if gotlock:
            ...
            for name in self.cache.keys():
                if name.startswith('relay-'):
                    ...
                    if now < exptime:
                        item = [obj.get('domain'), obj.get('relay'), obj.get('reason'), exptime.isoformat(sep=' ', timespec='seconds')]
                        items.append(item)
            items.sort(key=lambda x: x[0])
            self.lock.release()
        return items
...
class MySQLConfigBackend(ConfigBackendInterface):
    def get_domain_config_value(self, key):
        ...
        try:
            conn = get_session(self.config.get(self.section, 'dbconnection'))
            res = conn.execute(text("SELECT confvalue FROM ca_configoverride WHERE domain=:domain and confkey=:confkey"), {'domain': self.domain, 'confkey': key})
            sc = res.scalar()
            conn.remove()
        except Exception as e:
    ...
    def get_domain_config_all(self):
        ...
        try:
            conn = get_session(self.config.get(self.section, 'dbconnection'))
            res = conn.execute(text("SELECT confkey,confvalue FROM ca_configoverride WHERE domain=:domain"), {'domain': self.domain})
            for row in res:
                retval[row[0]] = row[1]
            conn.remove()
        except Exception as e:
...
class ConfigFileBackend(ConfigBackendInterface):
    ...
    def get_domain_config_value(self, key):
        if self.config.has_option(f'ca_{self.domain}', key):
            return self.config.get(f'ca_{self.domain}', key)
        return None

    def get_domain_config_all(self):
        retval = {}
        section = f'ca_{self.domain}'
...
class SMTPTestCommandLineInterface(BackendMixin):
    ...
    def put_address(self, *args):
        ...
        config = get_config()
        self._init_cache(config, self.section)
        self.cache.put_address(address, ttl, pos, message, 'cmdline-static-add')
    ...
    def update(self, *args):
        ...
        if result.state != SMTPTestResult.TEST_OK:
            ...
            self.cache.skiplist(domain, relay, servercachetime, result.stage, result.errormessage, 'cmdline-static-add')
