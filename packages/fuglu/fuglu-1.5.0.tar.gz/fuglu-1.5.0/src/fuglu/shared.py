...
class Suspect(SimpleTimeoutMixin):
    ...
    def parse_from_type_header(self, header:str='From', validate_mail:bool=True, recombine:bool=True, use_cache:bool=True) -> tp.List[tp.Tuple[str,str]]:
        ...
        for display, mailaddress in email.utils.getaddresses(from_headers):
        ...
        from_addresses_decoded = [(Suspect.decode_msg_header(display), mail.strip())
...
class SuspectFilter(FileList):
    ...
    def _load_perlstyle_line(self, line):
        ...
        try:
            ...
            raise Exception(f'Could not compile regex {regex} in file {self.filename} due to {e.__class__.__name__}: {str(e)}')
    ...
    def strip_text(self, content, remove_tags=None, replace_nbsp=True, use_bfs=True, fugluid: str = "<>"):
    ...
    def matches(self, suspect, extended=False, verbose=False):
    ...
    def get_args(self, suspect, extended=False):
        ...
        for tup in self.get_list():
            ...
            vals = self.get_field(suspect, fieldname)
            if vals is None or len(vals) == 0:
                self.logger.debug(f'{suspect.id} No field {fieldname} found')
                continue
            for val in vals:
                if val is None:
                    continue
                try:
                    ...
                    if pattern.search(strval) is not None:
                        ...
                        self.logger.debug(f'{suspect.id} NO MATCH field={fieldname} arg={arg} regex={pattern.pattern} against value={val}')
                except UnicodeEncodeError:
                    pass
