...
def clip_videos_by_frame_ids(file_paths: List[Union[str, os.PathLike]],
    ...
    for cnt, i in enumerate(frm_ids):
        ...
        if i[0] >= i[1]: raise FrameRangeError(msg=f'Start frame for video {i} is before or the same as the end frame ({i[0]}, {i[1]})', source=clip_videos_by_frame_ids.__name__)
