...
from numerous import __version__ as current_version
...
def _update_with_latest_version(state: VersionState) -> VersionState:
    if state.last_checked_at:
        last_checked_date = datetime.strptime(
            state.last_checked_at, "%Y-%m-%d %H:%M:%S",
        ).astimezone()
    ...
    try:
        ...
        state.last_checked_at = datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S")  # noqa: E501
