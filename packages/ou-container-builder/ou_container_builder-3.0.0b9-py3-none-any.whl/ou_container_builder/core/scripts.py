...
def generate(state: State, progress: Progress) -> None:
    ...
    for idx, script in enumerate(state["scripts"]):
        if script["stage"] == "build":
            state.update(
                {"output_blocks": {"build": [{"block": docker_run_cmd(script["commands"]), "weight": 201 + idx}]}}
            )
        elif script["stage"] == "deploy":
            state.update(
                {"output_blocks": {"deploy": [{"block": docker_run_cmd(script["commands"]), "weight": 201 + idx}]}}
            )
        elif script["stage"] == "startup":
            if not script["commands"][0].startswith("#!"):
                script["commands"].insert(0, "#!/bin/bash")
            filename = f"{SCRIPT_BASE_WEIGHT + idx}-{script['name'].lower().replace(' ', '-')}"
                        ...
                        "deploy": [
                            {
                                "block": docker_copy_cmd(
                                    os.path.join("ou-builder-build", "scripts", "startup", filename),
