...
def generate(state: State, progress: Progress) -> None:  # noqa: ARG001
    ...
    if len(state["packages"]["pip"]["user"]) > 0:
        ...
        # Add the startup install script
        os.makedirs(os.path.join("ou-builder-build", "python"), exist_ok=True)
        with open(os.path.join("ou-builder-build", "python", "010-installing-python-packages"), "w") as out_f:
            out_f.write(STARTUP_INSTALL_SCRIPT)
        state.update(
            {
                "output_blocks": {
                    "deploy": [
                        {
                            "block": docker_copy_cmd(
                                os.path.join("ou-builder-build", "python", "010-installing-python-packages"),
                                "/usr/share/ou/startup.d/010-installing-python-packages",
                            ),
                            "weight": 191,
                        }
                    ]
