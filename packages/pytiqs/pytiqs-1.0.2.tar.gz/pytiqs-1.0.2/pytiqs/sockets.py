...
class TiqsSocket(object):
    ...
    ROOT_URI = "wss://wss.pytiqs.trading"
    ...
    def __init__(self, app_id: str, token: str, debug: bool = False, root: Optional[str] = None,
        ...
        self.on_ticks = None  # type: Optional[Callable[[Any, bytes], None]]
    ...
    def _on_connect(self, ws: websocket.WebSocket, response: ConnectionResponse) -> None:
        self.ws = ws
        if self.on_connect:
    ...
    def _on_error(self, ws: websocket.WebSocket, code: int, reason: str) -> None:
        log.error("Connection error: {} - {}".format(code, str(reason)))

        if self.on_error:
            self.on_error(self, code, reason)

    def _on_message(self, ws: websocket.WebSocket, payload: bytes, is_binary: bool) -> None:
        if self.on_message:
    ...
    def _parse_text_message(self, ws: websocket.WebSocket, payload: Union[str, bytes]) -> None:
