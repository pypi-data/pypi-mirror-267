    ...
    struct bav_variable *v)
{
  ...
  bap_seeklast_itermon_mpz (&I->fin, &itercoeff_samerank_mpz, &last_term,
      last_number);
  I->outof = false;
    ...
    struct bav_variable *v)
{
  ...
  bav_init_term (&last_term);
  last_number = v->number.tab[bav_R_Iordering ()];
  bap_end_itermon_mpz (&I->fin, A);
  bap_term_itermon_mpz (&last_term, &I->fin);
  bap_begin_itermon_mpz (&I->debut, A);
  bap_seekfirst_itermon_mpz (&I->debut, &itercoeff_samerank_mpz, &last_term,
      last_number);
  I->outof = false;
...
/*
 * texinfo: bap_prev_itercoeff_mpz
    ...
    struct bap_itercoeff_mpz *I)
{
  ...
  last_number = I->last_variable->number.tab[bav_R_Iordering ()];
  bav_init_term (&last_term);
  bap_set_itermon_mpz (&I->fin, &I->debut);
  bap_prev_itermon_mpz (&I->fin);
  if (!bap_outof_itermon_mpz (&I->fin))
    {
      bap_term_itermon_mpz (&last_term, &I->fin);
    ...
    struct bav_term *T)
{
  ...
  if ((R == A && B != BAP_NOT_A_POLYNOM_mpz) || (R == B
          && A != BAP_NOT_A_POLYNOM_mpz))
  ...
  if (code == ba0_gt)
    ...
    {
      bap_seeklast_itermon_mpz (&iter.fin, &itercoeff_samerank_mpz,
          last_term, last_number);
...
bap_close_itercoeff_mpz (
    struct bap_itercoeff_mpz *iter)
{
  iter = (struct bap_itercoeff_mpz *) 0;
}
