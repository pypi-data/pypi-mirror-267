    ...
    struct bav_variable *v)
{
  ...
  bap_seeklast_itermon_mpzm (&I->fin, &itercoeff_samerank_mpzm, &last_term,
      last_number);
  I->outof = false;
...
/*
 * texinfo: bap_end_itercoeff_mpzm
    ...
    struct bav_variable *v)
{
  ...
  bav_init_term (&last_term);
  last_number = v->number.tab[bav_R_Iordering ()];
  bap_end_itermon_mpzm (&I->fin, A);
  bap_term_itermon_mpzm (&last_term, &I->fin);
  bap_begin_itermon_mpzm (&I->debut, A);
  bap_seekfirst_itermon_mpzm (&I->debut, &itercoeff_samerank_mpzm, &last_term,
      last_number);
  I->outof = false;
...
/*
 * texinfo: bap_prev_itercoeff_mpzm
    ...
    struct bap_itercoeff_mpzm *I)
{
  ...
  last_number = I->last_variable->number.tab[bav_R_Iordering ()];
  bav_init_term (&last_term);
  bap_set_itermon_mpzm (&I->fin, &I->debut);
  bap_prev_itermon_mpzm (&I->fin);
  if (!bap_outof_itermon_mpzm (&I->fin))
    {
      bap_term_itermon_mpzm (&last_term, &I->fin);
    ...
    struct bav_term *T)
{
  ...
  if ((R == A && B != BAP_NOT_A_POLYNOM_mpzm) || (R == B
          && A != BAP_NOT_A_POLYNOM_mpzm))
  ...
  if (code == ba0_gt)
    ...
    {
      bap_seeklast_itermon_mpzm (&iter.fin, &itercoeff_samerank_mpzm,
          last_term, last_number);
...
bap_close_itercoeff_mpzm (
    struct bap_itercoeff_mpzm *iter)
{
  iter = (struct bap_itercoeff_mpzm *) 0;
}
