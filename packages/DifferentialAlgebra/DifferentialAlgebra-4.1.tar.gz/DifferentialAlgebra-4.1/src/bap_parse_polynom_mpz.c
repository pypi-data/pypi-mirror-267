 ...
 * difference @math{p - q}.
 * This function relies on @code{bap_scanf_polynom_mpz}.
 */
...
bap_scanf_polynom_eqn_mpz (
    void *R)
{
  struct bap_polynom_mpz *A = (struct bap_polynom_mpz *) R;
  struct bap_polynom_mpz B, C;
  struct ba0_mark M;

  if (A == (struct bap_polynom_mpz *) 0)
    A = bap_new_polynom_mpz ();
...
/*
 * texinfo: bap_scanf_simplify_polynom_mpz
...
BAP_DLL void *
bap_scanf_simplify_polynom_mpz (
    void *A)
{
  struct bap_polynom_mpz *P;

  P = bap_scanf_polynom_mpz (A);
 ...
 */
  ba0_push_another_stack ();
  ba0_record (&M);
  coeffs = (struct ba0_listof_mpz *) 0;
...
#if ! defined (BAD_FLAG_mint_hp)
      coeffs =
          (struct ba0_listof_mpz *) ba0_cons_list (ba0_new_mpz (),
          (struct ba0_list *) coeffs);
#else
      coeffs =
          (struct ba0_listof_mpz *) ba0_cons_list (0,
          (struct ba0_list *) coeffs);
#endif
      terms =
 ...
 */
  ...
  while (coeffs != (struct ba0_listof_mpz *) 0)
    {
#if ! defined (BAD_FLAG_mint_hp)
      bap_write_creator_mpz (&crea, (struct bav_term *) terms->value,
          coeffs->value);
#else
      bap_write_creator_mpz (&crea, (struct bav_term *) terms->value,
...
bap_printf_polynom_mpz (
    void *R)
{
  struct bap_polynom_mpz *P = (struct bap_polynom_mpz *) R;
...
BAP_DLL void *
bap_scanf_simplify_expanded_polynom_mpz (
    void *A)
{
  struct bap_polynom_mpz *P;
  ...
  if (bav_depends_on_zero_derivatives_of_parameter_term (&P->total_rank,
          &bav_global.parameters) || bap_has_doubled_terms_mpz (P))
    bap_simplify_zero_derivatives_of_parameter_polynom_mpz (P, P,
        &bav_global.parameters);
