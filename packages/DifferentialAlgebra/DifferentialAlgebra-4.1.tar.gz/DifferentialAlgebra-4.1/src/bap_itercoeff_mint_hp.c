...
bap_begin_itercoeff_mint_hp (
    struct bap_itercoeff_mint_hp *I,
    ...
    struct bav_variable *v)
{
  ...
  bap_seeklast_itermon_mint_hp (&I->fin, &itercoeff_samerank_mint_hp, &last_term,
      last_number);
  I->outof = false;
...
/*
 * texinfo: bap_end_itercoeff_mint_hp
    ...
    struct bav_variable *v)
{
  ...
  bav_init_term (&last_term);
  last_number = v->number.tab[bav_R_Iordering ()];
  bap_end_itermon_mint_hp (&I->fin, A);
  bap_term_itermon_mint_hp (&last_term, &I->fin);
  bap_begin_itermon_mint_hp (&I->debut, A);
  bap_seekfirst_itermon_mint_hp (&I->debut, &itercoeff_samerank_mint_hp, &last_term,
      last_number);
  I->outof = false;
...
/*
 * texinfo: bap_prev_itercoeff_mint_hp
    ...
    struct bap_itercoeff_mint_hp *I)
{
  ...
  last_number = I->last_variable->number.tab[bav_R_Iordering ()];
  bav_init_term (&last_term);
  bap_set_itermon_mint_hp (&I->fin, &I->debut);
  bap_prev_itermon_mint_hp (&I->fin);
  if (!bap_outof_itermon_mint_hp (&I->fin))
    {
      bap_term_itermon_mint_hp (&last_term, &I->fin);
    ...
    struct bav_term *T)
{
  ...
  if ((R == A && B != BAP_NOT_A_POLYNOM_mint_hp) || (R == B
          && A != BAP_NOT_A_POLYNOM_mint_hp))
  ...
  if (code == ba0_gt)
    ...
    {
      bap_seeklast_itermon_mint_hp (&iter.fin, &itercoeff_samerank_mint_hp,
          last_term, last_number);
...
bap_close_itercoeff_mint_hp (
    struct bap_itercoeff_mint_hp *iter)
{
  iter = (struct bap_itercoeff_mint_hp *) 0;
}
