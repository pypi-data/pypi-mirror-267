    ...
    struct bap_polynom_mpz *Q)
{
  ...
  struct bap_polynom_mpz *P = (struct bap_polynom_mpz *) 0, *R;
  ...
  if (Q != BAP_NOT_A_POLYNOM_mpz)
    {
      ...
      bap_begin_creator_mpz (&creaQ, P, &TQmax, bap_exact_total_rank,
    ...
    struct bap_polynom_mpz *Q)
{
  ...
  struct bap_polynom_mpz *AA = (struct bap_polynom_mpz *) 0;
  ...
  while (divisible && !bap_outof_itercoeff_mpz (&iter))
    {
      ...
      if (Q != BAP_NOT_A_POLYNOM_mpz)
              ...
              while (!bap_outof_itermon_mpz (&itermon))
                {
                  ...
                  bav_mul_term (&U, &U, &T);
                  bap_write_creator_mpz (&crea, &U,
                      *bap_coeff_itermon_mpz (&itermon));
        ...
        divisible =
            bap_is_factor_elem_polynom_mpz (&c, B, BAP_NOT_A_POLYNOM_mpz);
      if (divisible)
    ...
    struct bav_variable *x)
{
  ...
  struct bap_polynom_mpz *AA, *quotient, *reste;
  struct bap_polynom_mpz *Pquotient = (struct bap_polynom_mpz *) 0;
  struct bap_polynom_mpz *Preste = (struct bap_polynom_mpz *) 0;
  ba0_mpz_t *lc;
  struct bav_term T, U;
  struct bav_variable *v, *w;
  bav_Iordering r;
 ...
 */
      ...
      bap_begin_creator_mpz (&crea_quotient, Pquotient, &T,
 ...
 */
      ...
      bap_begin_creator_mpz (&crea_reste, Preste, &T, bap_approx_total_rank,
          nbmon * (degB + 1));
    }
  ...
  while (!bap_outof_itercoeff_mpz (&iter))
    {
      ...
      bap_term_itercoeff_mpz (&T, &iter);
      if (deg_lambda >= degB)
        bap_pseudo_division_elem_polynom_mpz (quotient, reste, &lambda, B, x,
            &ibp);
      if (Q != BAP_NOT_A_POLYNOM_mpz && deg_lambda >= degB)
        {
          bap_mul_polynom_mpz (quotient, quotient,
  ...
  while (ddeg >= 0)
    {
      ...
      ddeg =
          bap_degree_polynom_mpz (&reste, v) - bap_degree_polynom_mpz (B,
          v);
      first = false;
    }
  ba0_pull_stack ();
