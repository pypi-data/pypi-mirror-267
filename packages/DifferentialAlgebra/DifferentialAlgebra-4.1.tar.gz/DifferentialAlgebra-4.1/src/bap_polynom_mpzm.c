...
static struct bap_polynom_mpzm *
...
BAP_DLL struct bap_polynom_mpzm *
    ...
    struct bap_polynom_mpzm *B)
{
  ...
  if (bap_is_zero_polynom_mpzm (B))
  ...
  else
    {
      ...
      bap_begin_creator_mpzm (&crea, A, &B->total_rank, bap_exact_total_rank,
          nbmon);
    ...
    void *Q)
{
  ...
  struct bap_polynom_mpzm *B = (struct bap_polynom_mpzm *) Q;
...
/*
 * texinfo: bad_compare_rank_polynom_mpzm
 * A comparison function for sorting tables of polynomials
 * by increasing rank, using @code{qsort}.
 */

BAP_DLL int
bap_compare_rank_polynom_mpzm (
    const void *PP,
    const void *QQ)
{
  struct bap_polynom_mpzm *P = *(struct bap_polynom_mpzm **) PP;
  struct bap_polynom_mpzm *Q = *(struct bap_polynom_mpzm **) QQ;

  if (bap_lt_rank_polynom_mpzm (P, Q))
    return -1;
  else if (bap_equal_rank_polynom_mpzm (P, Q))
    return 0;
  else
    return 1;
}
 ...
 * occuring in @var{A}. This function is often used in conjunction
 * with @code{bav_R_mark_variables} and @code{bav_R_marked_variables}.
 */
...
bap_involved_parameters_polynom_mpzm (
    struct bav_tableof_parameter *R,
    struct bap_polynom_mpzm *A,
    struct bav_tableof_parameter *P)
{
  ...
  ba0_realloc_table ((struct ba0_table *) R, P->size);
  for (i = 0; i < A->total_rank.size; i++)
    {
      ...
      if (bav_is_a_parameter (v->root, &j, P)
        ...
        {
          ba0_realloc_table ((struct ba0_table *) R, R->size + 1);
 ...
 * @code{bav_R_mark_variables} and @code{bav_marked_ranks_term}.
 */
...
bap_initial_and_reductum_polynom_mpzm (
    struct bap_polynom_mpzm *initial,
    struct bap_polynom_mpzm *reductum,
    struct bap_polynom_mpzm *A)
{
  ...
  if (bap_is_numeric_polynom_mpzm (A))
    BA0_RAISE_EXCEPTION (BAP_ERRCST);
  if ((A == initial && reductum != BAP_NOT_A_POLYNOM_mpzm) || (A == initial
          && A == reductum))
...
bap_lcoeff_and_reductum_polynom_mpzm (
    struct bap_polynom_mpzm *lcoeff,
    struct bap_polynom_mpzm *reductum,
    struct bap_polynom_mpzm *A,
    struct bav_variable *v)
{
  ...
  if (bap_is_numeric_polynom_mpzm (A) || (v != BAV_NOT_A_VARIABLE
          && v != bap_leader_polynom_mpzm (A)))
    {
      if (!bap_depend_polynom_mpzm (A, v))
      ...
      else
        {
          ...
          bap_initial_and_reductum_polynom_mpzm (lcoeff !=
              BAP_NOT_A_POLYNOM_mpzm ? &lc : BAP_NOT_A_POLYNOM_mpzm,
              reductum !=
              BAP_NOT_A_POLYNOM_mpzm ? &red : BAP_NOT_A_POLYNOM_mpzm, &AA);
  ...
  else
    {
      ...
      bap_initial_and_reductum_polynom_mpzm (lcoeff !=
          BAP_NOT_A_POLYNOM_mpzm ? &lc : BAP_NOT_A_POLYNOM_mpzm,
          reductum != BAP_NOT_A_POLYNOM_mpzm ? &red : BAP_NOT_A_POLYNOM_mpzm,
          A);
    ...
    bav_Idegree d)
{
  ...
  if (R == (struct bap_polynom_mpzm *) 0)
    {
    ...
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C != (struct bap_polynom_mpzm *) 0 && C->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C == R)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (bap_is_numeric_polynom_mpzm (A) || (v != BAV_NOT_A_VARIABLE
          && !bap_depend_polynom_mpzm (A, v)))
    {
      if (d == 0)
        {
          if (C != A && C != (struct bap_polynom_mpzm *) 0)
            bap_set_polynom_mpzm (C, A);
          bap_set_polynom_zero_mpzm (R);
        }
      else
        {
          if (C != (struct bap_polynom_mpzm *) 0)
  ...
  if (C != A && R != A)
    {
      ba0_pull_stack ();
      if (C != (struct bap_polynom_mpzm *) 0)
        bap_begin_creator_mpzm (&creaC, C, &A->total_rank,
            bap_approx_total_rank, nbmon / 2 + 1);
      bap_begin_creator_mpzm (&creaR, R, &A->total_rank, bap_approx_total_rank,
          nbmon / 2 + 1);
      ba0_push_another_stack ();
    }
  else
    {
      ...
      if (C != (struct bap_polynom_mpzm *) 0)
        bap_begin_creator_mpzm (&creaC, &CC, &A->total_rank,
            bap_approx_total_rank, nbmon / 2 + 1);
      ...
      if ((i < T.size && T.rg[i].deg == d) || (i == T.size && d == 0))
          ...
          if (C != A && R != A)
            {
              ba0_pull_stack ();
              if (C != (struct bap_polynom_mpzm *) 0)
                bap_write_creator_mpzm (&creaC, &U,
      ...
      else
        {
          if (C != A && R != A)
            {
              ba0_pull_stack ();
              bap_write_creator_mpzm (&creaR, &T,
                  *bap_coeff_itermon_mpzm (&iter));
              ba0_push_another_stack ();
            }
          else
...
comp_polynom_ascending (
    ...
    const void *y)
{
  struct bap_polynom_mpzm *A = *(struct bap_polynom_mpzm * *) x;
  struct bap_polynom_mpzm *B = *(struct bap_polynom_mpzm * *) y;
  struct bav_term TA, TB;
  struct ba0_mark M;
  enum ba0_compare_code code;
    ...
    enum ba0_sort_mode mode)
{
  switch (mode)
    ...
    case ba0_descending_mode:
      qsort (T->tab, T->size, sizeof (struct bap_polynom_mpzm *),
          &comp_polynom_descending);
      break;
...
static char _struct_polynom_rang[] = "struct bap_polynom_mpzm *->total_rank";
...
{
  struct bap_polynom_mpzm *A = (struct bap_polynom_mpzm *) AA;
  ba0_int_p n = 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info (A, sizeof (struct bap_polynom_mpzm),
        _struct_polynom);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (struct bap_polynom_mpzm *) ba0_new_addr_gc_info (AA, _struct_polynom);
  else
    A = (struct bap_polynom_mpzm *) AA;

  A->clot =
      (struct bap_clot_mpzm *) bap_garbage2_clot_mpzm (A->clot, ba0_isolated);
