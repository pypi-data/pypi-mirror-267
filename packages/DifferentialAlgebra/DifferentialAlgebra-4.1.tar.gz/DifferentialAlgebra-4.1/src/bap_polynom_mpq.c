...
static struct bap_polynom_mpq *
bap_new_polynom_clot_mpq (
    struct bap_clot_mpq *C)
{
  struct bap_polynom_mpq *A;

  A = (struct bap_polynom_mpq *) ba0_alloc (sizeof (struct
          bap_polynom_mpq));
  bap_init_polynom_clot_mpq (A, C);
  return A;
}
...
BAP_DLL struct bap_polynom_mpq *
bap_new_polynom_mpq (
    void)
{
  struct bav_term T;
    ...
    struct bap_polynom_mpq *B)
{
  ...
  if (bap_is_zero_polynom_mpq (B))
  ...
  else
    {
      ...
      bap_begin_creator_mpq (&crea, A, &B->total_rank, bap_exact_total_rank,
          nbmon);
    ...
    void *Q)
{
  ...
  struct bap_polynom_mpq *B = (struct bap_polynom_mpq *) Q;
...
/*
 * texinfo: bad_compare_rank_polynom_mpq
 * A comparison function for sorting tables of polynomials
 * by increasing rank, using @code{qsort}.
 */

BAP_DLL int
bap_compare_rank_polynom_mpq (
    const void *PP,
    const void *QQ)
{
  struct bap_polynom_mpq *P = *(struct bap_polynom_mpq **) PP;
  struct bap_polynom_mpq *Q = *(struct bap_polynom_mpq **) QQ;

  if (bap_lt_rank_polynom_mpq (P, Q))
    return -1;
  else if (bap_equal_rank_polynom_mpq (P, Q))
    return 0;
  else
    return 1;
}
...
bap_initial_and_reductum_polynom_mpq (
    struct bap_polynom_mpq *initial,
    struct bap_polynom_mpq *reductum,
    struct bap_polynom_mpq *A)
{
  ...
  if (bap_is_numeric_polynom_mpq (A))
    BA0_RAISE_EXCEPTION (BAP_ERRCST);
  if ((A == initial && reductum != BAP_NOT_A_POLYNOM_mpq) || (A == initial
          && A == reductum))
    ...
    struct bav_variable *v)
{
  ...
  if (bap_is_numeric_polynom_mpq (A) || (v != BAV_NOT_A_VARIABLE
          && v != bap_leader_polynom_mpq (A)))
    {
      if (!bap_depend_polynom_mpq (A, v))
      ...
      else
        {
          ...
          bap_initial_and_reductum_polynom_mpq (lcoeff !=
              BAP_NOT_A_POLYNOM_mpq ? &lc : BAP_NOT_A_POLYNOM_mpq,
              reductum !=
              BAP_NOT_A_POLYNOM_mpq ? &red : BAP_NOT_A_POLYNOM_mpq, &AA);
  ...
  else
    {
      ...
      bap_initial_and_reductum_polynom_mpq (lcoeff !=
          BAP_NOT_A_POLYNOM_mpq ? &lc : BAP_NOT_A_POLYNOM_mpq,
          reductum != BAP_NOT_A_POLYNOM_mpq ? &red : BAP_NOT_A_POLYNOM_mpq,
          A);
    ...
    bav_Idegree d)
{
  ...
  if (R == (struct bap_polynom_mpq *) 0)
    ...
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C != (struct bap_polynom_mpq *) 0 && C->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C == R)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (bap_is_numeric_polynom_mpq (A) || (v != BAV_NOT_A_VARIABLE
          && !bap_depend_polynom_mpq (A, v)))
    {
      if (d == 0)
        {
          if (C != A && C != (struct bap_polynom_mpq *) 0)
            bap_set_polynom_mpq (C, A);
          bap_set_polynom_zero_mpq (R);
        }
      else
        {
          if (C != (struct bap_polynom_mpq *) 0)
  ...
  if (C != A && R != A)
    {
      ba0_pull_stack ();
      if (C != (struct bap_polynom_mpq *) 0)
        bap_begin_creator_mpq (&creaC, C, &A->total_rank,
            bap_approx_total_rank, nbmon / 2 + 1);
      bap_begin_creator_mpq (&creaR, R, &A->total_rank, bap_approx_total_rank,
          nbmon / 2 + 1);
      ba0_push_another_stack ();
    }
  else
    {
      ...
      if (C != (struct bap_polynom_mpq *) 0)
        bap_begin_creator_mpq (&creaC, &CC, &A->total_rank,
            bap_approx_total_rank, nbmon / 2 + 1);
...
comp_polynom_ascending (
    ...
    const void *y)
{
  struct bap_polynom_mpq *A = *(struct bap_polynom_mpq * *) x;
  struct bap_polynom_mpq *B = *(struct bap_polynom_mpq * *) y;
  struct bav_term TA, TB;
  struct ba0_mark M;
  enum ba0_compare_code code;
    ...
    enum ba0_sort_mode mode)
{
  switch (mode)
    ...
    case ba0_descending_mode:
      qsort (T->tab, T->size, sizeof (struct bap_polynom_mpq *),
          &comp_polynom_descending);
      break;
...
static char _struct_polynom_rang[] = "struct bap_polynom_mpq *->total_rank";
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    n += ba0_new_gc_info (A, sizeof (struct bap_polynom_mpq),
        _struct_polynom);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (struct bap_polynom_mpq *) ba0_new_addr_gc_info (AA, _struct_polynom);
  else
    A = (struct bap_polynom_mpq *) AA;

  A->clot =
      (struct bap_clot_mpq *) bap_garbage2_clot_mpq (A->clot, ba0_isolated);
