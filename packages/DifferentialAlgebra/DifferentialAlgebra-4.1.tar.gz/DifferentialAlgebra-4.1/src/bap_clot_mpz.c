...
/*
   Allocate a struct bap_table2of_monom_mpz * with a capacity of at least n monomials.
   Initialize the coefficients.
*/

static struct bap_table2of_monom_mpz *
new_table2of_monom_mpz (
    struct bap_termanager *tgest,
    ba0_int_p n)
{
  struct bap_table2of_monom_mpz *mont;
  ba0_int_p i;

  mont =
      (struct bap_table2of_monom_mpz *) ba0_alloc (sizeof (struct
    ...
    struct bap_clot_mpz *C)
{
  ...
  if (C == (struct bap_clot_mpz *) 0)
    return;

  if (C->tab.size > C->tab.alloc || C->size > C->alloc)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (C->size > 0 && C->tab.tab == (struct bap_table2of_monom_mpz * *) 0)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  for (i = 0; i < C->tab.size; i++)
    {
      if (C->tab.tab[i] == (struct bap_table2of_monom_mpz *) 0)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  while (i < C->tab.alloc
      && C->tab.tab[i] != (struct bap_table2of_monom_mpz *) 0)
    cumul_alloc += C->tab.tab[i++]->alloc;

  if (cumul_alloc != C->alloc)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
#else
  C = (struct bap_clot_mpz *) 0;
#endif
}
...
/*
  ...
  The sub-arrays of struct bap_table2of_monom_mpz * are shared.
*/

static void
init_set_clot_mpz (
    struct bap_clot_mpz *C,
    struct bap_clot_mpz *D)
{
  C->alloc = D->alloc;
  C->size = D->size;
...
BAP_DLL struct bap_clot_mpz *
bap_new_clot_mpz (
    struct bav_term *T)
{
  struct bap_clot_mpz *C;

  C = (struct bap_clot_mpz *) ba0_alloc (sizeof (struct bap_clot_mpz));
  init_clot_mpz (C, T);
  return C;
}
...
bap_next_itermon_clot_mpz (
    struct bap_itermon_clot_mpz *iter)
{
  struct bap_table2of_monom_mpz *m;
...
{
  struct bap_table2of_monom_mpz *mi, *mj;
    ...
    ba0_int_p numero)
{
  ...
  tab = (struct bap_table2of_monom_mpz * *) iter->clot->tab.tab;
  primary = 0;
  secondary = numero;
 ...
 * If some new arrays of type @code{struct bap_table2of_monom_mpz *} are
...
/*
  Returns the struct bap_table2of_monom_mpz * crea points to.
  Allocates a struct bap_table2of_monom_mpz * if there is no place left.
  ...
  if (clot->alloc == crea->iter.num.combined)
    {
      if (clot->tab.alloc == crea->iter.num.primary)
        {
          struct bap_table2of_monom_mpz **new_tab;
          ba0_int_p n, new_alloc;

          new_alloc = 2 * clot->tab.alloc + 1;
          new_tab =
              (struct bap_table2of_monom_mpz * *) ba0_alloc (sizeof (void *) *
              new_alloc);
          memcpy (new_tab, clot->tab.tab,
              clot->tab.alloc * sizeof (struct bap_table2of_monom_mpz *));
          for (n = clot->tab.alloc; n < new_alloc; n++)
            new_tab[n] = (struct bap_table2of_monom_mpz *) 0;
          clot->tab.alloc = new_alloc;
          clot->tab.tab = (struct bap_table2of_monom_mpz * *) new_tab;
        }
      mont =
...
{
  struct bap_itermon_clot_mpz iter;
  struct bap_table2of_monom_mpz *d_mont, *s_mont;
    ...
    struct bap_creator_clot_mpz *crea)
{
  ...
  for (i = clot->tab.size; i < clot->tab.alloc; i++)
    {
      m = clot->tab.tab[i];
      if (m != (struct bap_table2of_monom_mpz *) 0)
        m->size = 0;
    }
  check_clot_mpz (clot);
}
...
static char _struct_table2of_monom_zipterm[] =
    "struct bap_table2of_monom_mpz *->zipterm";
static char _struct_table2of_monom_coeff[] =
    "struct bap_table2of_monom_mpz *->coeff";

static ba0_int_p
garbage1_table2of_monom_mpz (
    struct bap_termanager *tgest,
    struct bap_table2of_monom_mpz *A,
    enum ba0_garbage_code code)
{
  ba0_int_p n = 0;

  if (A == (struct bap_table2of_monom_mpz *) 0)
    return 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info (A, sizeof (struct bap_table2of_monom_mpz),
        _struct_mont);
...
static char _struct_clot_tab[] = "struct bap_clot_mpz *->tab.tab";
...
bap_garbage1_clot_mpz (
    ...
    enum ba0_garbage_code code)
{
  struct bap_clot_mpz *A = (struct bap_clot_mpz *) AA;
  ba0_int_p i, n = 0;
  ...
  if (A->tab.tab)
    n += ba0_new_gc_info (A->tab.tab,
        sizeof (struct bap_table2of_monom_mpz *) * A->tab.alloc,
        _struct_clot_tab);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (struct bap_table2of_monom_mpz *) ba0_new_addr_gc_info (A,
        _struct_mont);
  ...
  A->coeff =
      (ba0_mpz_t *) ba0_new_addr_gc_info (A->coeff, _struct_table2of_monom_coeff);
...
bap_garbage2_clot_mpz (
    ...
    enum ba0_garbage_code code)
{
  struct bap_clot_mpz *A;
  ba0_int_p i;

  if (code == ba0_isolated)
    A = (struct bap_clot_mpz *) ba0_new_addr_gc_info (AA, _struct_clot);
  else
    A = (struct bap_clot_mpz *) AA;

  if (A->tab.tab)
    A->tab.tab =
        (struct bap_table2of_monom_mpz * *) ba0_new_addr_gc_info (A->tab.tab,
        _struct_clot_tab);
  ...
  for (i = 0; i < A->tab.alloc; i++)
    A->tab.tab[i] =
        garbage2_table2of_monom_mpz (&A->tgest, A->tab.tab[i], ba0_isolated);
