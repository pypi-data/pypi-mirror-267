...
class TestDifferentialAlgebra(unittest.TestCase):
    def test_54(self):
        ...
        P = v[x,x] - 2
        S = A.prem (P)
        self.assertEqual (S, (v[x,x] - 2))
        P = v[x,x] - 2/Integer(3)
        S = A.prem (P)
        self.assertEqual (S, (3*v[x,x] - 2)/3)
        A = RegularDifferentialChain ([(u+1)*(v[x]**2 - 4*v), u[x]**2 - 4*u], R, attributes = ['prime', 'autoreduced', 'differential'])
        P = v[x,x] - 2
        S = A.prem (P)
