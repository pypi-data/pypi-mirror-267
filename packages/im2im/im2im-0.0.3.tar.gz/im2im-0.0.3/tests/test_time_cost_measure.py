...
from src.im2im.time_cost_measure import time_cost, time_cost_in_kg
...
@patch("src.im2im.test_image_util.random_test_image_and_expected")
...
def test_time_cost_in_kg(kg):
    costs = [1, 2, 3, 4]
    with patch('src.im2im.time_cost_measure.time_cost') as mock_time_cost:
