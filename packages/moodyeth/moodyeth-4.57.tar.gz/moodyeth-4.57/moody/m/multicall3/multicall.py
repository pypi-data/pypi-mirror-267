...
from .constants import (ASYNC_SEMAPHORE, GAS_LIMIT,
                                 MULTICALL2_ADDRESSES, MULTICALL3_ADDRESSES,
                                 MULTICALL3_BYTECODE, w3)
from .loggers import setup_logger
from .utils import (await_awaitable, chain_id, gather,
...
def _raise_or_proceed(e: Exception, ct_calls: int, ConnErr_retries: int) -> None:
    ...
    if isinstance(e, aiohttp.ClientOSError):
        ...
        if "('Connection aborted.', ConnectionResetError(104, 'Connection reset by peer'))" not in str(
                e) or ConnErr_retries > 5:
            raise e
