...
class BmobResult:
	...
	def __getattr__(self, name):  return self.jsonData[name] if name in self.jsonData else None
...
def BmobRequest(url, method = 'GET', headers = None, body = None, timeout = 10):
	...
	if method != None:
		...
		try:
			...
			return BmobResult().init(res.read())
			...
			try:
				if hasattr(e, "reason"):
					...
					reason = None
				return BmobResult().init(e.read())
			except Exception as e:
				print("Req failed wih response.init:", e)
				errMsg = "Unknown Error"
				return BmobResult().init({'results':errMsg})
	else:
		errMsg = "Unknown Error"
		return BmobResult().init({'results':errMsg})
...
class Bmob:
	...
	def findObjects(self, className, where = None, limit = None, skip = None, order = None, include = None, keys = None, count = None, groupby = None, groupcount = None, min = None, max = None, sum = None, average = None, having = None):
		...
		try:
			...
			print(e)
			return BmobResult().init({'results':msg})
