...
def load_configs(filenames=None, section="sardana"):
...
#:
#: - `False` - use names instead of numeric IDs in configuration
#:             (available since 3.4.0 release)
#: - `True` (default) -  use numeric IDs in configuration
USE_NUMERIC_ELEMENT_IDS = True
...
#:
#: - `False` (default) - keep using the old properties
#: - `True` -  migrate existing devices (create new property and
#:    delete the old one) and start using the new property for new devices
USE_PHYSICAL_ROLES_PROPERTY = False
