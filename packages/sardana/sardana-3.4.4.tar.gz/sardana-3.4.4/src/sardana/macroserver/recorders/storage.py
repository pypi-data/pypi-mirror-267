...
class FIO_FileRecorder(BaseFileRecorder):
    ...
    def _startRecordList(self, recordlist):
        ...
        for e in envRec['datadesc']:
            # For backward compatibility, 1D channels of 1 element
            # are added to ctNames and not mcaNames
            # See https://gitlab.com/sardana-org/sardana/-/issues/1740
            if len(e.shape) == 1 and e.shape[0] > 1:
...
class SPEC_FileRecorder(BaseFileRecorder):
    ...
    def _startRecordList(self, recordlist):
        ...
        snapshot_labels, snapshot_values, snapshot_labels_oned, snapshot_values_oned = \
            self._preparePreScanSnapshot(env)
        # format scan header
        data = {
            'serialno':  serialno,
            'title':     env['title'],
        ...
        for i, label in enumerate(snapshot_labels_oned):
            if i == 100:
                break
            header += "#U{:02d} {} = ".format(i, label)
            for value in snapshot_values_oned[i]:
                header += " " + str(value)
            header += "\n"
        header += '#N %(nocols)s\n'
    ...
    def _preparePreScanSnapshot(self, env):
        ...
        for column_desc in pre_scan_snapshot:
            ...
            if dtype not in self.supported_dtypes:
                ...
                continue
            if len(shape) > 0:
                labels_oned.append(label)
                values_oned.append(pre_scan_value)
            else:
                labels.append(label)
        ...
        return labels_chunks, values_chunks, labels_oned, values_oned
