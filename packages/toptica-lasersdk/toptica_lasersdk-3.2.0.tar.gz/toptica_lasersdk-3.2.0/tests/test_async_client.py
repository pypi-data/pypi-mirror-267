...
async def test_param_ref_bool(test, expected):

    connection = MockConnection([test])

    async with Client(connection) as client:
        result = await client.get('param-name', bool)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("test, expected", [('#t', True), ('#f', False)])
async def test_param_gsv_bool(test, expected):

    connection = MockConnection([test])

    async with Client(connection) as client:
        result = await client.get_set_value('param-name', bool)
...
@pytest.mark.asyncio
async def test_param_ref_atomic():

    connection = MockConnection(['(#t 5 2.5 "test" "YWQgYXN0cmEgcGVyIGFzcGVyYQ==")'])

    async with Client(connection) as client:
        result = await client.get('param-name', bool, int, float, str, bytes)
...
async def test_param_ref_atomic_error(test, expected):

    connection = MockConnection([test])

    with pytest.raises(DecopError):
        async with Client(connection) as client:
            await client.get('param-name', *expected)
...
async def test_param_ref_inferred(test, expected):

    connection = MockConnection([test])

    async with Client(connection) as client:
        result = await client.get('param-name')
...
@pytest.mark.asyncio
async def test_param_ref_atomic_inferred():
...
async def test_get_set_value(test, expected):

    connection = MockConnection([test])

    async with Client(connection) as client:
        result = await client.get_set_value('param-name', type(expected))
...
async def test_get_set_value_inferred(test, expected):

    connection = MockConnection([test])

    async with Client(connection) as client:
        result = await client.get_set_value('param-name')
...
@pytest.mark.asyncio
async def test_get_set_value_atomic():

    connection = MockConnection(['(#t 5 2.5 "test" "YWQgYXN0cmEgcGVyIGFzcGVyYQ==")'])

    async with Client(connection) as client:
        result = await client.get_set_value('param-name', bool, int, float, str, bytes)
...
@pytest.mark.asyncio
async def test_get_set_value_atomic_inferred():
...
async def test_get_set_value_atomic_error(test, expected):

    connection = MockConnection([test])

    with pytest.raises(DecopError):
        async with Client(connection) as client:
            await client.get_set_value('param-name', *expected)
...
async def test_param_set_bool(test, expected):

    connection = MockConnection(['5'])

    async with Client(connection) as client:
        result = await client.set('param-name', test)
...
@pytest.mark.asyncio
async def test_param_set_atomic():

    connection = MockConnection(['5'])

    async with Client(connection) as client:
        result = await client.set('param-name', True, 5, 2.5, 'test', b'ad astra per aspera')
...
async def test_exec(test, expected):

    connection = MockConnection(['()'])

    async with Client(connection) as client:
        await client.exec('command-name', *test)
...
async def test_exec_inputstream(test, expected):

    connection = MockConnection(['()'])

    async with Client(connection) as client:
        await client.exec('command-name', input_stream=test)
...
@pytest.mark.parametrize("test, expected, t",
                         [('()', '', str), ('baba\n()', 'baba\n', str), ('()', b'', bytes),
                          ('YmFiYQ==#\n()', b'baba', bytes), ('YmFi\nYQ==#\n()', b'baba', bytes)])
async def test_exec_output_type(test, expected, t):

    connection = MockConnection([test])

    async with Client(connection) as client:
        output = await client.exec('command-name', output_type=t)
...
async def test_exec_return_type(test, expected, t):

    connection = MockConnection([test])

    async with Client(connection) as client:
        ret = await client.exec('command-name', return_type=t)
...
async def test_exec_output_type_return_type(out_test, out_expected, out_t, ret_test, ret_expected, ret_t):

    connection = MockConnection([out_test + '\n' + ret_test])

    async with Client(connection) as client:
        out, ret = await client.exec('command-name', output_type=out_t, return_type=ret_t)
...
async def test_exec_missing_response(test, expected):

    connection = MockConnection([test])

    with pytest.raises(DecopError) as exc:
        async with Client(connection) as client:
            await client.exec('command-name')
...
async def test_exec_single_line_error(test, expected):
...
async def test_exec_multi_line_error(test, expected):
...
@pytest.mark.parametrize("test, expected",
                         [('Valid Data\nError: -1 Message', 'Error: -1 Message'),
                          ('Valid Data 1\nValid Data 2\nError: -1 Message', 'Error: -1 Message')])
async def test_exec_data_before_single_line_error(test, expected):
...
async def test_change_ul_empty_password_success(test, cmd_req, cmd_resp, mon_req, expected):

    connection = MockConnection([cmd_resp])

    async with Client(connection) as client:
        result = await client.change_ul(test, '')
...
@pytest.mark.parametrize("test, cmd_req, cmd_resp, mon_req, expected",
                         [(UserLevel.READONLY, '(exec \'change-ul 4 "")\n', "4", '(change-ul 4 "")\n', UserLevel.READONLY),
                          (UserLevel.NORMAL,   '(exec \'change-ul 3 "")\n', "3", '(change-ul 3 "")\n', UserLevel.NORMAL)])
async def test_change_ul_no_password_success(test, cmd_req, cmd_resp, mon_req, expected):
...
async def test_change_ul_empty_password_failure(test):
...
@pytest.mark.asyncio
@pytest.mark.parametrize("test", [UserLevel.MAINTENANCE, UserLevel.SERVICE, UserLevel.INTERNAL])
async def test_change_ul_no_password_failure(test):
...
@pytest.mark.asyncio
async def test_cancel_last_subscription():

    connection = MockConnection()

    async with Client(connection) as client:
        subscription = await client.subscribe('uptime', str)
...
@pytest.mark.asyncio
async def test_cancel_subscription_on_close():

    connection = MockConnection()

    async with Client(connection) as client:
        await client.subscribe('uptime', str)
...
@pytest.mark.asyncio
async def test_run_callback_2():

    connection = MockConnection(['3'])

    async with Client(connection) as client:
        subscription1 = await client.subscribe('uptime', str)
        subscription2 = await client.subscribe('uptime', str)
