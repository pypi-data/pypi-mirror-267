...
class Illuminat(MDApp):
    ...
    openai.api_key = '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'
...
7            device_id = str(uuid.uuid4())
    ...
    def chatClairty(self, prompt):
        response = openai.Completion.create(
            engine="gpt-3.5-turbo-instruct",  # You can use the latest engine available
            prompt=prompt,
            max_tokens=40,
            temperature=0.7
        )
    ...
    def generate_and_download_images(self, prompt, filename):
        ...
        # Assuming generate_image returns a JSON response
        response_json = self.generate_image(prompt_json, num_image=1, size='256x256', output_format='url')

        if response_json and 'created' in response_json and 'media' in response_json:
            print(response_json['created'])
            ...
            for i, media_item in enumerate(media):
                if 'url' in media_item:
                    ...
                    self.download_image(image_url, filename=filename)
                else:
                    print("Error: 'url' not found for media item.")
        else:
    ...
    def image_to_video_science(self):
        try:
            ...
            tenthline_firstword = words[9]
            text_png = ".png"
            eightline_firstword_pngggg = str(eigthline_firstword) + str(text_png)
            tenthline_firstword_pngggg = str(tenthline_firstword) + str(text_png)
            ...
            clip1 = ImageClip(eightline_firstword_pngggg).set_duration(clip1duration)
            clip2 = ImageClip(tenthline_firstword_pngggg).set_duration(clip2duration)
            clips.append(clip1)
            clips.append(clip2)
            ...
            print("Here reached")
            audio_clip = AudioFileClip(self.mastervariablemp3updated)
    ...
    def changechangemastervar(self):
        ...
        idfortheclicktostart.text = "CLICK"
    ...
    def change_video_source(self):
        ...
        clicktostarttext = self.root.get_screen('varscript').ids.clicktostarttext
    ...
    def get_audio_duration(self):
        if os.path.exists(self.mastervariablemp3updated):
        ...
        try:
            ...
            if text_to_convert and selected_language:
                ...
                if translated_text.strip():
                    ...
                    patherrrr = self.mastervariablemp3updated
                    if self.stored_lang == "English":
                        langfortts = "en-US"
                    elif self.stored_lang == "Hindi":
                        langfortts = "hi-IN"
    ...
    def changetonototamil(self):
        ...
        if storedLang == "Tamil":
            ...
            for kv_file in [clairtypathjj, bfquestionsiorjhti, bfquestssionsiorjhti, homepathrgth, loginojih, mainhjyio, prevarscriptavgpathnf, prevarscriptavgpoorpathnf, profilepathiohjotijhioyioja, profilepathiohjotijhioyioja, questionspathroj, questionspathrscopyojaa, questionspatolderhrscopyojaa, questionsscreenopioyhj, sinuppathriohj, sinudetailsppathriohj, varscioptpathtoph, varsciquestionsoptpathtoph, videoplaycodejrgij]:
