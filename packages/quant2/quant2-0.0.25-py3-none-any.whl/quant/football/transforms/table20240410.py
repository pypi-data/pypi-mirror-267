...
def check_samples(data: list[list]):
    ...
    for sample in data:
        a = int(sample[6]) + int(sample[7])  # 主队+客队
        b = sum([sample[i] for i in range(13, 121, 9)])  # 进球
        c = sum([sample[i] for i in range(17, 121, 9)])  # 乌龙
        d = sum([sample[i] for i in range(18, 121, 9)])  # 点球
        if a == (b + c + d):
            good_data.append(sample)
            continue
        bad_data.append(sample)
        bad_info.append((a, b, c, d))
...
def export_dataset(data: list[list], stages: int, encoder: Union[str, dict], whole: bool, nc: int, transforms: Union[str, list]):
    ...
    # [121:126] : [联赛,杯赛,已过3轮,已过6轮,已过9轮次]
    # [126:156] : [平均,最大,最小,范围,数量]x[*初赔]
    assert -1 < stages < 7
    sub_len = 9 * stages
    ...
    for _season, _home, _away, _vals, _outs, _qkey in zip(season_id, team_home, team_away, features_in, scores, qkeys):
        if _season not in encoder["season"] or _home not in encoder["team"] or _away not in encoder["team"]:
            continue
        X.append(encoder["season"][_season] + encoder["team"][_home] + encoder["team"][_away] + _vals)
        if whole:
