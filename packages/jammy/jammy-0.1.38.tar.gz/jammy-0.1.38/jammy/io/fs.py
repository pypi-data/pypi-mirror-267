...
def load_video(file, size=None, **kwargs):
    ...
    Parameters:
        ...
        size (tuple, optional): Desired size (width, height) to resize each frame. If None, frames are not resized.

    Returns:
        np.ndarray: Numpy array containing the video frames in the format (frames, height, width, channels).
                    The frames are returned in RGB format.
...
def dump_video(file: str, frames: np.ndarray, fps: int = 30, **kwargs):
    ...
    Additional keyword arguments are passed to `imageio.get_writer()` allowing customization of codec, bitrate, etc.
...
io_function_registry.register("dump", ".", dump_video)
