...
class AsgEbsManager:
    ...
    def latest_volume(self,az,state):
        ...
        if len(volumes['Volumes']) > 0:
            sorted_volume = [volume['VolumeId'] for volume in sorted(volumes['Volumes'], key=lambda volume: datetime.strptime(volume['CreateTime'], '%Y-%m-%dT%H:%M:%S.%f%z'))]
            return sorted_volume[0]
        return None

    def create_volume(self,az,size,iops,throughput,snapshot_id=None):
    ...
    def describe_volume(self,volume_id):
        ...
        volumes = self.ec2_client.client.describe_volumes(**args)['Volumes']
        volume = None
        if len(volumes) > 0:
            volume = volumes[0]
        if volume is None: 
    ...
    def describe_snapshot(self,snapshot_id):
        ...
        snapshots = self.ec2_client.client.describe_snapshots(**args)['Snapshots']
        snapshot = None
        if len(snapshots) > 0:
