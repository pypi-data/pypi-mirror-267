...
def check_remote_host(host):
    command = "docker swarm join --token dummy core2.ign8.it:2377"
    result = subprocess.run(command, capture_output=True, shell=True)
    pprint.pprint(result.returncode)
    print("--------------------------")
    if result.returncode == 130:
        # The remote host has a swarm runing"
        prettylog("INFO", f"Remote host {host} has a swarm running")
        return True
    else:
        prettylog("ERROR", f"Remote host {host} does not have a swarm running")
        return False
    ...
    for clusterserver in mycluster:
        ...
        if host_is_up(clusterserver):
            ...
            if swarmdetect:
                ...
                cstat[swarmdetect] = True
            else:
                cstat[clusterserver]['swarm'] = "down"
        else:
            cstat[clusterserver]['ip'] = "down"
            cstat[clusterserver]['swarm'] = "unknown"

    prettylog("INFO", "Cluster status", cstat)
