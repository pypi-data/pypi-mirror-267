...
def get_unique_id():
    instance_id = re.search(r'.*(.{3})$', gcp_utils.get_instance_id_short())
    process_id = str(os.getpid())
    thread_id = re.search(r'.*(.{4})$', str(threading.get_ident()))
...
class LoadGen:
    ...
    def start_workloads(self):
        ...
        while True:
            ...
            thread_id = re.search(r'.*(.{5})$', threading.get_ident())
            print(f"({get_unique_id()}) - Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    ...
    def run(self):
        ...
        thread_id = re.search(r'.*(.{5})$', threading.get_ident())
        print(f"({get_unique_id()}) - Delaying {self.start_delay} before starting work loop.")
