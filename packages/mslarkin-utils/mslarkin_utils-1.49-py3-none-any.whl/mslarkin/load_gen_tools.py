...
def compute_primes(cpu_intensity, start_at=1e15, max_primes=25):
    prime_count = 0
    number = start_at if start_at % 2 == 1 else start_at - 1
    time_hack = time.time()
    while prime_count < max_primes:
        if (time.time()-time_hack) >= 60:
            print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Computing primes...")
            time_hack = time.time()
        if is_prime(number):
            prime_count += 1
        number += 2
...
def triggered_w_load(cpu_intensity=0, max_primes=1, memory_mb=1):
    ...
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Starting triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}, memory_mb: {memory_mb}")
    start = time.time()
    ...
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Completed triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}. Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    return f"Done. Completed in {end-start}s"


def triggered_no_load(sleep_time=1):
    print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Starting triggered no-load process.  Sleep time: {sleep_time}")
    start = time.time()
    time.sleep(sleep_time)
    end = time.time()
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Completed triggered no-load process.  Sleep time: {sleep_time}.  Completed in {end-start} seconds.")
    return f"Done. Completed in {end-start}s"
...
class LoadGen:
    ...
    def start_workloads(self):
        ...
        while True:
            ...
            print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    ...
    def run(self):
        ...
        print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Delaying {self.start_delay} before starting work loop.")
        time.sleep(self.start_delay)
        while True:
            if self.randomize_work_break:
                self.work_break = random.randint(0, self.work_break)
            ...
            print(
                f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Starting a work loop: CPU Intensity: {self.cpu_intensity} ({get_cpu_load(self.cpu_intensity)}), max_primes: {self.max_primes}, memory_mb: {self.memory_mb}")
            self.start_workloads()
            print(
                f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{threading.get_ident()}) - Waiting for {self.work_break} seconds before starting next work loop (Random: {self.randomize_work_break})"
            )
