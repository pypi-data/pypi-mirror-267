...
def compute_primes(cpu_intensity, start_at=1e15, max_primes=25):
    ...
    time_hack = time.time()
    thread_id = re.search(r'.*(.{5})$', threading.get_ident())
    while prime_count < max_primes:
        if (time.time()-time_hack) >= 60:
            print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Computing primes...")
            time_hack = time.time()
        if is_prime(number):
            prime_count += 1
        number += 2
...
def triggered_w_load(cpu_intensity=0, max_primes=1, memory_mb=1):
    ...
    thread_id = re.search(r'.*(.{5})$', threading.get_ident())
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Starting triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}, memory_mb: {memory_mb}")
    start = time.time()
    ...
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Completed triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}. Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    return f"Done. Completed in {end-start}s"


def triggered_no_load(sleep_time=1):
    thread_id = re.search(r'.*(.{5})$', threading.get_ident())
    print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Starting triggered no-load process.  Sleep time: {sleep_time}")
    start = time.time()
    time.sleep(sleep_time)
    end = time.time()
    print(
        f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Completed triggered no-load process.  Sleep time: {sleep_time}.  Completed in {end-start} seconds.")
    return f"Done. Completed in {end-start}s"
...
class LoadGen:
    ...
    def start_workloads(self):
        ...
        while True:
            ...
            thread_id = re.search(r'.*(.{5})$', threading.get_ident())
            print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    ...
    def run(self):
        ...
        thread_id = re.search(r'.*(.{5})$', threading.get_ident())
        print(f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Delaying {self.start_delay} before starting work loop.")
        ...
        while True:
            ...
            print(
                f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Starting a work loop: CPU Intensity: {self.cpu_intensity} ({get_cpu_load(self.cpu_intensity)}), max_primes: {self.max_primes}, memory_mb: {self.memory_mb}")
            self.start_workloads()
            print(
                f"({gcp_utils.get_instance_id_short()} PID:{os.getpid()} Thread:{thread_id}) - Waiting for {self.work_break} seconds before starting next work loop (Random: {self.randomize_work_break})"
            )
