...
def make_edges(page, clip=None, tset=None):
    ...
    for bbox in bboxes:
        line_dicts = [
            make_line(path, bbox.tl, bbox.tr, clip),
            make_line(path, bbox.bl, bbox.br, clip),
            make_line(path, bbox.tl, bbox.bl, clip),
            make_line(path, bbox.tr, bbox.br, clip)
        ]
        if all(len(line_dict) > 0 for line_dict in line_dicts):
...
def find_tables(
    ...
    # ensure we extract minimum char bboxes
