...
class EvalMetricAnalysis(object):
    ...
    def _format_to_object_detection(self, predictions: List[Dict], references: List[Dict]):
        ...
        for item in references:
            ...
            if item.get("annotations") is None:
                ...
                reference_dict[im_id_int].append(anno)
                continue
            for anno in item["annotations"]:
                ...
                for idx in range(len(anno["labels"])):
                    ...
                    reference_dict[im_id_int].append(anno_copy)
                    ann_id += 1
        ...
        for item in predictions:
            ...
            if item["annotations"] is None or im_id_int not in reference_dict:
                continue
            for anno in item["annotations"]:
                anno["image_id"] = im_id_int
                for idx in range(len(anno["labels"])):
                    ...
                    if anno_copy["labels"][idx]["id"] not in self.label_id2index:
                        continue
                    anno_copy['category_id'] = anno_copy["labels"][idx]["id"]
                    anno_copy['score'] = anno_copy["labels"][idx]["confidence"]
                    prediction_list.append(anno_copy)
                    ann_id += 1
        ...
        return {"bbox": prediction_list}, {"bbox": reference_list}
    ...
    def _format_to_semantic_segmentation(self, predictions: List[Dict], references: List[Dict]):
        ...
        for item in references:
            ...
            if item.get("annotations") is None:
                reference_dict[im_id_int] = label_raw
                continue
            for anno in item["annotations"]:
                rle = anno["rle"]
            ...
            reference_dict[im_id_int] = {"mask": label_raw, "width": width, "height": height}
        ...
        for item in predictions:
            ...
            prediction_dict[im_id_int] = label_raw

        reference_list = []
        prediction_list = []
        for img_id, raw in reference_dict.items():
            reference_list.append(raw["mask"])
            # 只有同时拥有gt和预测结果才参与指标计算
            if img_id in prediction_dict:
                prediction_list.append(prediction_dict[img_id])
