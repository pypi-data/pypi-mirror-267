...
def get_pg_conn() -> Engine:
    ...
    db_name = os.environ.get("CREDIT_DB_NAME")
    if not db_name:
...
def get_sqlite_conn() -> Engine:
    db_name = os.environ.get("CREDIT_DB_NAME", "opencredit.db")
    db_path = os.environ.get("CREDIT_DB_PATH", "./data")
    db_test = os.environ.get("CREDIT_DB_TEST", "false") == "true"
    if db_test:
        db_name = f"opencredit_test_{int(time.time())}.db"
