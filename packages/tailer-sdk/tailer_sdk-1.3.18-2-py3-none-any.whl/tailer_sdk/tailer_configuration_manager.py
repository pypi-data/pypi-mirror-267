...
def deploy_configuration(
    ...
    try:
        ...
        # # DEBUG
        # #
        # url = "http://localhost:8080/configuration/v2"
...
def process_cf_deployment(
    ...
    if no_gcp_cf_deploy is True:
        ...
        toBeDeployed = check_configuration_for_cf_deploy(input_configuration)

        if toBeDeployed is True:
...
def process_configuration_context(
    ...
    if read_configuration["configuration_type"] == "table-to-storage":
        try:
            ...
            #
            for index, job in enumerate(read_configuration["tasks"]):
                read_configuration["tasks"][index]["sql"] = \
                    (base64.b64decode(job["sql"])).decode("utf-8")
    ...
    if read_configuration["configuration_type"] == "table-to-storage":
        try:
            ...
            # Above version 3
            #
            for index, job in enumerate(contextualized_configuration["tasks"]):

                contextualized_configuration["tasks"][index]["sql"] = \
                    str(base64.b64encode(job["sql"].encode("utf-8")), "utf-8")
                ...
                if args.arguments[1] is not None:
                    if args.arguments[1] == "help":
                        ...
                        if check_table_to_table(
                            ...
                            if read_configuration is None:
                                read_configuration = \
                                    process_configuration_file(args.arguments[1])
                                
                                doc_md = read_configuration["doc_md"]
