...
def cli():
    commands = [
        ...
        (["play", "PYQIE_APP_FILE(.pyxapp)"], play_pyqie_app),
        (["edit", "[PYQIE_RESOURCE_FILE(.pyxres)]"], edit_pyqie_resource),
        (["package", "APP_DIR", "STARTUP_SCRIPT_FILE(.py)"], package_pyqie_app),
        (["app2exe", "PYQIE_APP_FILE(.pyxapp)"], create_executable_from_pyqie_app),
        (["app2html", "PYQIE_APP_FILE(.pyxapp)"], create_html_from_pyqie_app),
        (["copy_examples"], copy_pyqie_examples),
    ]
    ...
    def print_usage(command_name=None):
        ...
        for command in commands:
            if command_name is None or command[0] == command_name:
                print(f"    pyqie {' '.join(command[0])}")
        _check_newer_version()

    num_args = len(sys.argv)
    if num_args <= 1:
        print(f"Pyqie {pyqie.VERSION}, a retro game engine for Python")
        print_usage()
        return
    for command in commands:
...
def _check_newer_version():
    url = "https://www.github.com/xiatian0716/pyqie"
    req = urllib.request.Request(url)
    latest_version = None
    try:
        with urllib.request.urlopen(req, timeout=3) as res:
            pattern = r"/xiatian0716/pyqie/releases/tag/v(\d+\.\d+\.\d+)"
    ...
    if parse_version(latest_version) > parse_version(pyqie.VERSION):
        print(f"A new version, Pyqie {latest_version}, is available.")
...
def _create_app_dir():
    play_dir = os.path.join(tempfile.gettempdir(), pyqie.WORKING_DIR, "play")
    ...
    for path in glob.glob(os.path.join(play_dir, "*")):
        ...
        if not pyqie.process_exists(pid):
            shutil.rmtree(path)
...
def _create_watch_info_file():
    watch_dir = os.path.join(tempfile.gettempdir(), pyqie.WORKING_DIR, "watch")
...
def watch_and_run_python_script(watch_dir, python_script_file):
    ...
    os.environ[pyqie.WATCH_INFO_FILE_ENVVAR] = _create_watch_info_file()
    try:
...
def _extract_pyqie_app(pyqie_app_file):
    pyqie_app_file = _complete_extension(pyqie_app_file, pyqie.APP_FILE_EXTENSION)
    _check_file_exists(pyqie_app_file)
    app_dir = _create_app_dir()
    zf = zipfile.ZipFile(pyqie_app_file)
    zf.extractall(app_dir)
    pattern = os.path.join(app_dir, "*", pyqie.APP_STARTUP_SCRIPT_FILE)
...
def play_pyqie_app(pyqie_app_file):
    startup_script_file = _extract_pyqie_app(pyqie_app_file)
    if startup_script_file:
        ...
        return
    print(f"file not found: '{pyqie.APP_STARTUP_SCRIPT_FILE}'")
    sys.exit(1)


def edit_pyqie_resource(pyqie_resource_file=None, starting_editor="image"):
    import pyqie.editor

    pyqie_resource_file = pyqie_resource_file or "my_resource"
    pyqie_resource_file = _complete_extension(
        pyqie_resource_file, pyqie.RESOURCE_FILE_EXTENSION
    )
    pyqie.editor.App(pyqie_resource_file, starting_editor)


def package_pyqie_app(app_dir, startup_script_file):
    ...
    setting_file = os.path.join(app_dir, pyqie.APP_STARTUP_SCRIPT_FILE)
    with open(setting_file, "w") as f:
        f.write(os.path.relpath(startup_script_file, app_dir))
    pyqie_app_file = os.path.basename(app_dir) + pyqie.APP_FILE_EXTENSION
    ...
    with zipfile.ZipFile(
        ...
        for file in files:
            if os.path.basename(file) == pyqie_app_file or "/__pycache__/" in file:
                continue
...
def create_executable_from_pyqie_app(pyqie_app_file):
    pyqie_app_file = _complete_extension(pyqie_app_file, pyqie.APP_FILE_EXTENSION)
    _check_file_exists(pyqie_app_file)
    app2exe_dir = os.path.join(tempfile.gettempdir(), pyqie.WORKING_DIR, "app2exe")
    if os.path.isdir(app2exe_dir):
        shutil.rmtree(app2exe_dir)
    pathlib.Path(app2exe_dir).mkdir(parents=True, exist_ok=True)
    pyqie_app_name = os.path.splitext(os.path.basename(pyqie_app_file))[0]
    startup_script_file = os.path.join(app2exe_dir, pyqie_app_name + ".py")
    with open(startup_script_file, "w") as f:
        f.write(
            "import os, pyqie.cli; pyqie.cli.play_pyqie_app("
            f"os.path.join(os.path.dirname(__file__), '{pyqie_app_name}.pyxapp'))"
        )
    ...
    command += f"--add-data {pyqie_app_file}{os.pathsep}. "
    modules = pyqie.utils.list_imported_modules(_extract_pyqie_app(pyqie_app_file))[
        "system"
    ]
    ...
    if os.path.isdir(app2exe_dir):
        shutil.rmtree(app2exe_dir)
    spec_file = os.path.splitext(pyqie_app_file)[0] + ".spec"
    if os.path.isfile(spec_file):
        os.remove(spec_file)


def create_html_from_pyqie_app(pyqie_app_file):
    pyqie_app_file = _complete_extension(pyqie_app_file, pyqie.APP_FILE_EXTENSION)
    _check_file_exists(pyqie_app_file)
    base64_string = ""
    with open(pyqie_app_file, "rb") as f:
        base64_string = base64.b64encode(f.read()).decode()
    pyqie_app_name = os.path.splitext(os.path.basename(pyqie_app_file))[0]
    with open(pyqie_app_name + ".html", "w") as f:
        f.write(
            "<!DOCTYPE html>\n"
            '<script src="https://cdn.jsdelivr.net/gh/xiatian0716/pyqie/wasm/pyqie.js">'
            "</script>\n"
            "<script>\n"
            f'launchPyqie({{ command: "play", name: "{pyqie_app_name}.pyxapp", '
            f'gamepad: "enabled", base64: "{base64_string}" }});\n'
            "</script>\n"
        )


def copy_pyqie_examples():
    src_dir = os.path.join(os.path.dirname(__file__), "examples")
    dst_dir = "pyqie_examples"
