...
from datasets.annotations.rbbox import RBBoxXYCenterWHA
from datasets.image_dataset import ImagesDataset
from datasets.meta import DatasetMeta, FileInfo, DatasetPart, DatasetType
from datasets.annotations.sbbox import BBoxXYXY, BBoxXYWH, BBoxXYCenterWH
...
def import_dataset(main_data_folder: Path, image_set: DatasetPart) -> ImagesDataset:
    ...
    if image_set.value == 'train':
        ...
        ann_file = main_data_folder / 'Train' / 'MW-R-annotations.json'
    if image_set.value == 'test':
        data_dir = main_data_folder / 'Test'
        ann_file = None
    else:
    ...
    for video_path in video_frame_dirs:
        ...
        for frame_image_path in frame_images:
            ...
            image_id = video_file_name[5:8]+video_file_name[9:] + '_' + frame_image_path.stem
            ...
            objects = []
            if frame_anns_dict is not None:
                for ann in frame_anns_dict[image_id]:
                    objects.append(
                        DetectionObjectBBox(
