...
class Challenger():
    ...
    def CalcularShapValuesLGBM(self, Corrida):
        
        train_undersampled_df, testing_df_m1, testing_df_m2, testing_df_m3 = self.LeerTablas()
        Nombre_Modelo = 'LGBM_' + str(Corrida)
        LGBM_scaler_train, LGBM_model_train = self.EntrenarModeloPandas('LGBM', train_undersampled_df, 
                                                                        self.PORCENTAJE_TRAINING, 
                                                                        self.LGBM_param_test,
                                                                        Nombre_Modelo)
        ...
        X_test = LGBM_model_train.predict_proba(train_undersampled_df.toPandas().drop([self.TGT_VARIABLES, 
                                                                                       self.CAMPO_CLAVE], axis=1))
        ...
        train = train_undersampled_df.toPandas()[[self.TGT_VARIABLES, self.CAMPO_CLAVE]]
