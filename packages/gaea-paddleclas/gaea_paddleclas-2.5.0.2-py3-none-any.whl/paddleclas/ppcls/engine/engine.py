...
import os
import platform
from jsonargparse import LoggerProperty
import paddle
...
class Engine(object):
    def __init__(self, config, mode="train"):
        ...
        self.work_dir = os.path.abspath(os.environ.get('WINDMILL_EXPERIMENT_WORK_DIR', 
                                        os.path.join(self.output_dir, "wwd")))
                        
        if not os.path.exists(self.work_dir) and self.local_rank == 0:
    ...
    def train(self):
        ...
        if self.best_schema_metric is not None and self.local_rank == 0:
            import json
            import fcntl
...
class ExportMultiAttrModel(TheseusLayer):
    """
    export multi attr model
    """
    ...
    def forward(self, x):
        ...
        if isinstance(output, list):
            ...
            out_class_ids = []            
            for head in output:
                res_output = F.softmax(head, axis=1)
