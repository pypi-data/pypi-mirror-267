...
class DeviceController(MainController):
    ...
    def delete_device_profile(self, device_profile_id: DeviceProfileId):
        ...
        """ return self.tb_client.delete_device_profile(
            device_profile_id=device_profile_id, async_req=True) """
    ...
    def get_tenant_devices(self):
        devices = self.tb_client.get_tenant_devices(page=0, page_size=100000)
        return devices.data

    def get_customer_devices(self, customer_id: CustomerId):
        devices = self.tb_client.get_customer_devices(
            customer_id=customer_id, page=0, page_size=100000)
        return devices.data

    def get_tenant_devices_profiles(self) -> List:
        device_list = list()
        devices = self.get_tenant_devices()
        for device in devices:
            profile = self.tb_client.get_device_profile_by_id(
    ...
    def get_customer_devices_profiles(self, customer_id: CustomerId) -> List:
        device_list = list()
        devices = self.get_customer_devices(customer_id=customer_id)
        for device in devices:
            profile = self.tb_client.get_device_profile_by_id(
                device.device_profile_id)
            item = dict()
