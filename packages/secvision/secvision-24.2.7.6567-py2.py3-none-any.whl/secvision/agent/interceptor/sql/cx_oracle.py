...
"""Intercept the SecVision appd_cx_oracle package.
...
def parse_oracle_kwargs_dsn(**kwargs):
    dsn = kwargs.pop('dsn')
    # added dsn regex check for kwargs of conn string
    dsn_match = DSN_RE.match(dsn)
    if dsn_match:
        return parse_cx_oracle_tns_dsn(dsn)
    else:
        return parse_cx_oracle_tns_url(dsn)
...
def intercept_cx_oracle_connection(agent, mod):
    ...
    class SessionPool(mod.SessionPool):
        def __init__(self, *args, **kwargs):
            self.args = args
            self.kwargs = kwargs
            super(SessionPool, self).__init__(*args, **kwargs)

        def acquire(self, *args, **kwargs):
            new_kwargs = kwargs
            # putting session pool obj in pool key in kwargs
            # since we are overriding the implemetation of
            # acquire method we need exact same replica
            # of the acquire method in session pool
            new_kwargs['pool'] = self
            return Connection(*args, **new_kwargs, dsn=getattr(self, 'dsn', None))
