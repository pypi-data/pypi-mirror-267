...
def test_replace_header_values(write_l1_task):
    ...
    header = task._replace_header_values(header=header, data=data)
...
def test_calculate_date_avg(write_l1_task):
    ...
    """
    task, _, header = write_l1_task
    assert task._calculate_date_avg(header=header) == "2020-01-02T12:00:00.000000"
...
def test_calculate_telapse(write_l1_task):
    ...
    """
    task, _, header = write_l1_task
    assert task._calculate_telapse(header=header) == 86400
...
def test_calculate_date_end(write_l1_task):
    ...
    """
    task, _, header = write_l1_task
    assert task._calculate_date_end(header=header) == "2020-01-02T00:00:00.100000"
...
def test_add_contributing_id_headers(write_l1_task):
    ...
    """
    task, _, header = write_l1_task
    header = task._add_contributing_id_headers(header=header)
...
def test_spectral_line_keys(write_l1_task, mocker):
    ...
    task, _, header = write_l1_task
    header = task._add_datacenter_headers(header=header, hdu_size=1024, stokes="I")
