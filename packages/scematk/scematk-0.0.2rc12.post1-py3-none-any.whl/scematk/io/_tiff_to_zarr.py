...
def tiff_to_zarr(tiff_path: str, zarr_path: str, meta_path: str, tile_size: int = 4096, chunk_size: int = 4096):
    ...
    with OpenSlide(tiff_path) as slide:
        ...
        out_img = zarr.zeros(out_shape, dtype='uint8', chunks=(chunk_size, chunk_size, 3), store = zarr.DirectoryStore(zarr_path))
        ...
        for y_min, x_min in tqdm(product(y_mins, x_mins), total=num_iter):
            ...
            tile = slide.read_region((x_min, y_min), 0, (x_max - x_min, y_max - y_min)).convert('RGB')
