...
class Arabic_Time:
    def __init__(self, time_object: Union[datetime.time, datetime.datetime]) -> None:
        if not isinstance(time_object, datetime.time) and not isinstance(time_object, datetime.datetime):
            raise TypeError(
                f"Arabic_Time class error: The parameter provided to the instance is not a datetime.time object nor a datetime.datetime object.")
    ...
    def time(self, separator: str = ":") -> str:
        if not isinstance(separator, str):
            raise TypeError(
                f"Arabic_Time class error: 'separator' parameter must be a string. Non string value was passed to the method '{self.time.__name__}'.")
