def hash_masking_policy_all(self, dbs:list[dict])->dict:
    data = {}
    for config in dbs:
        ...
        BODY = config['BODY'] if 'OWNER' in config else None
        
        data[config['FULL_POLICY_NAME']] = {}
        data[config['FULL_POLICY_NAME']]['owner'] = OWNER
        data[config['FULL_POLICY_NAME']]['db_hash'] = self.hash_masking_policy(SIGNATURE, RETURN_TYPE, EXEMPT_OTHER_POLICIES, OWNER, COMMENT, TAGS, BODY, GRANTS)
