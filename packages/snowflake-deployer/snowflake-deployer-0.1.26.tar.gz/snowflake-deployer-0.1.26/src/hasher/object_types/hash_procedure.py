...
import json
def hash_procedure(self, input_args:list, is_secure:bool, returns:str, language:str, null_handling:str, execute_as:str, owner:str, comment:str, tags:list, body:str, grants:list, imports:list, handler:str, runtime_version:str, packages:list)->str:
    ...
    null_handling_n = null_handling if null_handling is not None else ''
    execute_as_n = execute_as if execute_as is not None else ''
    ...
    # Also not including null_handling_n since this is not included in the example and generally uses the default time.  

    tpl = (input_args_n, is_secure_n, language_n, execute_as_n, owner_n, comment_n, tags_n, body_n, grants_n, imports_n, handler_n, runtime_version_n, packages_n)
