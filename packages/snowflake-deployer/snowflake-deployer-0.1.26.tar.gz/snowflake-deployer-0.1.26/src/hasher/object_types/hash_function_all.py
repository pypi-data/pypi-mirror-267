def hash_function_all(self, dbs:list[dict])->dict:
    data = {}
    for config in dbs:
        ...
        data[config['FULL_FUNCTION_NAME']] = {}
        data[config['FULL_FUNCTION_NAME']]['owner'] = OWNER
        data[config['FULL_FUNCTION_NAME']]['db_hash'] = self.hash_function(INPUT_ARGS, IS_SECURE, RETURNS, LANGUAGE, OWNER, COMMENT, TAGS, BODY, GRANTS, IMPORTS, HANDLER, RUNTIME_VERSION, PACKAGES)
