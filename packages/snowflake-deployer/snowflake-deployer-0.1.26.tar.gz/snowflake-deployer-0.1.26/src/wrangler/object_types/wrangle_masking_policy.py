...
def _get_grants(self_sf, full_policy_name:str, grant_dict:dict):
    grants_raw = self_sf._sf.grants_get(full_policy_name, 'masking policy')
...
def _get_tag_references(self_sf, database_name:str, full_policy_name:str, tag_dict:dict):
    tags_raw = self_sf._sf.tag_references_get(database_name, full_policy_name, 'masking policy')
    tag_dict[full_policy_name] = tags_raw

def wrangle_masking_policy(self, database_name:str, schema_name:str, env_database_prefix:str, env_role_prefix:str, deploy_db_name:str, ignore_roles_list:str, deploy_tag_list:list[str], current_role:str, available_roles:list[str], handle_ownership, semaphore)->dict:
    ...
    tag_dict = {}
    for d in polices:
        full_policy_name = database_name + '.' + schema_name + '.' + d['MASKING_POLICY_NAME'].replace(' ','')
