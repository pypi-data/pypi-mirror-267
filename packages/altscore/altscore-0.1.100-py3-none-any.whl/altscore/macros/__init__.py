...
class MacrosSync:
    ...
    def create_borrower(self, borrower_data: dict) -> BorrowerSync:
        ...
        for field_key in borrower_fields_to_create:
            ...
            self.altscore_client.borrower_central.borrower_fields(
                {
                    "borrower_id": borrower_id,
                    "key": key,
                    "value": value
        ...
        if len(borrower_data.get("identities", [])) > 0:
            for identity in borrower_data["identities"]:
                identity_id = self.altscore_client.borrower_central.addresses.create(
                    {
                        "borrower_id": borrower_id,
                        **identity
                    }
                ...
                if len(identity.get("attachments", [])) > 0:
                    identity_obj = self.altscore_client.borrower_central.addresses.retieve(identity_id)
        ...
        if len(borrower_data.get("borrower_fields", [])) > 0:
            for borrower_field in borrower_data["borrower_fields"]:
                borrower_field_id = self.altscore_client.borrower_central.addresses.create(
                    {
                        "borrower_id": borrower_id,
                        **borrower_field
                    }
                )
                if len(borrower_field.get("attachments", [])) > 0:
                    borrower_field_obj = self.altscore_client.borrower_central.addresses.retieve(borrower_field_id)
                    for attachment in borrower_field["attachments"]:
                        borrower_field_obj.post_attachment(attachment)
        ...
        if len(borrower_data.get("documents", [])) > 0:
            for document in borrower_data.get("documents"):
                ...
                if len(document.get("attachments", [])) > 0:
                    document_obj = self.altscore_client.borrower_central.addresses.retieve(document_id)
...
class MacrosAsync:
    ...
    async def create_borrower(self, borrower_data: dict) -> BorrowerAsync:
        ...
        for field_key in borrower_fields_to_create:
            key = field_key.split(".")[-1]
            value = borrower_data[field_key]
            calls.append(self.altscore_client.borrower_central.borrower_fields(
                {
                    "borrower_id": borrower_id,
                    "key": key,
                    "value": value
        ...
        if len(borrower_data.get("identities", [])) > 0:
            for identity in borrower_data["identities"]:
                identity_id = await self.altscore_client.borrower_central.addresses.create(
                    {
                        "borrower_id": borrower_id,
                        **identity
                    }
                ...
                if len(identity.get("attachments", [])) > 0:
                    identity_obj = await self.altscore_client.borrower_central.addresses.retieve(identity_id)
                    for attachment in identity["attachments"]:
                        await identity_obj.post_attachment(attachment)

        if len(borrower_data.get("borrower_fields", [])) > 0:
            for borrower_field in borrower_data["borrower_fields"]:
                borrower_field_id = await self.altscore_client.borrower_central.addresses.create(
                    {
                        "borrower_id": borrower_id,
                        **borrower_field
                    }
                ...
                if len(borrower_field.get("attachments", [])) > 0:
                    borrower_field_obj = await self.altscore_client.borrower_central.addresses.retieve(
                        borrower_field_id)
                    for attachment in borrower_field["attachments"]:
                        await borrower_field_obj.post_attachment(attachment)
        ...
        if len(borrower_data.get("documents", [])) > 0:
            for document in borrower_data.get("documents"):
                ...
                if len(document.get("attachments", [])) > 0:
                    document_obj = await self.altscore_client.borrower_central.addresses.retieve(document_id)
                    for attachment in document["attachments"]:
