...
def cli(ctx, awsprofile):
    ...
    click.echo(
        click.style(
            '\nThis CLI will be deprecated soon, please use the new one for Okta\n'
            'https://cloud.docs.zalando.net/reference/zaws/index.html\n',
            bold=True,
            fg="yellow",
        )
    )
...
def login(obj, account_role_or_alias, refresh, awsprofile):
    ...
    new_role_name = profile.role_name
    if profile.role_name == 'PowerUser' or profile.role_name == 'Emergency':
        new_role_name = 'Manual'

    new_command = click.style(
            '"zalando-aws-cli login {} {}"\n'.format(profile.account_name, new_role_name),
            bold=True,
            fg="yellow",
    )
    click.echo('Please run instead {}'.format(new_command))
