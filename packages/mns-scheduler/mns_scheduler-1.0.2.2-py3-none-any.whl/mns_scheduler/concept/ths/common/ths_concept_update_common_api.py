...
def update_ths_concept_detail(ths_symbol_all_concepts, symbol):
    ...
    for concept_one in ths_symbol_all_concepts.itertuples():
        try:
            ths_concept_one_db = all_ths_concept.loc[all_ths_concept['web_concept_code'] == int(concept_one.cid)]
            if data_frame_util.is_not_empty(ths_concept_one_db):
                concept_code = list(ths_concept_one_db['symbol'])[0]
