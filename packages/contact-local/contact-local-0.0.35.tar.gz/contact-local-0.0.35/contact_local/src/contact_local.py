...
class ContactsLocal(GenericCRUD):
    ...
    def __get_data_json_compare_for_upsert_contact_dict(self, contact_dict: dict) -> dict:
        data_json_compare = {}
        if 'email1' in contact_dict:
            data_json_compare['email1'] = contact_dict.get('email1', None)
        if 'phone1' in contact_dict:
            data_json_compare['phone1'] = contact_dict.get('phone1', None)
        if 'email2' in contact_dict:
            data_json_compare['email2'] = contact_dict.get('email2', None)
        if 'phone2' in contact_dict:
            data_json_compare['phone2'] = contact_dict.get('phone2', None)
        if 'email3' in contact_dict:
            data_json_compare['email3'] = contact_dict.get('email3', None)
        if 'phone3' in contact_dict:
            data_json_compare['phone3'] = contact_dict.get('phone3', None)
        if len(data_json_compare) == 0:
            if 'website1' in contact_dict:
                data_json_compare['website1'] = contact_dict.get('website1', None)
            elif 'first_name' in contact_dict:
                data_json_compare['first_name'] = contact_dict.get('first_name', None)
        else:
            logger.error("can't find email or phone in contact_dict for compare_data_json")
            raise ContactInsertionException("compare_data_json cannot be empty when contact_dict does not have email or phone")
