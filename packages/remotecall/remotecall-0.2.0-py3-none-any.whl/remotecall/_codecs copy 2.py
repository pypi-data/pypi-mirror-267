...
class NoneCodec(Codec[NoneType, NoneType]):
    ...
    @classmethod
    def is_matching_type(cls, class_object: Type[Any]) -> bool:
        return class_object is type(None)
...
class NamedTupleCodec(Codec[NamedTuple, R]):
    CONTENT_TYPE = APPLICATION_TUPLE
    ...
    def create_for_type(cls, class_object: Type[T]) -> Codec:
        ...
        if cls.CONTENT_TYPE == APPLICATION_DICT:
            return NamedTupleCodec[class_object, dict]()
        if cls.CONTENT_TYPE == APPLICATION_TUPLE:
            return NamedTupleCodec[class_object, tuple]()
    ...
    def decode(self, data: bytes, content_type: str) -> NamedTuple:
        class_object = self.get_decode_type()

        if content_type in (APPLICATION_DICT, APPLICATION_NAMEDTUPLE):
...
class DataClassCodec(Codec[T, R]):
    ...
    def create_for_type(cls, class_object: Type) -> Codec:
        ...
        if cls.CONTENT_TYPE == APPLICATION_DATACLASS:
