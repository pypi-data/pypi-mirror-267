...
async def deploy_debug(app: App, project_directory_name: str) -> None:
    ...
    benthos_echo_config = {
        ...
        "pipeline": {
            "processors": [
                {"mapping": 'foo.payload = root\n foo.subject = meta("_seaplane_request_id")'},
                # {"mapping": 'foo.payload = root\nfoo.subject = meta("nats_subject")'},
                {
                    "cache": {
                        "resource": "response",
                        "operator": "add",
    ...
    #         "volumes": ["./resources/webhook-replicate.py:/var/scripts/webhook-replicate.py:ro"],
    #         "configs": [
    #             {
    ...
    #             {
    #                 "id": "replicate"
    #                 "execute_command": "/var/scripts/webhook-replicate.py"
    #                 "command-working-directory": "/var/webhook"
    #                 "pass-arguements-to-command": [
    #                     {
    ...
    for dag in app.dag_registry.values():
        for task in dag.flow_registry.values():
            ...
            with open(
                ...
                yaml.dump(new_flow_config["io_config"], stream)
    ...
    services = ["nats1", "nats2", "nats3", "qdrant", "redis"]
    ...
    for base_store_name in SP_UTIL_STORES:
        ...
        await create_store_debug(store_name, replicas=1)
...
async def destroy_debug(app: App) -> None:
    ...
    #     for dag in app.dag_registry.values():
    #         await delete_stream_debug(dag.name)
    #     for stream in SP_UTIL_STREAMS:  # TODO: take this out after testing?
    #         await delete_stream_debug(stream)
