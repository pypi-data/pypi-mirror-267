...
class Station:
    ...
    def _measure(self) -> List[float]:
        results = []
        for param, gain in self._params:
            measurement = param()
            if isinstance(measurement, (int, float)):
                results.append(measurement / gain)
            else:
                results.append(np.mean(measurement) / gain)
        return results
    ...
    @_interruptible
    def watch(self, delay: float=0.0, max_duration=None, want_setpoint_column=True):
        
        if want_setpoint_column:
            with db.Writer(self._basedir, fsync_every=1 if delay >= 10.0 else 10) as w, self._plotter as p:
                self._print(f'Starting run with ID {w.id}')
                w.metadata['type'] = '1D'
                w.metadata['delay'] = delay
                w.metadata['max_duration'] = max_duration
                w.metadata['columns'] = ['time', 'dummy_setpoint_column'] + self._col_names()
                ...
                while max_duration is None or time.monotonic() - t_start < max_duration:
                    time.sleep(delay)
                    data = [time.time(), time.time()] + self._measure()
                    w.add_point(data)
                    p.add_point(data)
    ...
    def megasweep(self, slow_param, slow_v, fast_param, fast_v, slow_delay=0, fast_delay=0):
        with db.Writer(self._basedir, fsync_every=1 if ((slow_delay >= 10.0) or (fast_delay >= 10.0)) else 10) as w, self._plotter as p:
            self._print(f'Starting run with ID {w.id}')
    ...
    def sweep_vna_1d(self, delay: float=0.0):
        with db.Writer(self._basedir, fsync_every=1000) as w, self._plotter as p:
            instruments_used = []
            param_indices = []
            for i in range(len(self._params)):
                instrument = str(self._params[i][0].instrument)
    ...
    def sweep_vna_2d(self, slow_param, slow_v, slow_delay=0):
        with db.Writer(self._basedir, fsync_every=5001 if slow_delay >= 10.0 else 10002) as w, self._plotter as p:
            instruments_used = []
            param_indices = []
            for i in range(len(self._params)):
                instrument = str(self._params[i][0].instrument)
