...
class LinkValidator:
    ...
    def resolve_link(self, root: Path, link: str):
        resolved_link = link
    ...
    def is_local_link(self, link: str):
        if not link.endswith(".md"):
            return False
        return not (link.startswith("http") or link.startswith("#"))

    def find_link(self, root: Path, link: str, pages: list):
        resolved_link = self.resolve_link(root, link)
        for page in pages:
    ...
    def validate_links(self, pages):
        ...
        for page in pages:
            ...
            for link in page.links:
                href = link["href"]
                if not self.is_local_link(href):
                    continue
                link_target = self.find_link(page_dir, href, pages)
                if not link_target:
                    failed_links.append((href, page.file_path))
                    continue
