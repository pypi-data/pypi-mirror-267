...
from test_tele.plugin_models import FileType
from test_tele.utils import cleanup, stamp
...
class TgcfMessage:
    def __init__(self, message: Message) -> None:
        ...
        self.links = self.get_entity_link()
        self.new_file = None
        self.cleanup = False
        self.reply_to = None
    ...
    def get_entity_link(self) -> str:
        index = 0
        entity_link = []
        for url_entity, inner_text in self.message.get_entities_text(MessageEntityTextUrl):
            url = url_entity.url
            has_url = False
            for link in entity_link:
                if re.search(rf'{re.escape(url)}', link):
                    has_url = True
                    break
            if not has_url:
                index += 1
                entity_link.append(f"[{index}]({url})")
        text = f"\n\n{(' | ').join(entity_link)}" if entity_link else '' 
        return text
...
def load_plugins() -> Dict[str, TgcfPlugin]:
    ...
    for item in PLUGINS:
        ...
        try:  # try to load first party plugin
            plugin_module = import_module("test_tele.plugins." + plugin_id)
