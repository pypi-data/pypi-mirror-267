...
def test_architecture_validate_architecture(default_arch: str) -> None:
    assert settings.Architecture(architecture=default_arch)  # type: ignore[arg-type]  # nosec: B101

    with raises(ValueError):
        settings.Architecture(architecture="foo")  # type: ignore[arg-type]
...
def test_systemsettings(
    ...
    get_default_packagerepo_mock.return_value = settings.PackageRepo(
        ...
        name="default",  # type: ignore[arg-type]
...
def test_urlvalidationsettings(
    ...
    with expectation:
        settings.UrlValidationSettings(urls=urls, tls_required=tls_required)  # type: ignore[arg-type]
...
@mark.parametrize(
    ...
    [
        (
            AnyUrl("https://foobar.io/bar/baz"),
            True,
        ),
        (
            AnyUrl("http://foobar.io/bar/baz"),
            False,
        ),
        (
            AnyUrl("https://foobeh.io/bar/baz"),
            False,
        ),
    ],
)
def test_urlvalidationsettings_validate_url(
    ...
    caplog.set_level(DEBUG)
    urls: list[HttpUrl] = [HttpUrl("https://foobar.io/")]
    validator = settings.UrlValidationSettings(urls=urls, tls_required=True)
