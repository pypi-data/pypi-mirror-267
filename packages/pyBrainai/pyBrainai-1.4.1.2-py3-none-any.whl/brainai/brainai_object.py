...
class brainaiObject(dict):
    ...
    def __init__(
        ...
        **params,
        ...
        super(brainaiObject, self).__init__()
    ...
    def __setattr__(self, k, v):
        if k[0] == "_" or k in self.__dict__:
            return super(brainaiObject, self).__setattr__(k, v)
    ...
    def __delattr__(self, k):
        if k[0] == "_" or k in self.__dict__:
            return super(brainaiObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(brainaiObject, self).__setitem__(k, v)
    ...
    def refresh_from(
        ...
        for k, v in values.items():
            super(brainaiObject, self).__setitem__(
                k, util.convert_to_brainai_object(v, api_key, api_version, organization)
            )
    ...
    def request(
        ...
        if stream:
            assert not isinstance(response, brainaiResponse)  # must be an iterator
            return (
                util.convert_to_brainai_object(
                    line,
                    api_key,
                    self.api_version,
                    self.organization,
            ...
            return util.convert_to_brainai_object(
                response,
                api_key,
                self.api_version,
                self.organization,
    ...
    def to_dict_recursive(self):
        d = dict(self)
        for k, v in d.items():
            if isinstance(v, brainaiObject):
                d[k] = v.to_dict_recursive()
            elif isinstance(v, list):
                d[k] = [
                    e.to_dict_recursive() if isinstance(e, brainaiObject) else e
                    for e in v
                ]
        return d
    ...
    def __copy__(self):
        ...
        for k, v in self.items():
            ...
            super(brainaiObject, copied).__setitem__(k, v)
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in self.items():
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(brainaiObject, copied).__setitem__(k, deepcopy(v, memo))
