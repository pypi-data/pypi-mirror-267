...
import brainai
from brainai.datalib.numpy_helper import numpy as np
...
def get_embedding(text: str, engine="text-similarity-davinci-001", **kwargs) -> List[float]:
    ...
    return brainai.Embedding.create(input=[text], engine=engine, **kwargs)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await brainai.Embedding.acreate(input=[text], engine=engine, **kwargs))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = brainai.Embedding.create(input=list_of_text, engine=engine, **kwargs).data
...
async def aget_embeddings(
    ...
    data = (await brainai.Embedding.acreate(input=list_of_text, engine=engine, **kwargs)).data
    return [d["embedding"] for d in data]
