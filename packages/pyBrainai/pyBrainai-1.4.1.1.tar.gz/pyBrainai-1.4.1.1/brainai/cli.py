...
import brainai
from brainai.upload_progress import BufferReader
from brainai.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = brainai.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = brainai.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        if args.completions and args.completions > 1 and args.stream:
            raise ValueError("Can't stream multiple completions with brainai CLI")

        kwargs = {}
        if args.model is not None:
            kwargs["model"] = args.model
        resp = brainai.Engine(id=args.id).generate(
...
class ChatCompletion:
    ...
    def create(cls, args):
        ...
        resp = brainai.ChatCompletion.create(
            # Required
            model=args.model,
            engine=args.engine,
            messages=messages,
...
class Completion:
    ...
    def create(cls, args):
        ...
        resp = brainai.Completion.create(
            engine=args.engine,
            model=args.model,
            n=args.n,
            max_tokens=args.max_tokens,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = brainai.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = brainai.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = brainai.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = brainai.Deployment.create(
...
class Model:
    @classmethod
    def get(cls, args):
        resp = brainai.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = brainai.Model.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = brainai.Model.list()
        print(models)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = brainai.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = brainai.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = brainai.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = brainai.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
    ...
    def create_edit(cls, args):
        ...
        resp = brainai.Image.create_edit(
            image=image_reader,
            mask=mask_reader,
            prompt=args.prompt,
            size=args.size,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = brainai.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = brainai.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = brainai.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            brainai.File.retrieve(file)
            return file
        except brainai.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = brainai.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = brainai.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = brainai.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise brainai.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = brainai.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = brainai.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise brainai.error.brainaiError(
                message=(
                    "The --stream parameter is deprecated, use fine_tunes.follow "
                    "instead:\n\n"
                    "  brainai api fine_tunes.follow -i {id}\n".format(id=args.id)
                ),
            )

        resp = brainai.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = brainai.FineTune.retrieve(job_id).status
            sys.stdout.write(
                "\nStream interrupted. Job is still {status}.\n"
                "To resume the stream, run:\n\n"
                "  brainai api fine_tunes.follow -i {job_id}\n\n"
                "To cancel your job, run:\n\n"
                "  brainai api fine_tunes.cancel -i {job_id}\n\n".format(
        ...
        events = brainai.FineTune.stream_events(job_id)
        ...
        resp = brainai.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
            sys.stdout.write("\nJob complete! Status: succeeded 🎉")
            sys.stdout.write(
                "\nTry out your fine-tuned model:\n\n"
                "brainai api completions.create -m {model} -p <YOUR_PROMPT>".format(
                    model=resp["fine_tuned_model"]
                )
            )
        elif status == "failed":
            sys.stdout.write(
                "\nJob failed. Please contact us through our help center at help.brainai.com if you need assistance."
            )
        sys.stdout.write("\n")

    @classmethod
    def cancel(cls, args):
        resp = brainai.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = brainai.FineTune.delete(sid=args.id)
        print(resp)
...
class FineTuningJob:
    ...
    def list(cls, args):
        has_ft_jobs = False
        for fine_tune_job in brainai.FineTuningJob.auto_paging_iter():
            has_ft_jobs = True
            print(fine_tune_job)
        if not has_ft_jobs:
    ...
    def _maybe_upload_file(
        ...
        sys.stdout.flush()
        status = brainai.File.wait_for_processing(resp["id"])
        if status != "processed":
            raise brainai.error.brainaiError(
    ...
    def create(cls, args):
        ...
        resp = brainai.FineTuningJob.create(**create_args)
        print(resp)
        return

    @classmethod
    def get(cls, args):
        resp = brainai.FineTuningJob.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = brainai.FineTuningJob.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise brainai.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = brainai.FineTuningJob.retrieve(id=args.id)["result_files"][0]
        resp = brainai.File.download(id=result_file)
        print(resp.decode("utf-8"))
    ...
    def events(cls, args):
        ...
        while has_more:
            resp = brainai.FineTuningJob.list_events(id=args.id, after=after)  # type: ignore
    ...
    @classmethod
    def cancel(cls, args):
        resp = brainai.FineTuningJob.cancel(id=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import brainai.wandb_logger

        resp = brainai.wandb_logger.WandbLogger.sync(
            id=args.id,
...
Mutually exclusive with `top_p`.""",
    ...
    group.add_argument(
        "-e",
        "--engine",
        help="The engine to use. See https://learn.microsoft.com/en-us/azure/cognitive-services/brainai/chatgpt-quickstart?pivots=programming-language-python for more about what engines are available.",
    )
    group.add_argument(
        "-m",
        "--model",
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        "-e",
        "--engine",
        help="The engine to use. See https://platform.brainai.com/docs/engines for more about what engines are available.",
    )
    sub.add_argument(
        "-m",
        "--model",
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        "-p",
        "--purpose",
        help="Why are you uploading this file? (see https://platform.brainai.com/docs/api-reference/ for purposes)",
        required=True,
    )
    ...
    sub.add_argument(
        ...
        "be the ID of a file uploaded through the brainai API (e.g. file-abcde12345), "
    ...
    sub.add_argument(
        ...
        'For example, `brainai api fine_tunes.create -t test.jsonl -m ada --suffix "custom model name" '
    ...
    sub.add_argument(
        ...
        "This can be the ID of a file uploaded through the brainai API (e.g. file-abcde12345), "
        'a local file path, or a URL that starts with "http".',
    )
    sub.add_argument(
        "-v",
    ...
    sub.add_argument(
        ...
        'For example, `brainai api fine_tuning.job.create -t test.jsonl -m gpt-3.5-turbo-0613 --suffix "first finetune!" '
        "could generate a model with the name "
