...
import brainai
from brainai import api_requestor, error, util
from brainai.api_resources.abstract.api_resource import APIResource
...
class EngineAPIResource(APIResource):
    ...
    def class_url(
        ...
        if typed_api_type in (ApiType.AZURE, ApiType.AZURE_AD):
            ...
            if engine is None:
                raise error.InvalidRequestError(
                    "You must provide the deployment name in the 'engine' parameter to access the Azure brainai service"
                )
    ...
    def create(
        ...
        **params,
        ...
        if stream:
            # must be an iterator
            assert not isinstance(response, brainaiResponse)
            return (
            ...
            obj = util.convert_to_brainai_object(
                response,
                api_key,
                api_version,
                organization,
    ...
    def instance_url(self):
        ...
        if self.typed_api_type in (ApiType.AZURE, ApiType.AZURE_AD):
            api_version = self.api_version or brainai.api_version
            if not api_version:
                raise error.InvalidRequestError(
                    "An API version is required for the Azure API type."
                )
