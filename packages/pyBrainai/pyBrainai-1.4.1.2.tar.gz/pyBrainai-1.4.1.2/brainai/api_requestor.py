...
import brainai
from brainai import error, util, version
from brainai.brainai_response import brainaiResponse
from brainai.util import ApiType
...
def _requests_proxies_arg(proxy) -> Optional[Dict[str, str]]:
    ...
    if proxy is None:
        ...
        raise ValueError(
            "'brainai.proxy' must be specified as either a string URL or a dict with string URL under the https and/or http keys."
        )
...
def _make_session() -> requests.Session:
    if brainai.requestssession:
        if isinstance(brainai.requestssession, requests.Session):
            return brainai.requestssession
        return brainai.requestssession()
    if not brainai.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; brainai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(brainai.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or brainai.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(brainai.api_type)
        )
        self.api_version = api_version or brainai.api_version
        self.organization = organization or brainai.organization
    ...
    def _check_polling_response(self, response: brainaiResponse, predicate: Callable[[brainaiResponse], bool]):
        if not predicate(response):
            return
        error_data = response.data['error']
        message = error_data.get('message', 'Operation failed')
        code = error_data.get('code')
        raise error.brainaiError(message=message, code=code)
    ...
    def _poll(
        ...
        interval = None,
        delay = None
    ) -> Tuple[Iterator[brainaiResponse], bool, str]:
        if delay:
            time.sleep(delay)
    ...
    def request(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[brainaiResponse, bool, str]:
        pass
    ...
    def request(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[Union[brainaiResponse, Iterator[brainaiResponse]], bool, str]:
        pass
    ...
    async def arequest(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[AsyncGenerator[brainaiResponse, None], bool, str]:
        pass
    ...
    async def arequest(
        ...
        request_id: Optional[str] = ...,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = ...,
    ) -> Tuple[Union[brainaiResponse, AsyncGenerator[brainaiResponse, None]], bool, str]:
        pass
    ...
    def request_headers(
        ...
        user_agent = "brainai/v1 PythonBindings/%s" % (version.VERSION,)
        if brainai.app_info:
            user_agent += " " + self.format_app_info(brainai.app_info)
        ...
        if brainai.app_info:
            ua["application"] = brainai.app_info

        headers = {
            "X-brainai-Client-User-Agent": json.dumps(ua),
        ...
        if self.organization:
            headers["brainai-Organization"] = self.organization

        if self.api_version is not None and self.api_type == ApiType.RUBIK_AI:
            headers["brainai-Version"] = self.api_version
        if request_id is not None:
            headers["X-Request-Id"] = request_id
        if brainai.debug:
            headers["brainai-Debug"] = "true"
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "brainai bindings. Please contact us through our help center at help.brainai.com for "
                "assistance." % (method,)
            )

        headers = self.request_headers(method, headers, request_id)

        util.log_debug("Request to brainai API", method=method, path=abs_url)
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(brainai.proxy),
            "timeout": timeout,
        }
        try:
            ...
            raise error.APIConnectionError("Error communicating with brainai") from e

    def _interpret_response(
        self, result: requests.Response, stream: bool
    ) -> Tuple[Union[brainaiResponse, Iterator[brainaiResponse]], bool]:
    ...
    ) -> Tuple[Union[brainaiResponse, AsyncGenerator[brainaiResponse, None]], bool]:
        """Returns the response(s) and a bool indicating whether it is a stream."""
        if stream and "text/event-stream" in result.headers.get("Content-Type", ""):
            return (
    ...
    ) -> Union[brainaiResponse, error.APIError]:
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = brainai.aiosession.get()
    if user_set_session:
