...
import brainai
from brainai import version
from brainai.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    if args.api_key is not None:
        brainai.api_key = args.api_key
    if args.api_base is not None:
        brainai.api_base = args.api_base
    if args.organization is not None:
        brainai.organization = args.organization
    if args.proxy is not None:
        brainai.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                brainai.proxy['https'] = proxy
            elif proxy.startswith('http'):
                brainai.proxy['http'] = proxy

    try:
        args.func(args)
    except brainai.error.brainaiError as e:
        display_error(e)
        return 1
