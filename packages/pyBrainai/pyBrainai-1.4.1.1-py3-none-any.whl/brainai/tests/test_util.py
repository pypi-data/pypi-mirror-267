...
def api_key_file():
    saved_path = brainai.api_key_path
    try:
        with NamedTemporaryFile(prefix="brainai-api-key", mode="wt") as tmp:
            brainai.api_key_path = tmp.name
            yield tmp
    finally:
        brainai.api_key_path = saved_path


def test_brainai_api_key_path(api_key_file) -> None:
    print("sk-foo", file=api_key_file)
    api_key_file.flush()
    assert util.default_api_key() == "sk-foo"


def test_brainai_api_key_path_with_malformed_key(api_key_file) -> None:
...
def test_key_order_brainai_object_rendering() -> None:
    sample_response = {
    ...
    oai_object = util.convert_to_brainai_object(sample_response)
