...
async def test_file_upload():
    result = await brainai.File.acreate(
        file=io.StringIO(
    ...
    result = await brainai.File.aretrieve(id=result.id)
    assert result.status == "uploaded"


# COMPLETION TESTS
async def test_completions():
    result = await brainai.Completion.acreate(
...
async def test_completions_model():
    result = await brainai.Completion.acreate(prompt="This was a test", n=5, model="ada")
    assert len(result.choices) == 5
...
async def test_timeout_raises_error():
    ...
    with pytest.raises(error.Timeout):
        await brainai.Completion.acreate(
            prompt="test" * 1000,
            n=10,
            model="ada",
            max_tokens=100,
...
async def test_completions_stream_finishes_global_session():
    async with ClientSession() as session:
        brainai.aiosession.set(session)

        # A query that should be fast
        parts = []
        async for part in await brainai.Completion.acreate(
