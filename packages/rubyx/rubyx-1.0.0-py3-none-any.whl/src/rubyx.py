...
Usage:
  rubyx ( new | use | disable | enable | rm ) <program> [ -t key:value ]...
  rubyx programs [ --show-disabled --show-empty-scope ]
  rubyx programs where <tag_name> is [ before | after ] <value> [ --show-disabled --show-empty-scope ]
  rubyx program ( active | update ( <program>... | - ) -t key:value... [--append-tags])
  rubyx domains [ --resolved [ --no-private ] | --unresolved | --view <view> ] [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx domains where <tag_name> is [ before | after ] <value> [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx domains where <tag_name> is [ before | after ] <value> ( and <tag_name> is [ before | after ] <value> )... [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx domain ( add | remove | update ) ( - | <domain>... ) [ -p <program> -s <source> --show-new ( -t key:value... [--append-tags] ) --ignore-scope ]
  ...
  rubyx ips where <tag_name> is [ before | after ] <value> ( and <tag_name> is [ before | after ] <value> )... [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx ip ( add | remove | update ) ( - | <ip>... ) [ -p <program> -s <source> --show-new ( -t key:value... [--append-tags] ) --ignore-scope ]
  rubyx scope ( in | out ) [ (--wildcard [--top] ) ] [ ( -p <program> ) | ( --all [--show-disabled] ) ]
  rubyx scope filter ( in | out ) [ (--wildcard [--top] ) ] [ ( -p <program> ) | ( --all [--show-disabled] ) ]
  rubyx ( inscope | outscope ) ( add | remove ) ( - | <element>... ) [ -p <program> ]
  rubyx urls [ -d <hostname> | ( -p <program> | ( --all [--show-disabled] ) ) ] [ --with-query | --root ]
  rubyx urls where <tag_name> is [ before | after ] <value> [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx urls where <tag_name> is [ before | after ] <value> ( and <tag_name> is [ before | after ] <value> )... [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx url add ( - | <url>... ) [ -d <hostname> -s <source> -p <program> --show-new ( -t key:value... [--append-tags] ) --ignore-scope ]
  ...
  rubyx services where <tag_name> is [ before | after ] <value> ( and <tag_name> is [ before | after ] <value> )... [ -p <program> | ( --all [--show-disabled] ) ]
  rubyx service add ( - | <service>... ) [ -s <source> -p <program> --show-new ( -t key:value... [ --append-tags ] ) --ignore-scope ]
  ...
  rubyx show ( - | <document>... )
  rubyx remove ( - | <document>... ) [ --yes ]
  rubyx listen
  rubyx alert ( - | <message>... ) [ -s <source> --show-new -t key:value... ] 
  rubyx tags [<name>] [ -p <program> | --all ] 
  rubyx server upgrade [--yes]
...
import json
import re
from . import rubyx_api
from urllib.parse import urlparse
from docopt import docopt

CONFIG_FILE = '~/.rubyx/config.json'
...
class RubyxClient:
    ...
    '''
    The RUBYX client is responsible for ensuring the added domain is not explicitly outscoped,
...
#                        blacklisted_domain = True
    ...
    '''
    The RUBYX client is responsible for ensuring the added urls are added to the appropriate hostname (domain or IP),
    ...
    def add_urls(self, urls):
        ...
        for url in urls:
            ...
            # rubyx url add '/relative' without a -d hostname set;
    ...
    def remove_urls(self, urls):
        ...
        # to support `rubyx urls -d example.com | rubyx url remove -`
    ...
    def add_services(self, services):
        ...
        # RUBYX doesn't currently support IPs or CIDR as scope, so
    ...
    def search_tags(self, doctype):
        ...
        # it will always assume 'before' in e.g. rubyx programs where x is before 1 and y is after 2
        # would be interpreted as rubyx programs where x is before 1 and y is BEFORE 2
    ...
    def matches_scope(domain, scope):
        for s in scope:
            ...
            if s.startswith('*.') and domain.endswith('.'+s[2:]):
                return True
            # CIDR match
            if REGEX_IP.match(domain) and REGEX_IP.match(s) and RubyxClient.ip_in_cidr(domain, s):
                return True
        return False    
...
def main():
    try:
        ...
        result = RubyxClient(arguments).run()
        if result:
