...
class RUBYXApi:
    ...
    def __init__(self, couchdb_url, user, pwd, discord_webhook = None, ignore_ssl_errors = False, debug = False):
        auth = user+':'+pwd
    ...
    def create_new_program(self, program_name=None, disabled=False, passive_only=False, tags=[]):
        if not program_name:
            ...
            r = self.requests_session.put(self.RUBYX_API+'/'+requests.utils.quote(program_name, safe=''), json.dumps(program), headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def get_domains_by_program_name(self, program_name=None, show_disabled=False):
        if program_name:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/domains?reduce=false&key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
            return [r['value'] for r in r.json()['rows']]
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/domains?reduce=false', headers={"Authorization": self.auth})
    ...
    def get_urls_by_hostname(self, hostname=None, with_query=False, root_only=False):
        if hostname:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/urls_by_hostname?reduce=false&key="'+hostname+'"', headers={"Authorization": self.auth})
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/urls_by_hostname?reduce=false', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def get_urls_by_program(self, program=None, with_query=False, root_only=False, show_disabled=False):
        if program:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/urls_by_program?reduce=false&key="'+requests.utils.quote(program, safe='')+'"', headers={"Authorization": self.auth})
            return self.process_urls(r.json()['rows'], with_query, root_only)
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/urls_by_program?reduce=false', headers={"Authorization": self.auth})
            if show_disabled:
                # no filter, so return everything
    ...
    def get_services_by_program_name(self, program_name=None, show_disabled=False):
        if program_name:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/services?reduce=false&key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
            return [r['value'] for r in r.json()['rows']]
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/services?reduce=false', headers={"Authorization": self.auth})
            if show_disabled:
                # no filter, so return everything
                return [r['value'] for r in r.json()['rows']]
            else:
    ...
    def get_documents_of_type(self, doctype, program_name = None):
        ...
        if program_name:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/'+doctype+'s?reduce=false&key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/'+doctype+'s?reduce=false', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
        return [r['value'] for r in r.json()['rows']]
    ...
    def get_documents_view(self, program_name, doctype, view, show_disabled=False):
        if doctype not in self.doctypes:
            raise Exception('This doctype is not supported')
        if program_name:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/'+doctype+'s_'+view+'?reduce=false&key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
            return [r['value'] for r in r.json()['rows']]
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/'+doctype+'s_'+view+'?reduce=false', headers={"Authorization": self.auth})
            if show_disabled:
                # no filter, so return everything
                return [r['value'] for r in r.json()['rows']]
            else:
    ...
    def get_ips_by_program_name(self, program_name=None, filter_cdn=False, show_disabled=False):
        ...
        if program_name:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/ips'+cdn_filter+'?reduce=false&key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
            return [r['value'] for r in r.json()['rows']]
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/ips'+cdn_filter+'?reduce=false', headers={"Authorization": self.auth})
    ...
    def get_programs(self, show_disabled=False, show_empty_scope=False):
        if show_disabled:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/programs?reduce=false', headers={"Authorization": self.auth})
        else:
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/programs?reduce=false&key=false', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
        ...
        try:
            ...
            print('[WARNING] Your RUBYX server views are deprecated, it is strongly recommended to upgrade. Run `rubyx server upgrade` to upgrade.')
            return [r['value'] for r in r.json()['rows']]
    ...
    def get_program_scope(self, program_name):
        ...
        r = self.requests_session.get(self.RUBYX_API+'/'+requests.utils.quote(program_name, safe=''), headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def update_program_scope(self, program_name, inscope, outscope, program=None):
        ...
        if program['inscope'] != inscope or program['outscope'] != outscope:
            ...
            r = self.requests_session.put(self.RUBYX_API+'/'+requests.utils.quote(program_name, safe='')+'?rev='+program['_rev'], json.dumps(program), headers={"Authorization": self.auth})
            if 'error' in r.json():
                raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def get_all_program_documents(self, program_name):
        r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/program_all_documents?key="'+requests.utils.quote(program_name, safe='')+'"', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def get_agents(self):
        ...
        r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/agents', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
        return r.json()['rows']
    ...
    def add_documents(self, doctype, identifiers, program_name, source=None, tags=[]):
        ...
        if program_name == '@INFER':
            # When related identifiers are supplied, e.g. `rubyx ip add 1.1.1.1:example.com`
            ...
            if relname:
                ...
                r = self.requests_session.post(self.RUBYX_API+'/_all_docs?include_docs=true', json.dumps({'keys': [x for docid in identifiers.keys() for x in identifiers[docid]]}), headers={"Authorization": self.auth, 'Content-Type': 'application/json'})
            ...
            r = self.requests_session.post(self.RUBYX_API+'/_all_docs?include_docs=true', json.dumps({'keys': [x for x in self.get_programs()]}), headers={"Authorization": self.auth, 'Content-Type': 'application/json'})
        ...
        for docid in identifiers.keys():
            ...
            if program_name == '@INFER':
                ...
                if len(infered_program_name) > 0:
                    ...
                    if doctype == 'domain':
                        from . import RubyxClient
                        infered_program_name = [p for p in program_scopes.keys() if RubyxClient.matches_scope(docid, program_scopes[p]['in']) and not RubyxClient.matches_scope(docid, program_scopes[p]['out'])]
                        if len(infered_program_name) > 0:
                            infered_program_name = infered_program_name[0]
                    elif doctype == 'url':
                        from . import RubyxClient
                        infered_program_name = [p for p in program_scopes.keys() if RubyxClient.matches_scope(identifiers[docid]['hostname'], program_scopes[p]['in']) and not RubyxClient.matches_scope(identifiers[docid]['hostname'], program_scopes[p]['out'])]
                        if len(infered_program_name) > 0:
                            infered_program_name = infered_program_name[0]
        ...
        r = self.requests_session.post(
            self.RUBYX_API+'/_bulk_docs', json.dumps(bulk),
        ...
        if 'error' in r.json():
            raise Exception('Unexpected RUBYX response: '+r.json()['error'])
    ...
    def remove_document(self, doctype, document):
        ...
        r = self.requests_session.get(self.RUBYX_API+'/'+requests.utils.quote(document, safe=''), headers={"Authorization": self.auth})
        ...
        if '_rev' in r.json():
            r = self.requests_session.delete(self.RUBYX_API+'/'+requests.utils.quote(document, safe='')+'?rev='+r.json()['_rev'], headers={"Authorization": self.auth})
            if 'error' in r.json() and r.json()['error'] != 'not_found':
                raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def remove_documents(self, docids):
        ...
        r = self.requests_session.post(self.RUBYX_API+'/_all_docs?include_docs=true', json.dumps({'keys': [x for x in docids]}), headers={"Authorization": self.auth, 'Content-Type': 'application/json'})
    ...
    def get_document(self, docid):
        ...
        if docid.startswith('_'):
            docid = '.'+docid
        r = self.requests_session.get(self.RUBYX_API+'/'+requests.utils.quote(docid, safe=''), headers={"Authorization": self.auth})
        if 'error' in r.json() and r.json()['error'] == 'not_found':
            return None
        return r.text
    ...
    def get_documents(self, docids):
        ...
        r = self.requests_session.post(self.RUBYX_API+'/_all_docs?include_docs=true', json.dumps({'keys': [x if not x.startswith('_.') else '.'+x for x in docids]}), headers={"Authorization": self.auth, 'Content-Type': 'application/json'})
        ...
        if '_rev' in r.json():
            ...
            if document_changed:
                r = self.requests_session.put(
                    self.RUBYX_API+'/'+requests.utils.quote(document, safe='')+'?rev='+r.json()['_rev'],
            ...
            r = self.requests_session.put(
                self.RUBYX_API+'/'+requests.utils.quote(document, safe=''),
                json.dumps(updates),
                headers={"Authorization": self.auth, "Content-Type": "application/json"}
            )
    ...
    def update_documents(self, doctype, batch_updates, append_tags=False):
        ...
        # first, get the latest revisions of each of the listed documents
        r = self.requests_session.post(self.RUBYX_API+'/_all_docs?include_docs=true', json.dumps({'keys': [x for x in batch_updates.keys()]}), headers={"Authorization": self.auth, 'Content-Type': 'application/json'})
    ...
    def listen_for_changes(self):
        ...
        url = self.RUBYX_API+'/_changes?feed=continuous;include_docs=true;since='
        error = None
        seq = 'now'
        change_data = []
        last_update = 0
    ...
    '''
    Handle a batch of changes by sending a notification if it
    ...
    def handle_changes(self, changes, seq):
        ...
        new = {'program': [], 'domain': [], 'ip': [], 'service': [], 'url': []}
        update = {'program': [], 'domain': [], 'ip': [], 'service': [], 'url': []}
    ...
    '''
    Execute hooks in ~/.rubyx/hooks/{domain,ip,url,service}/{new,update}/
    '''
    def execute_hooks(self, hooktype, doctype, identifiers):
        ...
        hookdir = expanduser('~/.rubyx/hooks/'+doctype+'/'+hooktype+'/')
        try:
    ...
    def run_agent(self, agent_name, program_name):
        r = self.requests_session.get(self.RUBYX_API+'/agents_api_gateway', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
        gateway = r.json()['url']
    ...
    def set_agent_gateway(self, url):
        ...
        if gateway:
            ...
            self.requests_session.put(self.RUBYX_API+'/agents_api_gateway', json.dumps({'doctype': 'config', 'url': url}), headers={"Authorization": self.auth})
    ...
    def create_new_agent(self, agent_name):
        if not agent_name:
            ...
            r = self.requests_session.put(self.RUBYX_API+'/agent_'+agent_name, json.dumps(agent), headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def create_alert(self, alert_message, program_name, source_name):
        ...
        r = self.requests_session.post(self.RUBYX_API, json.dumps(alert), headers={"Content-Type": "application/json", "Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
            
    '''
    List scopes across programs using _view/scope - does not support filtering by program.
    To get scope for one program, use `rubyx program scope`.
    '''
    def get_scopes(self, show_disabled=False):
        r = self.requests_session.get(
            self.RUBYX_API+'/_design/rubyx/_view/scope',
    ...
    def search_tags(self, key, value, doctype = None, program = None, show_disabled=False , show_empty_scope=False):
        if doctype and doctype not in self.doctypes:
            raise Exception('This doctype is not supported')
        r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/search_tags?key=["'+requests.utils.quote(key, safe='')+'", "'+requests.utils.quote(value, safe='')+'"]', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def search_tags_between(self, key, value, before_after, doctype = None, program = None, show_disabled=False, show_empty_scope=False):
        ...
        if before_after == 'before':
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/search_tags?startkey=["'+requests.utils.quote(key, safe='')+'"]&endkey=["'+requests.utils.quote(key, safe='')+'","'+requests.utils.quote(value, safe='')+'"]', headers={"Authorization": self.auth})
        elif before_after == 'after':
            r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/search_tags?startkey=["'+requests.utils.quote(key, safe='')+'","'+requests.utils.quote(value, safe='')+'"]&endkey=["'+key+'ZZZZZ"]&', headers={"Authorization": self.auth})
        if 'error' in r.json():
            raise Exception('RUBYX server error: '+r.json()['error'])
    ...
    def get_tags(self, tagname, program_name=None):
        ...
        r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx/_view/tags?'+filter_name, headers={"Authorization": self.auth})
        results = r.json()['rows']
    ...
    def server_upgrade(self, admin, password):
        '''
        Upgrade server to the latest views and ensure access rights are correct (see https://github.com/honoki/rubyx-server/pull/2)
        '''
        print('Downloading latest views from https://github.com/honoki/rubyx-server')
        r = self.requests_session.get('https://raw.githubusercontent.com/honoki/rubyx-server/main/couchdb/views.json')
        views = r.json()
        print('Comparing to current views...')
        admin_auth = 'Basic '+base64.b64encode((admin+':'+password).encode('utf-8')).decode('utf-8')
        r = self.requests_session.get(self.RUBYX_API+'/_design/rubyx', headers={"Authorization": admin_auth})
        ...
        if not self.docs_are_equal(r.json(), views):
            ...
            views['_rev'] = rev
            r = self.requests_session.put(self.RUBYX_API+'/_design/rubyx', json.dumps(views), headers={"Authorization": admin_auth, 'Content-Type': 'application/json'})
            if r.status_code == 201:
                print('Server upgrade complete, please allow a few minutes for all reindexing to complete. View the progress on '+self.RUBYX_API.replace('/rubyx','')+'/_utils/#/activetasks')
            else:
        ...
        r = self.requests_session.get(self.RUBYX_API+'/access-test') 
        if r.status_code == 401:
            print('No issues detected!')
        else:
            print('CRITICAL! Unauthorized access to the database is enabled. Fixing configuration...')
            r = self.requests_session.put(self.RUBYX_API+'/_security', '{"admins":{"names":[]},"members":{"names":["rubyx"]}}', headers={"Authorization": admin_auth, 'Content-Type': 'application/json'})
            if r.status_code == 200:
                ...
                print('curl -ik -X PUT "'+self.RUBYX_API+'/_security" -u admin:password -d \'\'')
