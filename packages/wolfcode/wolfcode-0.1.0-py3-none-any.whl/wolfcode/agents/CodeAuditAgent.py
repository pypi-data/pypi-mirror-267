...
from wolfcode.output_schemas import AuditReport
from wolfcode.config import load_config
...
class CodeAuditAgent(object):
    ...
    DEFAULT_CONFIG_PATH = "wolfcode/settings/code_audit_config.toml"
    ...
    def __init__(self, files: List[Path], config_path: Optional[Path]) -> None:
        ...
        if config_path:
            ...
            self.settings = load_default(CodeAuditAgent.DEFAULT_CONFIG_PATH)
    ...
    def run(self):
        ...
        self.displayAuditReport(result)


    def displayAuditReport(self, report):
        console = Console()
        ...
        for item in report.bugs:
            ...
            console.print(f"  in [yellow] {item.suggestion.relevant_file}[/]", justify="left")
        ...
        summary = Panel.fit(Text(f"{report.summary}", style="yellow"), title="Summary", title_align="left", style="bold", border_style="green")
