# Dependancies
from main import copy, raeScore, dump, load_model, save_model, r2d2, TTSplit, normalize, denormalize, cross_val
from activations import *
# from _experimental import MCSoupClassifier, MCSoupRegressor
plt = None
...
class MCNeuralNetwork:
    ## Default Functions ## 
    def __init__(self, hidden_counts:list = [100], activations: any = ['relu'],
                 input_activation='lin', output_activation='lin'):
        """
        ...
        - `activations`
            - List or Str of the activation function(s) to use for the hidden layers
            - LIN:   Identity (x)
            - RELU:  max(x, 0)
            - lReLU: max(x, -0.3)
            - SILU:  x*sigmoid(x)
            ...
            - ROOT:  An unbounded atan function (no horizontal asymptotes)
            - SQR:   x = 0 for x <= 0, x = 1 for x > 0
            - RND:   round(x)

        - `input_activation`
            - Activation used for the input of the model (before any matrix calculations)
            - 'lin' (identity) is generally best, but sometimes other functions are better

        - `output_activation`
            - Activation used on the output of the model (after all matrix calculations)
            - Useful for applying things like 'SIG' for a [0, 1] output
        ...
        model = mc.MCNeuralNetwork(hidden_counts=[10], activations='relu')
        print(model)
        ...
        >>> 4. Activation Functions: ['LIN', 'RELU', 'LIN']
        ...
        if type(activations) == str:
            ...
            #     self.activationFunction = ['lin'] + ['relu']*(len(self.sizes)-2) + ['lin']
            # else:
            self.activationFunction = ['lin'] + [activations]*(len(self.sizes)-2) + ['lin']
        ...
        # Activation list if given a list of functions
            ...
            LW = len(self.sizes)-2
            if LA == LW:
                self.activationFunction = [input_activation] + activations + [output_activation]
            elif LA < LW:
        ...
        # If given a single external activation function
            ...
            self.activationFunction = [activations]*(len(self.sizes)-2)
        else:
    ...
    def Calculate(self, inVector, **kwargs):
        """
        ...
        # First Layer
        inVector = _applyActi(inVector, self.activationFunction[0])

        # All other layers (passing through weights)
        for i in range(1, len(self.sizes)):
            if self.use_biases:
                inVector = _applyActi(np.dot(inVector, self.weights[i-1]) + self.biases[i-1], self.activationFunction[i])
            else:
                inVector = _applyActi(np.dot(inVector, self.weights[i-1]), self.activationFunction[i])
    ...
    def setup_model(self, xArr=None, yArr=None, ignore_xy=False, remake_arrays=False):
        """
        Sets the initial input/ouput sizes, weight arrays, and bias arrays for the model

        If ignore_xy is True, this will attempt to use the current size parameters of the
        model to rebuild the weights and biases. This is useful for updating the model
        after changing something like the hidden layers' sizes.
        """
        # Initial setup
        if not ignore_xy:
            if self.inSize == -1:
            ...
            if self.outSize == -1:
                ...
                if len(yArr.shape) > 1:
                    ...
                    # Warn for number of Y features above 1 (or other amount)
                    # if self.outSize != 1:
        ...
        # Verify .sizes is correct
        self.sizes = [self.inSize] + self.hiddenSize + [self.outSize]

        # Setup Biases
        if self.biases == [] or remake_arrays:
            self.biases = []
            for si in self.sizes[1:]:
                self.biases.append(np.zeros(si))

        # Setup weights
        if self.weights == None or remake_arrays:
            self.weights = []
            ...
            self.parameters = sum([M.size for M in self.weights])
            if self.use_biases:
                self.parameters += sum([b.size for b in self.biases])
    ...
    def fit(self, xArr:np.ndarray, yArr:np.ndarray, Ieta:int = 9, Beta:int = 50, Gamma:int = 3, 
        ...
        # Verify data types
        if type(xArr) != np.ndarray or type(yArr) != np.ndarray:
            raise ValueError(f"An input data type is not correct | Expected two np.ndarrays, Got X as {type(xArr)}, Y as {type(yArr)}")
        ...
        if verbose:
            ...
            pix1Len = 0
            pix2Len = barLength
            print(f"{scoreType.upper() if ScoreFunction == None else ''} Score: {currentScore:.6f} | Training: {pix2*pix2Len}", end='\r')
        ...
        for I in range(Ieta):
            ...
            if len(xArr)*Beta >= 1e5 and useMP: # Multiprocessing method
                if I == 0:
                    print("Automatically using multiprocessing during this .fit call        ", end='\r')
                newNetScores = _multiNetScore(testNets, xArr=xArr, yTrue=yArr, scoreType=scoreType, useFast=useFast)
    ...
    def predict(self, xArr, useFast:bool = True):
        ...
        predictions = [self.Calculate(xArr[i], useFast=useFast) for i in range(dataSize)]
        ...
        predictions.shape = predictions.shape[:2]
        return predictions
    ...
    def load(path_or_name:str):
        return load_model(name=path_or_name)
...
# Autoencoder Based off of MCNeuralNetwork
class MCAutoEncoder(MCNeuralNetwork):
    def __init__(self, num_steps:int=2, activations="relu"):
        """
        Autoencoder model using MCNeuralNetwork as a base

        ### Inputs
        - num_steps
            - The number of times the hidden layers try to halve during the
              encoder creation. The total number of hidden layers is equal
              to (2*num_steps - 1)
        - activations
            - Same as MCNeuralNetwork
            - A single string will apply that activation to all layers
            - A list should contain N activations where N is the number
              of TOTAL layers in the model (input & output included)
        """
        
        # Inherit
        self.layers_set = False
        super().__init__([1]*(2*num_steps-1), activations=activations)

        # Attributes
        self.fitted = False

    def setup_model(self, xArr=None, yArr=None, ignore_xy=False):
        """
        Sets the initial input/ouput sizes, weight arrays, and bias arrays for the model
        ...
        if not self.layers_set:
            # Reset the model hidden layer size data
            ## Makes the hidden layers that squeeze then expand again
            layers_from_left = [max(round(self.inSize/(2**(i+1))), 1) for i in range(len(self.sizes)-2)]
            layers_from_right = [max(round(self.outSize/(2**((len(self.sizes)-2) - (i)))), 1) for i in range(len(self.sizes)-2)]
            decided_hidden_layers = [max(li, ri) for li, ri in zip(layers_from_left, layers_from_right)]

            self.sizes[-len(self.sizes)+1:-1] = decided_hidden_layers
            self.hiddenSize = decided_hidden_layers
...
# Proof of concept ensemble based off of MCNN
class MCSimpleEnsemble:
    def __init__(self, n_estimators = 10, hidden_counts = [100,], activations = "relu"):
        """
    ...
    def fit(self, X, Y, depth=1, verbose=True, scoreType='r2', **kwargs):
        self.models, self.weights = cross_val(self.base_model, X, Y, cv=self.n_estimators, 
...
# Multi-AF-Layer SUNN Model off of MCNN
    ...
    def fit(self, xArr:np.ndarray, yArr:np.ndarray, Ieta:int = 9, Beta:int = 50, 
        ...
        for I in range(Ieta):
            ...
            if len(xArr)*Beta >= 110000 and useMP: # Multiprocessing method
                newNetScores = _multiNetScore(testNets, xArr=xArr, yTrue=yArr, scoreType=scoreType, useFast=True)
    ...
    def calculate_su(self, xi):
        ...
        for i in range(len(self.sizes)):
            ...
            for key, AFtype in library:
                ...
                xi[afInds] = _applyActi(xi[afInds], AFtype)
...
class MCLinearRegression(MCNeuralNetwork):
    """
    Simple linear regressor
    """
    def __init__(self):
        # Set base model
        super().__init__(hidden_counts=[], activations=['lin', 'lin'])

        # Remove weight limits
        self.weight_bounds = (-np.inf, np.inf)
        self.bias_bounds = (self.weight_bounds)

# Simple Linear Classifier
## Works best with normalized data
class MCLinearClassifier(MCNeuralNetwork):
    """
    Simple (binary) linear classifier
    """
    def __init__(self):
        # Set base model
        super().__init__(hidden_counts=[], activations=['lin', 'sig'])
...
# Function to extend the height of an MCNeuralNetwork's hidden layers
...
# Great optimization form that made an optim. MCNN for a given data set
# Update this to use the new NeuralNetwork
def generate_model(X, Y, hidden_layers = [10, 10],
             test_activations=['lin', 'relu', 'sig', 'tanh', 'silu', 'dsilu', 'elu', 'root'],
             initial_activations='lin', train_split_percent=10, greedy=True, 
             target_score=0.99, iterations=4, Ieta=10, Beta=10, Gamma=2, 
             skip_current=True, verbose=True, _use_biases=True):
        ...
        model = MCNeuralNetwork(hidden_counts=hidden_layers, activations=af_start[1:-1], 
        ...
        for I in range(iterations):
            ...
            for li in range(num_layers):
                ...
                for afi, AF in enumerate(test_activations):
                    ...
                    if type(AF) != str:
                        raise ValueError(f"A test activation is not of type str! (Note external function types not supported here yet)")
...
# 'Method' to get an apply an activation function given its string name
def _applyActi(calcVec:np.ndarray, activationFunction):
...
def _multiNetScore(netList, xArr, yTrue, scoreType='r2', useFast=True, processes=10):
    # Get/return the model's scores
    pool = Pool(processes=processes)
    return pool.starmap(func=_score, iterable=[(net, xArr, yTrue, scoreType, useFast) for net in netList])

# MP helper for the main MP function above
def _score(net, xArr, yTrue, scoreType, useFast=True):
    # Alt. predict form for MP variable handling
    return net.score(xArr=xArr, yTrue=yTrue, useFast=useFast, method=scoreType)

# Helper to quickly confirm working nature of the models and primary functions
def _run_module_tests(print_sub_steps=True) -> None:
    """
    ...
    if len(X.shape) > 1:
        M = np.round(np.mean(X[:, 0]), 14)
        S = np.round(np.std(X[:, 0]), 14)

        if M != 0:
            print(f"Normalization mean is off! (Expected 0, got {M}")
            errors += 1
        if S != 1:
            print(f"Normalization mean is off! (Expected 0, got {S}")
            errors += 1

    # Verify denormalize behavior
    x_denorm = denormalize(X, xms)
    y_denorm = denormalize(Y, yms)

    x_diff = np.sum(x_copy - x_denorm)
    y_diff = np.sum(y_copy - y_denorm)

    if x_diff/x_copy.size >= 1e-12:
        print("X denormalization error is large!")
        errors += 1
    ...
    print("Cross Validation Scores:", cross_val(model, X, Y, cv=3, train_depth=1, verbose=False))
    if print_sub_steps:
    ...
    model = MCSimpleEnsemble(n_estimators=4, hidden_counts=[10, 10], activations=['relu', 'silu'])
    if print_sub_steps:
        print("Step 3 Part 1/4 Done (make MCSimpleEnsemble)")

    model.fit(xt, yt, Ieta=8, Beta=15, Gamma=3, verbose=False, scoreType='mae')
    if print_sub_steps:
    ...
    # Step 7 (Save/Load)
    model = generate_model(X, Y, hidden_layers=[10, 10],
                           test_activations=['lin', 'silu', 'dsilu'])
    model_score = model.score(xv, yv)

    save_model(model, "test_model")
    if print_sub_steps:
        print("Step 7 Part 1/4 Done (save_model on MCNeuralNetwork)")

    loaded_model = load_model("test_model")
    if print_sub_steps:
        print("Step 7 Part 2/4 Done (load_model on MCNeuralNetwork)")

    if abs(model_score-loaded_model.score(xv, yv)) < 1e-10:
        if print_sub_steps:
            print("Step 7 Part 3/4 Done (score check on loaded MCNeuralNetwork)")
    else:
    ...
    if print_sub_steps:
        print("Step 7 Part 4/4 Done (removed test save model 'test_model')")
    print("Step 7 Done - save_model and load_model verified\n")

    # Final precision readout
    t2 = pc()
    print("Final Info:")
    print(f"Time Taken: {round(t2-t1, 3)}s")
    print(f"Number of precision errors: {errors}")
    if errors == 0:
        print("(Yiippeee! No precision errors)")



# ==================== Initial Era ==================== #

## Old Functions - Founding Generation ##
def debug_calculate(self, inVector):
    ...
    if inVector.size == np.size(inVector, axis=0) and passedRowTest:
        ...
        if inVector.size == self.inSize:
            ...
            calcVec = _applyActi(calcVec, self.activationFunction[0])
            ...
            for i, wi in enumerate(self.weights):
                ...
                calcVec = _applyActi(calcVec, hiddenFunction)
...
def fastCalc(net:MCNeuralNetwork, inVector:np.ndarray):
    ...
    **** Replaced by improved default calculation method ****
    ...
    inVector = _applyActi(inVector, net.activationFunction[0])
    for i in range(1, len(net.sizes)):
        inVector = _applyActi(np.dot(inVector, net.weights[i-1]), net.activationFunction[i])
...
def oldFastCalc(net:MCNeuralNetwork, inVector:np.ndarray):
    ...
    **** Deprecated since version 1.2.2 ****
    ...
    # Possible fast calculations
    if net.sizes[0] == 1 and len(net.sizes) == 2: # ~45% of regular calc time
        inVector = _applyActi(inVector*net.weights[0], net.activationFunction[0])

    else:   # ~73% of regular calc time
        for i in range(len(net.sizes)-1):
            if i != 0:
                inVector = inVector.reshape((inVector.size, 1))
            inVector = _applyActi(sum(inVector*net.weights[i]), net.activationFunction[i])
