...
class Oso:
    ...
    def bulk_actions(
        ...
        for i, r in enumerate(resource):
            resource_typed_id = to_api_value(r)
            if resource_type is None:
                resource_type = resource_typed_id.type
            else:
                if resource_typed_id.type != resource_type:
                    raise ValueError(
                        f"bulk_actions resources must be of the same type. Expected {resource_type} got {resource_typed_id.type}"
                    )
            query.append(
                api.ActionsQuery(
                    actor_typed_id.type,
                    ...
                    resource_typed_id.type,
                    resource_typed_id.id,
                    context_facts=to_api_facts(context_facts) if i == 0 else [],
                )
            )
        result = self.api.post_bulk_actions(query)
        return list(map(lambda result: result.results, result))
