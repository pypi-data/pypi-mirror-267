...
class Launcher (webtest.Target):
    def __init__ (self, script = None, port = 5000, devel = False, ssl = False, silent = True, dryrun = False, temp_dir = None, **kargs):
        try: dryrun = kargs.pop ('dry') # lower version compat
        except KeyError: pass

        if not script and not dryrun:
            raise TypeError ('script is not given while not dryrun')
