...
class db_connection_sqlite:
	...
	def getBLOB(B,tableName,dispoDate=_A):
		D='SELECT File FROM `';E=dispoDate;F=tableName
		if E is _A:G=D+F+'` ORDER BY Dispo DESC LIMIT 1'
		else:G=D+F+"` WHERE Dispo='"+str(E)+"' ORDER BY Id DESC LIMIT 1"
		B.create_connection();C=B.connection.cursor();C.execute(G);A=C.fetchone()
		if A is not _A:
			if len(A)==1:A=A[0]
