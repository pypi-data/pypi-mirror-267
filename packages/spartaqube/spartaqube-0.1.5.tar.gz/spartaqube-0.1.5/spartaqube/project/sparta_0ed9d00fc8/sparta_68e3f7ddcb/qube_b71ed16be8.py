...
class db_connection_sql:
	...
	def setConnection(A,hostname,username,name,password='',port=3306,schemaName=_C):
		B=schemaName;C=username;A.hostname=hostname;A.user=C;A.db=name;A.password=password
		if B is _C:A.schemaName=C
		else:A.schemaName=B
	...
	def getBLOB(A,tableName,dispoDate=_C):
		C='SELECT File FROM `';D=dispoDate;E=tableName
		if D is _C:F=C+E+'` ORDER BY Dispo DESC LIMIT 1'
		else:F=C+E+"` WHERE Dispo='"+str(D)+"' ORDER BY Id DESC LIMIT 1"
		A.create_connection();B=A.connection.cursor();B.execute(F);G=B.fetchone()[0];B.close();A.close_connection();return G
	...
	def getAllTablesNbRecords2(A,tableNameArr,websocket):
		F='table';G='recordsNb';H='res';I=websocket;A.create_connection()
		for B in tableNameArr:
			try:C=A.connection.cursor();J=_O+B.replace(_A,_B)+_D;C.execute(J);K=C.fetchone()[0];D={H:1,G:K,F:B};E=json.dumps(D);I.send(text_data=E)
			except Exception as L:D={H:-1,G:0,F:B};E=json.dumps(D);I.send(text_data=E)
	...
	def createTable(D,tableName,columnName,idx):
		H=' varchar(64)';I=' datetime';E='Dispo';F='Date';C=tableName;C=C.replace(_A,_B);C=C.lower();A=''
		for(K,B)in enumerate(columnName):
			if K==0:
				if B==F or B==E:A=B.replace(_A,_B)+I
				else:A=B.replace(_A,_B)+H
			elif B==F or B==E:A=A+','+B.replace(_A,_B)+I
			else:A=A+','+B.replace(_A,_B)+H
		G=''
		if idx==F or idx==E:G='Idx datetime,'
		else:G='Idx varchar(64),'
		A='CREATE TABLE '+C+' (Id INT NOT NULL AUTO_INCREMENT,'+G+A+',PRIMARY KEY (Id))'
		if D.bPrint:print(A)
		D.create_connection();J=D.connection.cursor();J.execute(A);J.close();D.close_connection()
	...
	def getData(B,tableName,flds=_C,startDate=_C,endDate=_C,orderBy=_C):
		E=orderBy;F=startDate;G=tableName;D=endDate
		try:
			A=_I+G
			if F is not _C:
				A=A+" WHERE Idx >= '"+F+_A
