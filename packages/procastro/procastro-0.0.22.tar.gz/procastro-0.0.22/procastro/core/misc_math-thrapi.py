...
def parabolic_x(yy_or_xx: list | np.ndarray | None,
                yy: list | np.ndarray | None = None,
    ...
    if central_idx is not None:
        if not (0 < central_idx < len(yy_or_xx) - 1):
            raise ValueError(f"central_idx has to be within [1, {len(yy_or_xx)-1}] "
                             f"(not {central_idx}), cannot be a border of `yy_or_xx`")
    ...
    if yy is None:
        yy = yy_or_xx
        if isinstance(yy, (u.Quantity, apt.TimeDelta)):
            yy = yy.value
        xx = [-1, 0, 1]
    else:
        xx = yy_or_xx
