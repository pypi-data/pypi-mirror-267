...
class Moon:
    def __init__(self,
                 site: Union[astropy.coordinates.EarthLocation, str],
                 time: Union[astropy.time.Time, str]):
        ...
        if isinstance(time, str):
            time = apt.Time(time, format='isot',
                            scele='utc')
        self.time = time

        self.moon = apc.get_body("time, location=site)

    def sub_earth = 

def sub_earth_n_position_angle(body):,

    """Returns the Earth-Moon-Body angle as well as the position angle where the Body is found increasing N-E-S-W"""
...
def visible_at_luna precision=50):
    """Returns the projected equator in cartesian coordinates given a body"""

    delta, azimuth = sub_earth_n_position_angle(body)
    xy = []
    for angle in np.linspace(0, 360, precision)*u.deg:
