...
class Polygon:
    ...
    async def dark_pools(self):

        aggs = await self.get_aggs(ticker='AAPL', multiplier=1, timespan='minute', date_from='2024-04-12', date_to='2024-04-12')
        ...
        df_dollar_cost_above_10m['Timestamp'] = pd.to_datetime(df_dollar_cost_above_10m['Timestamp']).dt.strftime('%H:%M:%S')
