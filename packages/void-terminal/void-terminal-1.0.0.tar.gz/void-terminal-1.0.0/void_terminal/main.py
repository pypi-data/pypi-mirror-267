...
def main():
    ...
    with gr.Blocks(title="GPT academic optimization", theme=set_theme, analytics_enabled=False, css=advanced_css) as app_block:
        ...
        reset_server_side_args = (lambda history: ([], [], "Reset", json.dumps(history)),
                                  [history], [chatbot, history, status, history_cache])
        resetBtn.click(*reset_server_side_args)    # Clear history on the backend again，把history转存history_cache备用
        resetBtn2.click(*reset_server_side_args)   # Clear history on the backend again，把history转存history_cache备用
        ...
        app_block.load(None, inputs=[], outputs=None, _js=f"""()=>GptAcademicJavaScriptInit("{DARK_MODE}","{INIT_SYS_PROMPT}","{ADD_WAIFU}","{LAYOUT}")""")    # Configure dark theme or light theme
