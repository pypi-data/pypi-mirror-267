...
#Sequence diagram
PROMPT_2 = """
Please provide a sequence diagram around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
sequenceDiagram
...
#Class Diagram
PROMPT_3 = """
Please provide a class diagram around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
classDiagram
...
#Pie Chart
PROMPT_4 = """
Please provide a pie chart around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
...
#Gantt Chart
PROMPT_5 = """
Please provide a Gantt chart around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
gantt
...
#State Diagram
PROMPT_6 = """
Please provide a state diagram surrounding `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
stateDiagram-v2
   [*] --> Still
    Still --> [*]
...
PROMPT_7 = """
Please provide an entity relationship diagram around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
erDiagram
...
PROMPT_8 = """
Please provide a quadrant diagram around `{subject}`，Use mermaid syntax，Mermaid syntax example：
```mermaid
graph LR
...
{subject}
==========
Please provide a mind map of the content above，Fully consider the logic between them，Use mermaid syntax，Mermaid syntax example：
```mermaid
mindmap
  root((mindmap))
    Origins
...
def ParseHistoricalInput(history,llm_kwargs,file_manifest,chatbot,plugin_kwargs):
    ...
    if n_txt >= 20: print('Article is too long，Cannot achieve expected results')
    for i in range(n_txt):
        ...
        gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(i_say, i_say_show_user,  # i_say=questions actually asked to chatgpt， i_say_show_user=questions shown to the user
                                                                           llm_kwargs, chatbot,
                                                                           history=["The main content of the previous section is?", last_iteration_result], # iterate over the previous result
    ...
    gpt_say = plugin_kwargs.get("advanced_arg", "")     #Set the chart type parameter to the plugin parameter
    results_txt = '\n'.join(results)    #Merge Summary
    if gpt_say not in ['1','2','3','4','5','6','7','8','9']:    #If the plugin parameters are incorrect, use the dialogue model for judgment
        i_say_show_user = f'Next, determine the appropriate chart type,If there are 3 consecutive failures, a flowchart will be used to draw'; gpt_say = "[Local Message] Received。"   # user prompt
        ...
        for i in range(3):
            ...
            if gpt_say in ['1','2','3','4','5','6','7','8','9']:     #Determine if the return is correct
                break
...
def GenerateMultipleMermaidCharts(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if excption != "":
        ...
        if not file_exist:
            ...
            i_say_show_user = f'First, you extract an abstract from the history。'; gpt_say = "[Local Message] Received。"   # user prompt
            chatbot.append([i_say_show_user, gpt_say]); yield from update_ui(chatbot=chatbot, history=history)    # Update UI
            ...
            for i in range(file_num):     #Process files in order
                i_say_show_user = f"[{i+1}/{file_num}]Handle file{file_manifest[i]}"; gpt_say = "[Local Message] Received。"   # user prompt
                chatbot.append([i_say_show_user, gpt_say]); yield from update_ui(chatbot=chatbot, history=history)    # Update UI
