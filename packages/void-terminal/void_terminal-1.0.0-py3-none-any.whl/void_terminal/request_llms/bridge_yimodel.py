...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    if len(YIMODEL_API_KEY) == 0:
        raise RuntimeError("YIMODEL_API_KEY option is not set")
    if inputs == "":     inputs = "Empty input field"
    user_input = inputs
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    tokens = 600 if llm_kwargs['llm_model'] == 'yi-34b-chat-0205' else 4096    #yi-34b-chat-0205 has only 4k context...
    payload = {
        "model": model,
        "messages": messages,
