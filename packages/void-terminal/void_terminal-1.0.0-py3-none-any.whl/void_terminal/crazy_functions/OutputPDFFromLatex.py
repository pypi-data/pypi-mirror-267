...
def CorrectEnglishInLatexWithPDFComparison(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    if success:
        ...
        chatbot.append((f"Failed",
                        'Although PDF generation failed, But please check the results（Compressed file）, Contains a Tex document that has been translated, It is also readable, You can go to the Github Issue area, Provide feedback using the compressed package + ConversationHistoryArchive ...'))
        yield from update_ui(chatbot=chatbot, history=history);
        time.sleep(1)  # Refresh the page
        promote_file_to_downloadzone(file=zip_res, chatbot=chatbot)
...
def TranslatePDFToChineseAndRecompilePDF(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if repeat:
        ...
        try:
            ...
            report_exception(chatbot, history, b=f"Duplicate upload detected，But unable to find the relevant file")
            yield from update_ui(chatbot=chatbot, history=history)
            
            chatbot.append([f"No Relevant Files", 'Try to retranslate the PDF...'])
            yield from update_ui(chatbot=chatbot, history=history)
