...
def make_history_cache():
    ...
    # 定义history的后端state
    history = gr.State([])
    # 定义history的一items孪生的前端存储区（隐藏）
    history_cache = gr.Textbox(visible=False, elem_id="history_cache")
    # 定义history_cache->history的更新Method（隐藏）。In触发这items按钮When，会先执linejs代码更新history_cache，然后再执linepython代码更新history
