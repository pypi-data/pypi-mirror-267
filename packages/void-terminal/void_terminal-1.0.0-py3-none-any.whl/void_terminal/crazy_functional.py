...
def get_crazy_functions():
    ...
    from void_terminal.crazy_functions.GenerateMultipleMermaidCharts import GenerateMultipleMermaidCharts
    ...
    function_plugins = {
        ...
        "GenerateMultipleMermaidCharts(From the current conversation or path(.pdf/.md/.docx)Production Chart）": {
            ...
            "AdvancedArgs": True,
            "ArgsReminder": "Please enter the corresponding number for the graph type,If not entered, the model will judge on its own:1-Flowchart,Sequence Diagram,3-Class Diagram,4-Pie chart,5-Gantt chart,6-State Diagram,7-Entity relationship diagram,8-quadrant prompt diagram,Mind Map 9",
        },
        ...
        function_plugins.update(
            {
                "Inquire multiple GPT models（Manually specify which models to ask）": {
                    ...
                    "AsButton": False,
                    "AdvancedArgs": True,  # When calling，Invoke the advanced parameter input area（Default is False）
                    "ArgsReminder": "Support any number of llm interfaces，Separate with & symbol。For examplechatglm&gpt-3.5-turbo&gpt-4",  # Display prompt in the advanced parameter input area
    ...
    try:
        ...
        from void_terminal.crazy_functions.OutputPDFFromLatex import TranslatePDFToChineseAndRecompilePDF
