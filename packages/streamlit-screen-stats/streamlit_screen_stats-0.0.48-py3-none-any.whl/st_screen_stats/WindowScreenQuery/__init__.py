...
class WindowQuerySize:
    ...
    Params on init:
        - pause [int]: time to first pause the component to give it time to mount/load. Will only be implemented once when the app first loads. (uses st.session_state - parameter named 'firstRunComponent')
    ...
    def __init__(self, pause=1.5) -> None:
        """
            Pause 
        """
        self.pause = pause
        ...
        if self.pause != None:
            if "firstRunComponent" not in st.session_state:
                st.session_state["firstRunComponent"] = False
    ...
    def mediaQuery(self, mediaMatchQ:str=None, default=None, key=None):
        ...
        if "firstRunComponent" in st.session_state and not st.session_state["firstRunComponent"] and self.pause != None:
            time.sleep(self.pause)
            st.session_state["firstRunComponent"] = True
