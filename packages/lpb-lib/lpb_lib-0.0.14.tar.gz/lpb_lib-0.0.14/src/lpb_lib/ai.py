...
def corrige_mon_français(client_gpt, text, model= "gpt-3.5-turbo", max_tokens=64, seed=None, full_content = False, response_json=False):
    ...
    Args:
        ...
        :response_json: The response format True or False. default = False
    """
    system = "Je suis en train de rédiger un texte en français et j'aimerais que vous m'aidiez à vérifier l'orthographe. Le format de sortie sera 'text need correction:':True or False, and 'text_corrected': 'texte corrigé'"
    text_to_check = text
    resp = call_ai(
        client_gpt,
        model=model,
