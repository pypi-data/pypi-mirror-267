...
class Websocket:
    ...
    async def connect(self) -> None:
        if self.is_connected():
            ...
            await self.websocket.close(
                code=1006, message=b"pywave: Attempting reconnection."
            )
    ...
    async def listen(self) -> None:
        backoff = pywave.Backoff(base=1, maximum_time=60, maximum_tries=None)
        ...
        while True:
            ...
            if msg.type is aiohttp.WSMsgType.CLOSED:
                ...
                if msg.data == 1011:
                    ...
                    logger.error('Internal Lavalink Error encountered. Terminating pywave without retries.'
    ...
    async def process_data(self, data: Dict[str, Any]) -> None:
        ...
        if op == "stats":
            self.node.stats = pywave.Stats(self.node, data)
            return
    ...
    async def _get_event_payload(
        ...
        if name.startswith("Track"):
            ...
            track = await self.node.build_track(cls=pywave.Track, identifier=base64_)
    ...
    def dispatch(self, event, *args: Any, **kwargs: Any) -> None:
        self.node.bot.dispatch(f"pywave_{event}", *args, **kwargs)
