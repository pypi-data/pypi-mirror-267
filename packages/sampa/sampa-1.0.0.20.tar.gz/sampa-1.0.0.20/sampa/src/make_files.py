...
for i in range(len(files)):
    ...
    for j in range(len(task)):
        ...
        if (task[j] == 'z-scan'):
           ...
           exec(open(dir_codes + '/z-scan.py').read())
        #=============================================
        if (task[j] == 'relax'):
...
for i in range(len(files)):
    ...
    for m in range(len(task)):
        #--------------------------------------------------
        if (task[m][:6] == 'z-scan'):  k_dens = k_dens_scan
...
for i in range(len(files)):
    ...
    for j in range(len(task)):
        ...
        if (task[m][:6] == 'z-scan'):
           k_dens = 1/k_dens_relax;  type_kpoints = 1
        if (task[m][:5] == 'relax'):
           k_dens = 1/k_dens_relax;  type_kpoints = 1
        if (task[m][:5] == 'bader'):
           k_dens = 1/k_dens_bader;  type_kpoints = 1
        if (task[m][:3] == 'scf'):
           k_dens = 1/k_dens_scf;    type_kpoints = 1
        if (task[m][:3] == 'dos'):
           k_dens = 1/k_dens_dos;    type_kpoints = 1
        #--------------------------------------------
        ...
        if (task[j][:5] != 'bader'):
           ...
           #---------------------------------------------------------------
           # Inserindo o valor de ENCUT em cada arquivo INCAR -------------
           #---------------------------------------------------------------
    ...
    for j in range(len(task)):
        if (task[j] == 'z-scan'):
           ...
           if (i == 0):
              print(" ")
              print("-------------------------------------------------------------------------")
              print("Atualizando arquivos no diretório z-scan --------------------------------")
              print("-------------------------------------------------------------------------")
           #==============================
           if (porc >= t and porc <= 100):
              print(f'Progresso  {porc:>3,.0f}%')                 
           ...
           folders = list_folders(dir_task)
           for k in range(len(folders)):
               shutil.copyfile(dir_task + '/KPOINTS', dir_task + '/' + folders[k] + '/KPOINTS')
               shutil.copyfile(dir_task + '/POTCAR', dir_task + '/' + folders[k] + '/POTCAR')
               shutil.copyfile(dir_task + '/INCAR', dir_task + '/' + folders[k] + '/INCAR')
           #-------------------------------
           os.remove(dir_task + '/KPOINTS')
           os.remove(dir_task + '/POSCAR')
           os.remove(dir_task + '/POTCAR')
           os.remove(dir_task + '/INCAR')
