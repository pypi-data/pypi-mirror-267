...
@app.command()
def create(name=dir_name_venv, path='.', parent=''):
...
@app.command()
def active(name=dir_name_venv, ignore: Annotated[bool, typer.Option("--ignore/--no-ignore")] = True):
    path_venv = os.path.join(os.getcwd(), name)
    activate_venv(path_venv, ignore)
...
def sys_paths(key: Annotated[str, typer.Argument()] = ""):
    output_data_key(sys_paths_relative('', True), key)


@app.command()
def meta(path, key: Annotated[str, typer.Argument()] = ""):
    from ..utils.common import get_py_pkg_info
    output_data_key(get_py_pkg_info(path), key)
