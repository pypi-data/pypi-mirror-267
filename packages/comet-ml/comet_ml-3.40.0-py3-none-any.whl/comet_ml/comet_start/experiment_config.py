...
class ExperimentConfig:
    ...
    def __init__(
        ...
        self.disabled = config.get_bool(
            disabled, "comet.auto_log.disable", default=False, not_set_value=False
        )
...
def default_experiment_config(new_experiment: bool) -> ExperimentConfig:
    """
    The factory allowing to create default configuration options for the experiment.
    Args:
        new_experiment: (boolean) - If `True` the configuration options will be for new experiment,
            otherwise for existing.
    Returns:
        The initialized `RunConfig` object with parameter values set to defaults based on the value of the
        provided `new_experiment` parameter.
    """
    run_config = ExperimentConfig()
    for param in PARAMS_WITH_EXPERIMENT_DEFAULTS:
        attr_value = run_config.__getattribute__(param)
        if attr_value is None:
            run_config.__setattr__(param, new_experiment)
    return run_config
...
def effective_experiment_config(
    ...
    Args:
        user_config: The `RunConfig` object provided by user.
        new_experiment: If `True` the configuration options will be for new experiment, otherwise for existing.
    Returns:
        The new `RunConfig` object created from user config object by applying default values.
    """
...
def update_experiment_config_with_env_details(
    ...
    for param in ENVIRONMENT_DETAILS_PARAMS:
        ...
        elif param_value != log_env_details:
            LOGGER.warning(
