...
from .logging_messages import (
    ...
    CONNECTION_INVALID_HTTPS_PROXY_EXCEPTION,
    ...
    CONNECTION_WS_NOT_CLOSED_PROPERLY_WARNING,
...
class WebSocketConnection(threading.Thread):
    ...
    def __init__(
        self, ws_server_address: str, connection: RestServerConnection, verify_tls: bool
    ) -> None:
    ...
    def connect_ws(self) -> websocket.WebSocketApp:
        header = []
    ...
    def run(self):
        while self.closed is False:
            ...
            try:
                ...
                if not self.closed:
                    time.sleep(DEFAULT_WS_RECONNECT_INTERVAL)
            except Exception:
    ...
    def send(self, data: Any, retry: int = 5) -> None:
    ...
    def wait_for_finish(self, timeout: int = DEFAULT_WS_JOIN_TIMEOUT) -> bool:
        self.join(timeout)
        if self.is_alive():
            LOGGER.warning(CONNECTION_WS_NOT_CLOSED_PROPERLY_WARNING, timeout)
            return False
        else:
            LOGGER.debug("Websocket connection correctly closed")
            return True

    def _send(self, data: Any) -> bool:
        if self.ws.sock:
            self.ws.send(data)
    ...
    def wait_for_connection(
        self, num_of_tries=20, interval=DEFAULT_WS_RECONNECT_INTERVAL
    ):
        """
    ...
    def on_message(self, ws: Any, message: AnyStr) -> None:
...
def _debug_proxy_for_ws(target_url: str) -> Optional[str]:
...
def get_proxy_for_ws(ws_target_url: str) -> Optional[str]:
...
def get_and_format_proxy_for_ws(
    target_url: str,
) -> Tuple[Optional[str], Optional[int], Optional[Tuple[str, str]], Optional[str]]:
    ...
    if proxy_value is not None:
        ...
        if proxy_value_host is None:
            raise ValueError(CONNECTION_INVALID_HTTPS_PROXY_EXCEPTION % proxy_value)
...
def _check_response_status(
    response: requests.Response, method: str = "POST"
) -> requests.Response:
