...
from vloc.exception import VlocDetectError, VlocOcrError


if find_spec('vloc.plugin'):
    from vloc.plugin.__info__ import DetectInfo
else:
    from vloc.__detect__.__info__ import DetectInfo

VlocInfo = Annotated[DetectInfo, ...]
...
def detect_text(model: str,
                ...
                segment: bool = False,
                timeout: int = None,
                show: bool = False) -> VlocInfo | list[VlocInfo]:
    ...
    for txt in text:
        ...
        if all(item is None for item in ocr_items):
            raise VlocOcrError(f'OCR not detect text:{txt} in label:{label}')
...
def detect(model: str,
    ...
    temp_png = Path(VlocModule.history_path).joinpath('temp.png').as_posix()
    ...
    match multi_num:
        ...
        case False:
            max_det = 1
        case True:
            max_det = VlocModule.max_detect_num

    if not timeout:
        timeout = VlocModule.timeout

    for times in range(timeout):
        name = f'[{label.name}] {time_stamp} retry {times}'
        VlocModule.screenshot_method(temp_png)
        predict = VlocModule.model[model].predict(source=temp_png,
                                                  project=Path(VlocModule.history_path).as_posix(),
                                                  name=name,
                                                  classes=label.value,
                                                  max_det=max_det,
                                                  conf=VlocModule.conf_score,
                                                  save=True,
                                                  save_crop=True,
                                                  show=show)
    ...
    if not check_predict:
        if ALLURE_LOAD:
            allure.attach.file(Path(VlocModule.history_path).joinpath(f'{name}/temp.png').as_posix(), f'AI not detect {name}')
        raise VlocDetectError(f'AI not detect label:{label}')

    if ALLURE_LOAD:
        allure.attach.file(Path(VlocModule.history_path).joinpath(f'{name}/temp.png').as_posix(), name)

    crop_path = Path(VlocModule.history_path).joinpath(f'{name}/crops/{label.name}').as_posix()
...
def __parse_ai_infos(label: Enum, predict: Any, crop_list: list, sort_axis: str) -> list:
    x_scale = VlocModule.windows_size['width']/VlocModule.device_resolution['width']
    y_scale = VlocModule.windows_size['height']/VlocModule.device_resolution['height']
