...
from . import logger
from . import time
from .constants import SPEED_OF_LIGHT, EARTH_GRAVITATION_PARAM_MU
...
from .orbit.tle import TLE, read_celestrak, get_constellation_from_label
...
class Nav(object):
    ...
    def create_leo_navblock(satellite:Satellite, orbit:Kepler, sat_clock:Clock) -> RawNavBlock:
        ...
        cus = 0.0
        sqrta = math.pow(math.sqrt(EARTH_GRAVITATION_PARAM_MU) / orbit.mean_motion_rad_per_s, 1.0 / 3.0)  # n = sqrt(mu) / (sqrt(A))^3
        ...
        cis = 0.0
        lines.append(Nav._write_orbit_line(void, cic, OMEGA0, cis))
        ...
        # ORBIT - 5
        idot = 0.0
        delta_n_dot = orbit.n_dot_rad_per_s
        lines.append(Nav._write_orbit_line(idot, delta_n_dot, void, void))
    ...
    def write_from_tle(output, tle_list: List[TLE]) -> None:
        ...
        output.write(Nav.create_header(pgm='rinex_from_tle'))
...
def rinex_from_tle():
    ...
    argParser = argparse.ArgumentParser(description="Tool to convert a TLE file (from Celetrak) to RINEX file. Based on https://arxiv.org/abs/2401.17767",
                                        formatter_class=argparse.RawDescriptionHelpFormatter)  # for verbatim

    argParser.add_argument('celestrak_file', metavar='<filename>', type=str,
