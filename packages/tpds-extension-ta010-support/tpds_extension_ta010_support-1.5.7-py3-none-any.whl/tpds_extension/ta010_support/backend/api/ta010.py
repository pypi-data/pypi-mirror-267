...
class GenerateProvisioningPackage():
    def __init__(self, config_string, device_name) -> None:
        ...
        self.xml_file = f'{self.device_name}_{self.time_stamp}.xml'
    ...
    def process_xml(self):
        if self.xml_type == "prod_xml":
            ...
            self.provisioning_zip_file = f'{self.device_name}_{self.time_stamp}_prod.zip'
        else:
            log("No encryption for Proto XML")
            self.provisioning_zip_file = f'{self.device_name}_{self.time_stamp}_proto.zip'
    ...
    def __get_user_inputs(self):
        ...
        """
        user_inputs = TA010ProvisionUserInputs(self.xml_type, self.cert_type)
...
class TA010ProvisionUserInputs(TPInputDialog):
    '''
    Get inputs for TA010 Provisioning Eg. Encryption key
    '''
