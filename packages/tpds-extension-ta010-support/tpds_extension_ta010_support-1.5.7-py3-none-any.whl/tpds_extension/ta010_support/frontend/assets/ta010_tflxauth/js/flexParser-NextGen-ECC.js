...
function gererateXML(xml_type) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (keyLoadConfig[i] == "load") {
      ...
      if (certOptValue === 'custCert' && i === 1 && xml_type == "prov_proto") {
      ...
      if (certOptValue === "custCert" && xml_type === "prov_proto") {
        //device
  ...
  if (useCaseValid === false && slotDataValidity === true && isManIdValid === true) {
    ...
    if (xml_type === "proto_xml" || xml_type === "prod_xml") {
      if (certOptValue === "custCert") {
        alert("Custom certificate support is not available for Provisioning Package.")
      } else {
