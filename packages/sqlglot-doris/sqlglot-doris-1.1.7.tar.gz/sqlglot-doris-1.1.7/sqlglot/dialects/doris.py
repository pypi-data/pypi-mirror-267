...
def replace_column_type(self: Doris.Generator, column: exp.ColumnDef):
    ...
    if column.kind and column.kind.is_type(exp.DataType.Type.NULLABLE):
...
class Doris(MySQL):
    ...
    class Generator(MySQL.Generator):
        ...
        STRING_TYPE_MAPPING = {
            ...
            exp.DataType.Type.JSONB: "STRING",
        }
        ...
        def with_properties(self, properties: exp.Properties) -> str:
            return self.properties(properties, prefix=self.seg("PROPERTIES"))
        ...
        def create_mysql_or_oracle_or_clickhouse_sql(self, expression: exp.Create) -> str:
            ...
            # Remove primary_key, auto_increment, and unique information
        ...
        def create_hive_sql(self, expression: exp.Create) -> str:
            ...
            is_partition_table = expression.args.get("kind") in ("TABLE", "VIEW")
            parti_range_column = None
            disted_by_hash = None
            if has_schema and is_partition_table:
                # Look for PartitionedByProperty and ClusteredByProperty ahead of time
                ...
                # If PartitionedByProperty is found, process the corresponding logic
                if partition_by_property is not None:
                    # Access directly instead of calling the find method again
                    for col in partition_by_property.this.expressions:
                        # Get the value of kind through args and query it once in TYPE_MAPPING
                        col_kind = col.args["kind"].this
                        col_type = self.TYPE_MAPPING.get(col_kind, col_kind)
                        # If the column type is DATE or DATETIME, set parti_range_column
                        if col_type in ("DATE", "DATETIME"):
                            # The auto partition column must be NOT NULL
                ...
                # If ClusteredByProperty is found, process the corresponding logic
                if cluster_by_property is not None:
                    clust_bukets = [
                        f"`{col_bukets.name}`" for col_bukets in cluster_by_property.expressions
                    ]
            ...
            # Define the basic structure of SQL statements
            base_sql = f"{expression_sql} DUPLICATE KEY(`{first_field_name}`) "

            # DISTRIBUTED BY
            distributed_by_hash = (
                f"DISTRIBUTED BY HASH(`{first_field_name}`) BUCKETS AUTO"
                if disted_by_hash is None
                else f"DISTRIBUTED BY HASH({disted_by_hash}) BUCKETS AUTO"
            )
            ...
            if parti_range_column is not None:
                auto_partition_by_range = (
                    f"AUTO PARTITION BY RANGE date_trunc(`{parti_range_column}`, 'month') () "
                )
                dynamic_partition_properties = (
            ...
            # PROPERTIES
            common_properties = '"replication_allocation" = "tag.location.default: 3"'
        ...
        def create_postgres_sql(self, expression: exp.Create) -> str:
            ...
            parti_range_column = None
            if has_schema and is_partition_table:
                # Look for PartitionedByProperty ahead of time
                ...
                if partitioned_by_property_result:
                    ...
                    # For each partitioning column, check if there is a matching column of type 'DATE' or 'DATETIME'
                    for col in partitioned_expressions:
                        for col_test in expression_expressions:
                            col_this = col.name
                            # Confirm the type and check if the column name is within col_test.name
                            if col_this in col_test.name:
                                ...
                                if col_test_type in ("DATE", "DATETIME"):
                                    # The auto partition column must be NOT NULL
                                    constraints = col_test.args["constraints"]
                                    notnull = exp.ColumnConstraint(
                                        kind=exp.NotNullColumnConstraint()
                                    )
                                    if notnull not in constraints:
                                        constraints.insert(0, notnull)
                                    parti_range_column = col_this
                                    # After finding the column that meets the requirements, you can exit the inner loop
                                    break
            expressions = expression.this.expressions
            pg_list = []
            col_pg_list = []
            ...
            for column in col_pg_list:
                ...
                for constraint in column.constraints:
                    ...
                    if isinstance(constraint.kind, exp.UniqueColumnConstraint):
                        unique = constraint
                if primary_key:
                    if f"`{column.name}`" not in pg_list:
                        pg_list.append(f"`{column.name}`")
            ...
            for pk in expression.find_all(exp.PrimaryKey):
                for e in pk:
                    if f"`{e.name}`" not in pg_list:
                        pg_list.append(f"`{e.name}`")
            for uni in expression.find_all(exp.UniqueColumnConstraint):
                for e in uni.this:
                    if f"`{e.name}`" not in pg_list:
                        pg_list.append(f"`{e.name}`")
            ...
            # Key columns should be a ordered prefix of the schema.
            for index, key in enumerate(pg_list, start=0):
                for col in col_pg_list:
                    if key == f"`{col.name}`":
                        expressions.remove(col)
                        expressions.insert(index, col)
                        # Replace float and double with decimal, and replace text with varchar.
            ...
            # Define the attribute base string to avoid duplication in each condition
            base_properties = "PROPERTIES (" '"replication_allocation" = "tag.location.default: 3"'

            # Define the attribute string of the dynamic partition so that it can be used when needed
            dynamic_partition_properties = (
                ', "dynamic_partition.enable" = "true", '
                '"dynamic_partition.time_unit" = "month", '
                '"dynamic_partition.prefix" = "p", '
            ...
            # A string defining the distribution strategy
            def distributed_by_hash(prog: str) -> str:
                return f"DISTRIBUTED BY HASH({prog}) BUCKETS AUTO "

            # Construct SQL expressions based on conditions
            if pg_list:
                key_clause = f"UNIQUE KEY({pk_name}) "
                distributed_by = distributed_by_hash(pk_name)
                # Decide whether to add dynamic partitioning attributes based on partition columns
                if parti_range_column is None:  # UNIQUE model
                    properties_str = base_properties
                else:  # UNIQUE model partition
                    properties_str = base_properties + dynamic_partition_properties
                    range_partition_str = (
                        f"AUTO PARTITION BY RANGE date_trunc(`{parti_range_column}`, 'month') () "
                    )
                    key_clause += range_partition_str
            else:
                key_clause = f"DUPLICATE KEY(`{first_field_name}`) "
                distributed_by = distributed_by_hash(f"`{first_field_name}`")
                # Decide whether to add dynamic partitioning attributes based on partition columns
                if parti_range_column is None:  # DUPLICATE model
                    properties_str = base_properties
                else:  # DUPLICATE model partition
                    properties_str = base_properties + dynamic_partition_properties
                    range_partition_str = (
                        f"AUTO PARTITION BY RANGE date_trunc(`{parti_range_column}`, 'month') () "
                    )
            ...
            # Splicing the final SQL
            return f"{expression_sql} " f"{key_clause}" f"{distributed_by}" f"{properties_str}" f")"

        def create_presto_sql(self, expression: exp.Create) -> str:
            # Obtain the corresponding partition and bucket columns according to the situation
            has_schema = isinstance(expression.this, exp.Schema)
            is_partition_table = expression.args.get("kind") in ("TABLE", "VIEW")
            parti_range_column = None
            disted_by_hash = None
            if has_schema and is_partition_table:
                ...
                # Find the bucket column name based on the keyword 'bucketed_by'
