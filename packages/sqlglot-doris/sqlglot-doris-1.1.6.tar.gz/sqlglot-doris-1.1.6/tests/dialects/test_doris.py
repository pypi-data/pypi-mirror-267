...
class TestDoris(Validator):
    ...
    def test_oracle2doris_create(self):
        ...
        self.validate_all(
            'CREATE TABLE emp (id DECIMAL(6), name VARCHAR(75), dept_id DECIMAL(4), sal DECIMAL(8, 2)) UNIQUE KEY(`id`, `name`, `dept_id`) DISTRIBUTED BY HASH(`id`, `name`, `dept_id`) BUCKETS AUTO PROPERTIES ("replication_allocation" = "tag.location.default: 3")',
            read={
                "oracle": "CREATE TABLE emp(id NUMBER(6) PRIMARY KEY, name VARCHAR2(25) UNIQUE, sal NUMBER(8,2) CHECK(sal > 0 and sal < 1000000), dept_id number(4), constraint dept_fk foreign key(dept_id) REFERENCES dept(dept_id) on delete set null, UNIQUE(dept_id))",
            },
        )
