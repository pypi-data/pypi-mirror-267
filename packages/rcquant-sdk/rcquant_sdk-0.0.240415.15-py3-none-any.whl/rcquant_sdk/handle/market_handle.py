...
class MarketHandle():
    def __init__(self, tsocket: TSocket):
        ...
        self.__OHLC_Columns_Types = {
            ...
            "OpenPrice": float, "OpenBidPrice": float, "OpenAskPrice": float, "OpenBidVolume": int, "OpenAskVolume": int,
            "HighestPrice": float, "HighestBidPrice": float, "HighestAskPrice": float, "HighestBidVolume": int, "HighestAskVolume": int,
            "LowestPrice": float, "LowestBidPrice": float, "LowestAskPrice": float, "LowestBidVolume": int, "LowestAskVolume": int,
            "ClosePrice": float, "CloseBidPrice": float, "CloseAskPrice": float, "CloseBidVolume": int, "CloseAskVolume": int,
        ...
        for r in rsp_list:
            ...
            for df in rspparams.DataFileds:
                ...
                if marks[0] == 'zip':
                    ...
                    decombytes = zstd.decompress(df.Buffer)
                elif marks[0] == 'qtzip':
    ...
    def __create_fin_persists_save_param_data(self, instrument_id: str, range: str, df, **kwargs):
        ...
        print(kwargs.keys())
        print('compress' not in kwargs.keys())
        print(kwargs.get('compress'))
        ...
        for day, day_list in groups:
            ...
            if compress == 'zip':
                ...
                filed.Buffer = zstd.compress(pack_buffer)
            else:
