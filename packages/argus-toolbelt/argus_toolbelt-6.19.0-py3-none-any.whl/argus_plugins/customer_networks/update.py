...
def update(
    ...
    NOTICE: Any customer fields in the CSV file itself will be ignored. The customer specified in the CLI will be used
    regardless.

    :param customer: Customer shortname or ID
    :param file: Path to CSV file
    :param format: CSV format (available fields: address (alias Network), description, subnet_mask (optional, can be part of the address), zone, location (alias LocationShortName), Flags)
    ...
    _from_json = partial(CustomerNetwork.from_json, customer=customer)
    # Convert them to CustomerNetwork objects
    existing_networks = list(map(_from_json, existing_networks["data"]))
