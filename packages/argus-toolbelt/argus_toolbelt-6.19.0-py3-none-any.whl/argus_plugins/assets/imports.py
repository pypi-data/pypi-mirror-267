...
from argus_api.api.assets.v1.host import (
    add_host_asset,
    update_host_asset,
    search_host_assets,
    delete_host_asset,
)
from argus_api.api.assets.v1.service import (
...
def hosts(
    ...
    for host in assets:
        uniqueHosts[host["name"]]["ipAddresses"] = uniqueHosts[host["name"]][
            "ipAddresses"
        ].split(field_separator) + host["ipAddresses"].split(field_separator)
    ...
    for host in uniqueHosts.values():
        ...
        for ip in host["ipAddresses"]:
            if ip in existing_ip_assets:
                ...
                if host["name"] == existing_host["name"] and sorted(
                    host["ipAddresses"]
                ) == sorted([ip["address"] for ip in existing_host["ipAddresses"]]):
                    continue

                # Keep track of host iD to associate services later on
                host.update({"id": existing_host["id"]})

                if (replace_hosts and always_yes) or ask(
                    f"A host already exists with IP {ip}"
                ):
                    ...
                    if len(existing_host["ipAddresses"]) > 1:
                        try:
                            if not dry:
                                update_host_asset(
                                    existing_host["id"], deleteIpAddresses=[ip]
                                )
                            if splunk:
                                messages.append(
                                    {
                                ...
                                log.plugin(
                                    success(
                                        f"Updated {existing_host['name']} and removed IP {ip}"
                                    )
                                )
                            ...
                            if splunk:
                                ...
                                log.plugin(
                                    failure(
                                        f"Failed to remove ip {ip} from {existing_host['name']}: {error}"
                                    )
                                )
                    ...
                    # If the host only has one IP, delete the host altogether
                        ...
                        try:
                            if not dry:
                                delete_host_asset(existing_host["id"])
                            if splunk:
                                messages.append(
                                    {
                                        "timestamp": datetime.now().isoformat(),
                                ...
                                log.plugin(
                                    success(f"Removed host {existing_host['name']}")
                                )
        ...
        if host["name"] in existing_assets:
            ...
            if always_yes or ask(
                f"{host['name']} already exists, do you want to update it?"
            ):
                current_asset = existing_assets[host["name"]]
                try:
                    current_ips_with_range = [
                        f"{ip['address']}/{ip['maskBits']}"
                        for ip in current_asset["ipAddresses"]
                    ]
                    given_ips_with_range = [
                        f"{ip}/32" if "/" not in ip else ip
                        for ip in host["ipAddresses"]
                    ]
                    ...
                    host.update(
                        {
                            "addIpAddresses": diff(
                                given_ips_with_range, current_ips_with_range
                            ),
                            "deleteIpAddresses": diff(
                                current_ips_with_range, given_ips_with_range
                            ),
                        }
                    )
                        ...
                        host.update(
                            {
                                "addAliases": diff(
                                    ...
                                    [
                                        alias["fqdn"]
                                        for alias in current_asset["aliases"]
                                    ],
                                ),
                                "deleteAliases": diff(
                                    [
                        ...
                        host.update(
                            {
                                "addProperties": {
                                    ...
                                    if property
                                    in diff(
                                        host["properties"].keys(),
                                        current_asset["properties"].keys(),
                                    )
                                },
                    ...
                    # Skip updating if nothing has changed
                    if (
                        not host["addIpAddresses"]
                        and not host["deleteIpAddresses"]
                        and not host.get("addAliases")
                        and not host.get("deleteAliases")
                        and not host.get("addProperties")
                        and not host.get("deleteProperties")
            ...
            try:
                ...
                if splunk:
                    ...
                    log.plugin(
                        success(
                            f"Created {host['name']} ({', '.join(host['ipAddresses'])})"
                        )
                    )
    ...
    for service_name, host_list in services_to_attach_hosts.items():
        try:
                    ...
                    try:
                        if not dry:
                            attach_hosts_to_service(
                                id=service["id"],
                                hostAssetIDs=[
                                    host["id"] for host in host_list if "id" in host
                                ],
                            )
                            if splunk:
                                print(
                                ...
                                log.plugin(
                                    failure(
                                        f"Attached {len(host_list)} hosts to service {service_name}"
                                    )
                                )
                        ...
                        if splunk:
                            ...
                            log.plugin(
                                failure(
                                    f"Failed to attach hosts to service {service_name}: {error}"
                                )
                            )
            ...
            if splunk:
                ...
                log.plugin(
                    failure(f"Failed to look up service {service_name}: {error}")
                )
