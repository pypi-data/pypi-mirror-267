...
class Result(dict):
    ...
    def print(self,):
        ...
        if self.VIEW_TYPE == "cred":
            ...
            if self.username:c+=f"{self.T.MAGENTA}[ {self.T.YELLOW}CRED{self.T.MAGENTA} ]{self.T.MAGENTA}> {self.T.RED} Username{self.T.RESET}{' ':6}: {self.T.GREEN}{self.username}{self.T.RESET}\n"
