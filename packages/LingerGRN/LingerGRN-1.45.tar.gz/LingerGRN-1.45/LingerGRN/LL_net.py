...
## read the count file.
    ...
    TG=pd.DataFrame(adata_RNA.raw.X.toarray().T,index=adata_RNA.raw.var['gene_ids'].values,columns=adata_RNA.obs['barcode'].values)
## cell annotation
...
def TF_RE_binding(GRNdir,adata_RNA,adata_ATAC,genome,method,outdir):
    ...
    if method=='LINGER':
        ...
        for i in tqdm(range(23)):
            chr=chrom[i]
            mat=TF_RE_LINGER_chr(chr,outdir)
            result = pd.concat([result, mat], join='outer', axis=0)
    result.to_csv(outdir+'cell_population_TF_RE_binding.txt',sep='\t')


def cell_type_specific_TF_RE_binding_chr(adata_RNA,adata_ATAC,GRNdir,chrN,genome,celltype,outdir):
    ## the regions
        ## the regions
    O_overlap, N_overlap,O_overlap_u,N_overlap_u,O_overlap_hg19_u=load_region(GRNdir,genome,chrN,outdir)
    import numpy as np
...
def cell_type_specific_TF_RE_binding(GRNdir,adata_RNA,adata_ATAC,genome,celltype,outdir):
    label=adata_RNA.obs['label'].values.tolist()
    labelset=list(set(label))
    if celltype == 'all':
        for label0 in labelset:
            ...
            for i in tqdm(range(22)):
                chrN='chr'+str(i+1)
                out=cell_type_specific_TF_RE_binding_chr(adata_RNA,adata_ATAC,GRNdir,chrN,genome,celltype,outdir)
        #result=pd.concat([result,out],axis=1).fillna(0)
                result = pd.concat([result, out], join='outer', axis=0)
            chrN='chrX'
            out=cell_type_specific_TF_RE_binding_chr(adata_RNA,adata_ATAC,GRNdir,chrN,genome,celltype,outdir)
...
def cell_type_specific_trans_reg(GRNdir,adata_RNA,ATAC_file,celltype,outdir):
    import pandas as pd
    from scipy.sparse import coo_matrix
    import numpy as np
    import pandas as pd
