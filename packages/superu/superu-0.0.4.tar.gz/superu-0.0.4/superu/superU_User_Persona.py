...
class User_Persona:
    ...
    def process_openai_response(self, response):
        user_profile = {}
        user_profile['uuid'] = str(uuid.uuid4())  # Generating UUID
        user_profile['user_id'] = response["user_id"]
        user_profile['age'] = response["age"]
        user_profile['gender'] = response["gender"]["gender"]
        user_profile['confidence_score'] = response["gender"]["confidence_score"]
        user_profile['city'] = response["city"]
        user_profile['profession'] = response["profession"]
        user_profile['relationship_status'] = response["relationship_status"]
        user_profile['interests'] = response["interests"]
        return user_profile