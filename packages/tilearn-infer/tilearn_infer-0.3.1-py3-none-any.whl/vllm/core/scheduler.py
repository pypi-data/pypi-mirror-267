...
class Scheduler:
    ...
    def schedule(self) -> Tuple[List[SequenceGroupMetadata], SchedulerOutputs]:
        ...
        for seq_group in scheduler_outputs.scheduled_seq_groups:
            ...
            if seq_group.sampling_params.strict_consist:
                strict_consist_mode = True

        if seq_group_metadata_list:
            if strict_consist_mode:
                pad_seq_num = self.scheduler_config.max_num_seqs - len(seq_group_metadata_list)
                pad_seqs: List[SequenceGroupMetadata] = []
                for id in range(pad_seq_num):
                    seq_len = 1
                    ...
                    if is_prompt:
                        seq_data = {pad_id: seq_data}
                    else:
                        seq_data = {b * pad_id: seq_data for b in range(sampling_params.best_of)}
                    seq = SequenceGroupMetadata(
                        request_id=str(pad_id),
                        is_prompt=is_prompt,
                        seq_data=seq_data,
                ...
                seq_group_metadata_list += pad_seqs
                scheduler_outputs.num_batched_tokens += pad_seq_num
