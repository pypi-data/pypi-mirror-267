...
class VarianceScaling(Initializer):
  ...
  def __call__(self, shape):
    ...
    if self.distribution == "truncated_normal":
      ...
      res = bc.random.truncated_normal(-2, 2, shape, dtype=self.dtype) * stddev
    elif self.distribution == "normal":
      res = bc.random.randn(*shape, dtype=self.dtype) * jnp.sqrt(variance).astype(self.dtype)
    elif self.distribution == "uniform":
      res = (bc.random.uniform(low=-1, high=1, size=shape, dtype=self.dtype) *
