...
class Momentum(_WeightDecayOptimizer):
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    assert isinstance(train_states, dict), '"states" must be a dict of braincore.State.'

    for k, v in train_states.items():
      assert isinstance(v, bc.State), f'"{k}" must be an instance of braincore.State.'
      self.weight_states.add_unique_elem(k, v)
      self.momentum_states[k] = OptimState(bc.math.tree_zeros_like(v.value))

  def update(self, grads: dict):
    lr = self.lr()
    states_values, grad_values, momentum_values = to_same_dict_tree(self.weight_states, grads, self.momentum_states)
    momentum_values = jax.tree.map(lambda vv, gg: self.momentum * vv - lr * gg,
                                   momentum_values,
                                   grad_values)
    new_weight_values = jax.tree.map(functools.partial(_sgd, lr=lr, weight_decay=self.weight_decay),
                                     states_values,
                                     momentum_values)
...
class Adagrad(_WeightDecayOptimizer):
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      assert isinstance(v, bc.State), f'"{k}" must be an instance of braincore.State.'
      self.weight_states.add_unique_elem(k, v)
      self.cache_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
...
class Adadelta(_WeightDecayOptimizer):
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      ...
      self.weight_states.add_unique_elem(k, v)
      self.cache_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      self.delta_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
...
class Adam(_WeightDecayOptimizer):
  ...
  def __init__(
    ...
    self.eps = fcast(eps)
    self.m1_states = bc.visible_state_dict()
    self.m2_states = bc.visible_state_dict()
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      assert isinstance(v, bc.State), f'"{k}" must be an instance of braincore.State.'
      self.weight_states.add_unique_elem(k, v)
      self.m1_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      self.m2_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
...
class Adan(_WeightDecayOptimizer):
  ...
  def __init__(
    ...
    self.exp_avg_states = bc.visible_state_dict()
    self.exp_avg_sq_states = bc.visible_state_dict()
    self.exp_avg_diff_states = bc.visible_state_dict()
    self.pre_grad_states = bc.visible_state_dict()
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      assert isinstance(v, bc.State), f'"{k}" must be an instance of braincore.State.'
      self.weight_states.add_unique_elem(k, v)
      self.exp_avg_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      self.exp_avg_sq_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      self.exp_avg_diff_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      self.pre_grad_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
...
class AdamW(_WeightDecayOptimizer):
  ...
  def __init__(
    ...
    self.m2_states = bc.visible_state_dict()
    if self.amsgrad:
      self.vmax_states = bc.visible_state_dict()
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      ...
      self.m2_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
      if self.amsgrad:
        self.vmax_states[k] = OptimState(bc.math.tree_zeros_like(v.value))
...
class SM3(_WeightDecayOptimizer):
  ...
  def __init__(
    ...
    self.momentum = fcast(momentum)
    self.memory_states = bc.visible_state_dict()
  ...
  def register_trainable_weights(self, train_states: Optional[Dict[str, bc.State]] = None):
    ...
    for k, v in train_states.items():
      ...
      for i in range(ndim):
        shape = [1] * ndim
        shape[i] = rank[i]
        self.memory_states[f'{k}_m{i}'] = bc.State(jnp.zeros(shape, dtype=dtype))
      if self.momentum > 0.:
        self.memory_states[f'{k}_mbuffer'] = bc.State(jnp.zeros_like(v.value))
