...
class SecLoad:
    ...
    def CreateScript(self, script_name: str, source: str, sourcetype: ScriptType):
        if self.APIKey:
            ...
            response = requests.post(self.URL+"/secload/publicapi/CreateScript", json=data)
    ...
    def RemoveScript(self, script_name: str):
        if self.APIKey:
            response = requests.post(self.URL+"/secload/publicapi/RemoveScript", json={
                "Key": self.APIKey,
                "ScriptName": script_name,
            })
    ...
    def OverwriteScript(self, script_name: str, source: str, sourcetype: ScriptType):
        if self.APIKey:
            ...
            response = requests.post(self.URL+"/secload/publicapi/OverwriteScript", json=data)
            if response.status_code == 200:
                return response.text
            else:
                raise Exception(f"ERROR {response.status_code} - {response.text}")
    ...
    def ListScripts(self):
        if self.APIKey:
            response = requests.post(self.URL+"/secload/publicapi/ListScripts", json={
                "Key": self.APIKey,
            })
    ...
    def GenerateKey(self, script_name: str, username: str="username", time: int=5):
        if self.APIKey:
            response = requests.post(self.URL+"/secload/publicapi/GenerateKey", json={
                "Key": self.APIKey,
                "ScriptName": script_name,
                "Username": username,
                "Time": time
    ...
    def GetScriptSource(self, script_name: str, sourcetype: ScriptType):
        if self.APIKey:
            ...
            response = requests.post(self.URL+"/secload/publicapi/GetScriptSource", json=data)
            if response.status_code == 200:
                return response.text
            else:
                raise Exception(f"ERROR {response.status_code} - {response.text}")
