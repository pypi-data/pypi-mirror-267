...
class Node(ABC):
    ...
    def _step(self, input: Optional[Batch]) -> Optional[Batch]:
        try:
            ...
            self._metrics.update('processed', len(input) if isinstance(input, Batch) else len(output))
        except Exception as e:
            output = Batch(status=BatchStatus.ERROR, error=f'During processing raise exception {e}')
    ...
    def _run(self):
        while True:
            ...
            if isinstance(input, Batch) and input.is_end:
                self._put(input)
                self._status = NodeStatus.FINISH
                break
            elif isinstance(input, Batch) and input.is_error:
            ...
            if isinstance(output, Batch) and output.is_ok:
                self._put(output)
            elif isinstance(output, Batch) and output.is_end:
                self._put(output)
                self._status = NodeStatus.FINISH
                break
            elif isinstance(output, Batch) and output.is_error:
