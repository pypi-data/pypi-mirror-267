...
def __bootstrap__():
   ...
   if os_name=="linux" and machine_name == "aarch64":
      so_file = os.path.join(os.path.dirname(__file__),f"seekerfree.cpython-{py_version}-aarch64-{os_name}.so")
   else:
      so_file = os.path.join(os.path.dirname(__file__),f"seekerfree.cpython-{py_version}-{os_name}.so")
   spec = importlib.util.spec_from_file_location("seekerfree", so_file)
