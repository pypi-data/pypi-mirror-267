...
def adapt_persptrum(dataframe):
    del dataframe["topic"]
    dataframe= dataframe.rename(columns={'claim': 'topic', "pid":"id", "perspective": "text"})
    labels = {"SUPPORT": 1, "UNDERMINE":0}
    dataframe["stance"] = dataframe["stance"].apply(lambda x: labels[x])
    return dataframe
    ...
    if adapt:
        for split in dataset:
            if experiment == "ibmsc":
                ...
                dataset[split] = adapt_persptrum(dataset[split])

    return dataset


def split_perspectrum_claims_train_val(part_size):
    train_csv_path = "../experiment/perspectrum-train.csv"
    val_csv_path = "../experiment/perspectrum-validation.csv"

    train_df = pd.read_csv(train_csv_path)
    val_df = pd.read_csv(val_csv_path)

    merged_df = pd.concat([train_df, val_df], ignore_index=True)
    unique_claims = merged_df['claim'].unique()
    num_parts = math.ceil(len(unique_claims) / part_size)

    for i in range(1, num_parts):
        merged_df[f'claim-{i * part_size}'] = False

    for i in range(num_parts):
        start_idx = 0
        end_idx = (i + 1) * part_size
        if i == num_parts - 1:
            merged_df['claim-all'] = True
            break
        claims_in_part = unique_claims[start_idx:end_idx]
        for claim in claims_in_part:
            merged_df.loc[merged_df['claim'] == claim, f'claim-{(i + 1) * part_size}'] = True

    output_csv_path = "../experiment/perspectrum-claim-split-train-val.csv"
...
def save_experiment_results(experiment, df_run_results, run_name):
    ...
    df_new_results.to_csv(path_experiment_results, sep= "\t", encoding="utf-8")
