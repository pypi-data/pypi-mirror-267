...
def load_splits(experiment, oversample=True, validate=True, best_split=False, adapt=True, topic_count=None, debug=False):
    ...
    if experiment == "perspectrum":
        ...
        df_training = df_training.rename(columns={'claim': 'text'}).head(10)
        df_validation = df_validation.rename(columns={'claim': 'text'}).head(10)
        df_test = df_test.rename(columns={'claim': 'text'}).head(10)
    if debug:
