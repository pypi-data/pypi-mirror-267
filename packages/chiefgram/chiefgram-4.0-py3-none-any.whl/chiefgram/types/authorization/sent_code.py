...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~chiefgram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~chiefgram.Client.sign_in` or :meth:`~chiefgram.Client.sign_up`).

        next_type (:obj:`~chiefgram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~chiefgram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~chiefgram.Client.resend_code`.
    """
