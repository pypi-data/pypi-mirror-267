...
try:
    ...
    log.info("Using TgCyber")


    def ige256_encrypt(data: bytes, key: bytes, iv: bytes) -> bytes:
        return tgcyber.ige256_encrypt(data, key, iv)


    def ige256_decrypt(data: bytes, key: bytes, iv: bytes) -> bytes:
        return tgcyber.ige256_decrypt(data, key, iv)


    def ctr256_encrypt(data: bytes, key: bytes, iv: bytearray, state: bytearray = None) -> bytes:
        return tgcyber.ctr256_encrypt(data, key, iv, state or bytearray(1))


    def ctr256_decrypt(data: bytes, key: bytes, iv: bytearray, state: bytearray = None) -> bytes:
        return tgcyber.ctr256_decrypt(data, key, iv, state or bytearray(1))
    ...
    log.warning(
        "TgCyber is missing! "
        "chiefgram will work the same, but at a much slower speed. "
        "More info: https://docs.chiefgram.org/topics/speedups"
    )
