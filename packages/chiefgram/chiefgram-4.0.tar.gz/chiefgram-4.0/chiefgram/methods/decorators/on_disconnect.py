...
class OnDisconnect:
    def on_disconnect(self=None) -> Callable:
        ...
        :obj:`~chiefgram.handlers.DisconnectHandler`.
        """

        def decorator(func: Callable) -> Callable:
            if isinstance(self, chiefgram.Client):
                self.add_handler(chiefgram.handlers.DisconnectHandler(func))
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append((chiefgram.handlers.DisconnectHandler(func), 0))
