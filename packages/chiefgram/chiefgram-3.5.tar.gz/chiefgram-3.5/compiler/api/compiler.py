...
DESTINATION_PATH = Path("chiefgram/raw")
NOTICE_PATH = "NOTICE"
...
def get_docstring_arg_type(t: str):
    if t in CORE_TYPES:
        ...
        return "Any object from :obj:`~chiefgram.raw.types`"
    elif t == "!X":
        return "Any function from :obj:`~chiefgram.raw.functions`"
    elif t.lower().startswith("vector"):
        return "List of " + get_docstring_arg_type(t.split("<", 1)[1][:-1])
    else:
        return f":obj:`{t} <chiefgram.raw.base.{t}>`"
...
def start(format: bool = False):
    ...
    for qualtype in types_to_constructors:
        ...
        docstring += f"\n\n    Constructors:\n" \
                     ...
                     f"        .. currentmodule:: chiefgram.raw.types\n\n" \
                     f"        .. autosummary::\n" \
                     f"            :nosignatures:\n\n" \
                     f"            {items}"
        ...
        if references:
            docstring += f"\n\n    Functions:\n        This object can be returned by " \
                         ...
                         f"        .. currentmodule:: chiefgram.raw.functions\n\n" \
                         f"        .. autosummary::\n" \
                         f"            :nosignatures:\n\n" \
                         f"            " + references
    ...
    for c in combinators:
        ...
        if c.section == "types":
            ...
            docstring += f"\n    Constructor of :obj:`~chiefgram.raw.base.{c.qualtype}`."
        else:
    ...
    with open(DESTINATION_PATH / "all.py", "w", encoding="utf-8") as f:
        ...
        for c in combinators:
            f.write(f'\n    {c.id}: "chiefgram.raw.{c.section}.{c.qualname}",')
