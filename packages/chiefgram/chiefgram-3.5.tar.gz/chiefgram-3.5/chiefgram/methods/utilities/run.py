...
import chiefgram
from chiefgram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~chiefgram.Client.start`, :meth:`~chiefgram.idle` and :meth:`~chiefgram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that chiefgram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`chiefgram.compose`.
