...
class Dispatcher:
    ...
    def __init__(self, client: "chiefgram.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await chiefgram.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await chiefgram.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                chiefgram.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                chiefgram.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                chiefgram.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                chiefgram.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                chiefgram.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                chiefgram.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except chiefgram.StopPropagation:
                                raise
                            except chiefgram.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
