...
class Dispatcher:
    ...
    def __init__(self, client: "haku.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await haku.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def bot_business_message_parser(update, users, chats):
            return (
                await haku.types.Message._parse(
                    self.client,
                    update.message,
                    users,
                    chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await haku.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                haku.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                haku.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                haku.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                haku.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                haku.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                haku.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await haku.types.Story._parse(self.client, update.story, update.peer),
                StoryHandler
            )

        async def message_bot_na_reaction_parser(update, users, chats):
            return (
                haku.types.MessageReactionUpdated._parse(self.client, update, users, chats),
                MessageReactionUpdatedHandler
            )

        async def message_bot_a_reaction_parser(update, users, chats):
            return (
                haku.types.MessageReactionCountUpdated._parse(self.client, update, users, chats),
                MessageReactionCountUpdatedHandler
            )

        async def bot_business_connect_parser(update, users, chats):
            return (
                await haku.types.BotBusinessConnection._parse(self.client, update.connection),
                BotBusinessConnectHandler
            )
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except haku.StopPropagation:
                                raise
                            except haku.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
