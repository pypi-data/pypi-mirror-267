...
class Object:
    def __init__(self, client: "haku.Client" = None):
        self._client = client

    def bind(self, client: "haku.Client"):
        """Bind a Client instance to this and to all nested Haku objects.

        Parameters:
            client (:obj:`~haku.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing Haku objects with ``repr`` and ``eval``.
        """
        self._client = client
