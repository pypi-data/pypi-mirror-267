...
class SignIn:
    async def sign_in(
        ...
        Returns:
            :obj:`~haku.types.User` | :obj:`~haku.types.TermsOfService` | bool: On success, in case the
            authorization completed, the user is returned. In case the phone number needs to be registered first AND the
            terms of services accepted (with :meth:`~haku.Client.accept_terms_of_service`), an object containing
