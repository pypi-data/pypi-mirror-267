...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~haku.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~haku.Client.sign_in` or :meth:`~haku.Client.sign_up`).

        next_type (:obj:`~haku.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~haku.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~haku.Client.resend_code`.
    """
