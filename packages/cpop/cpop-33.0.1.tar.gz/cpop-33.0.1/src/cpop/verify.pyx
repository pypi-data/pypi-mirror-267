...
def contract(
    raws: Iterable["cpop.loader.LoadedMod"],
    mod: "cpop.loader.LoadedMod",
):
    """
    ...
    sig_miss = []
    for raw in raws:
        if isinstance(raw, cpop.loader.LoadError):
    ...
    if sig_errs or sig_miss:
        ...
        if sig_miss:
            ...
            for err in sig_miss:
                msg += f"{err}\n"
        msg = msg.strip()
        raise cpop.exc.ContractSigException(msg)
...
def sig(func: Callable, ver: Callable) -> list[str]:
    ...
    if errors:
        if hasattr(ver.__code__, "co_filename"):
            errors.append(
                f"Enforcing signature: {ver.__code__.co_filename}::{ver.__name__}"
            )
        else:
