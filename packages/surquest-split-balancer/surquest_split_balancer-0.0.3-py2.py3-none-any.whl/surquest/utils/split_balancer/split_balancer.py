...
class SplitBalancer:
    ...
    def _get_model(self):
        ...
        if only_integers is True:
            ...
            avg_target_char = sum(
                x[(i, groups[0])]*simple_char[idx] for idx, i in enumerate(pool)
            )*const

            avg_control_char = sum(
                x[(i, groups[1])]*simple_char[idx] for idx, i in enumerate(pool)
            )*coef
    ...
    def solve(self, limit=180, remote=False, api_key=None):
        ...
        return {"assignments": groups, "stats": avg}

    @staticmethod
    def simplify_characteristics(characteristics, do_rescale=True):
        ...
        if do_rescale:
            # Rescale the distances between 0 and 1000
            distances = np.multiply(distances, 1000).astype(int)
