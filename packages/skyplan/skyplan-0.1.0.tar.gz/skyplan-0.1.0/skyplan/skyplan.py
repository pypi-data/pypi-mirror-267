...
# github      : https://github.com/erdogant/skyplan
...
# formatter = logging.Formatter('[%(asctime)s] [skyplan]> %(levelname)s> %(message)s', datefmt='%H:%M:%S')
formatter = logging.Formatter('[skyplan] >%(levelname)s> %(message)s')
...
class skyplan():
    ...
    def __init__(self, method='xgboost', verbose='info'):
        """Initialize skyplan with user-defined parameters."""
        self.method=method
        # Set the logger
...
def convert_verbose_to_new(verbose):
    ...
    if not isinstance(verbose, str) and verbose<10:
        status_map = {
            ...
            4: 'debug',
            5: 'debug'}
        if verbose>=2: print('[skyplan] WARNING use the standardized verbose status. The status [1-6] will be deprecated in future versions.')
...
if __name__ == "__main__":
    import skyplan as skyplan
    df = skyplan.import_example()
    out = skyplan.fit(df)
    fig,ax = skyplan.plot(out)
