...
#def getCompleteness(a, b, c, alpha, beta, gamma, resolution, unique_refl):
#    volume = unitCellVolume(float(a),float(b), float(c), float(alpha), float(beta), float(gamma))
#    total_refl = volume*(4/3)*np.pi*(1/resolution)**3
#    return (unique_refl / total_refl)*100
