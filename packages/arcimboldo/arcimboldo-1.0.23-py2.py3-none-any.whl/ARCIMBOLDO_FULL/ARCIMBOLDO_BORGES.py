...
#updateAnomParamDic(hkl=None, mtz=None, current_directory=None, cell_dim=None, sg_number=1,otherAnomParamDic=None, st     artExpAnomDic=None)
    ...
    for step_i in range(2):
        ...
        for clusi in range(len(orderedClusters)):
            ...
            if topExp_run is not None:
                ...
                if distribute_computing == "multiprocessing":
                    if sym.PROCESSES - 1 < 8:
                        topExp_run = 2 * (sym.PROCESSES - 1) if sym.PROCESSES > 4 else 4
                    else:
    ...
    if coiled_coil:
        try:
            solved = SELSLIB2.verification_coil_predicted(lock=lock, cm=cm, sym=sym, DicGridConn=DicGridConn, DicParameters=DicParameters,
    ...
    if not solved and NC >= 2 and coiled_coil:
        multicopy = True
    else:
        multicopy = Config.getboolean("ARCIMBOLDO-BORGES", "multicopy")

    #RUNNING LITE MULTICOPY
    if multicopy:
        print("\nStarting multicopy step")
        #Reading FOMS for the prioritization of the 1st fragment
        ...
        for i, elem in enumerate(list_pdbs):
            ...
            SELSLIB2.writeMultiCopyFile(lock, current_directory, nameOutput, 
                                        currentMulticopy={bordata.get("ARCIMBOLDO", "name_job"): str(os.path.normpath(html_path))},
                                        multicopyDict=best_multicopies,
                                        numberDiscarded=number_discarded)
            ...
            if os.path.exists(os.path.join(lite_dir, 'best.pdb')):
                best_multicopies[bordata.get("ARCIMBOLDO", "name_job")] = str(os.path.normpath(html_path))
            else:
                number_discarded += 1
        ...
        for i, line in enumerate(lines):
            if i != 0 and i <= 5:
                ...
                ARCIMBOLDO_LITE.startARCIMBOLDO(bordata, "borfile", DicParameters=DicParameters, DicGridConn=DicGridConn,
                                                cm=cm, sym=sym, doTest=False, mtz_given=mtz, F_given=F, SIGF_given=SIGF,
                                                normfactors=normfactors, tncsfactors=tncsfactors, Intensities=Intensities,
                                                Aniso=Aniso, nice=nice, out_phaser_given=aniout, fneed=fneed, isShredder=True, doVerification=True)
        ...
        SELSLIB2.writeMultiCopyFile(lock, current_directory, nameOutput, multicopyDict=best_multicopies, numberDiscarded=number_discarded)
    try:
...
#-----------------------------------------------------------------------------------------------------------------------#
    ...
    Config = configparser.ConfigParser()
    #Config.read_file(cStringIO.StringIO(Data.defaults_bor))
