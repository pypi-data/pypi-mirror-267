...
def startARCIMBOLDO(BorData, input_bor, DicParameters={}, DicGridConn={}, cm=None, sym=None, doTest=True, mtz_given="",
                    ...
                    isMulticopy=False, doVerification=False):
    ...
    if doVerification and coiled_coil:
        results_verification = SELSLIB2.read_all_data_from_expansion_directory(os.path.join(current_directory, "./8_EXP_LIBRARY"))
        SELSLIB2.verification_coil_predicted_multicopy(lock=lock, cm=cm, sym=sym, nice=nice, DicGridConn=DicGridConn,
                                                       DicParameters=DicParameters, current_dir=current_directory,
                                                       output_name=nameOutput, job_name='9_VERIFICATION', space_group=spaceGroup,
                                                       fromNcycles=1, toNcycles=nautocyc, mtz=mtz, MW=MW, NC=NC,
                                                       F=F, SIGF=SIGF, res_refin=res_refin, Intensities=Intensities, Aniso=Aniso,
                                                       normfactors=normfactors, tncsfactors=tncsfactors, RMSD=RMSD, quate=quate,
                                                       laue=laue, ncs=ncs, spaceGroup=spaceGroup, hkl=hkl, ent=ent, seq=seq,
                                                       cell_dim=cell_dim, shlxLinea0=shlxLinea0, shlxLineaB=shlxLinea1, shlxLineaLast=shlxLineaLast,
                                                       USE_PACKING=USE_PACKING, USE_TRANSLA=USE_TRANSLA, USE_TNCS=USE_TNCS,
                                                       solution_sorting_scheme=solution_sorting_scheme, topExp=topExp,
                                                       topNextFragment=topNextFragment, startExpAnomDic=startExpAnomDic,
                                                       pattersonPeaks=PattersonPeaksData, llgdic=llgdic,
                                                       nBunchAutoTracCyc=nBunchAutoTracCyc, clu_all_merged=CluAll, conv_names_merged=results_verification)
