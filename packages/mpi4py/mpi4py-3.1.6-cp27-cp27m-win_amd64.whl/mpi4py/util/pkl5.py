...
def _recv_raw(comm, recv, buf, source, tag, status=None):
    ...
    bufs = list(imap(_new_buffer, info[1:]))
    with _bigmpi as bigmpi:
...
def _mrecv_data(message, mrecv, status=None):
    ...
    rmsg = iter(message)
    icnt = len(message) - 1
    info = _mrecv_info(rmsg.next(), icnt, status)
    data = _new_buffer(info[0])
    bufs = list(imap(_new_buffer, info[1:]))
    with _bigmpi as bigmpi:
        mrecv(rmsg.next(), bigmpi(data))
        for rbuf in bufs:
            mrecv(rmsg.next(), bigmpi(rbuf))
...
def _testall(requests, testall, statuses):
    if isinstance(statuses, list):
        for _ in xrange(len(requests) - len(statuses)):
    ...
    if statuses is not None:
        stsarray = []
        for req, sts in izip(requests, statuses):
...
class Request(tuple):
    ...
    def __new__(cls, request=None):
        ...
        if isinstance(request, MPI.Request):
            request = (request,)
        return super(Request, cls).__new__(cls, request)
    ...
    def get_status(self, status=None):
        ...
        return all(imap(MPI.Request.Get_status, self, statuses))
...
class Message(tuple):
    ...
    def __new__(cls, message=None):
        ...
        if isinstance(message, MPI.Message):
            message = (message,)
        return super(Message, cls).__new__(cls, message)
