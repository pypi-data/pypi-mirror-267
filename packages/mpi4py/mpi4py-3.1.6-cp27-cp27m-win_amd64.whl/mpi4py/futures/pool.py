...
class MPIPoolExecutor(Executor):
    ...
    def map(self, fn, *iterables, **_3to2kwargs):
        if 'unordered' in _3to2kwargs: unordered = _3to2kwargs['unordered']; del _3to2kwargs['unordered']
        else: unordered = False
        if 'chunksize' in _3to2kwargs: chunksize = _3to2kwargs['chunksize']; del _3to2kwargs['chunksize']
        else: chunksize = 1
        ...
        """
        # pylint: disable=arguments-differ
        return self.starmap(fn, izip(*iterables), timeout, chunksize, unordered)
    ...
    def shutdown(self, wait=True, **_3to2kwargs):
        if 'cancel_futures' in _3to2kwargs: cancel_futures = _3to2kwargs['cancel_futures']; del _3to2kwargs['cancel_futures']
        else: cancel_futures = False
