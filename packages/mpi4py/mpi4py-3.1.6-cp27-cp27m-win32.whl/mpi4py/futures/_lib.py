...
def _manager_thread(pool, options):
    ...
    threads = [threading.Thread(target=worker) for _ in xrange(size - 1)]
...
def _manager_comm(pool, options, comm, full=True):
    ...
    queue = pool.setup(size)
    workers = Stack(reversed(xrange(size)))
    ...
    def __enter__(self):
        ...
        if MPI.COMM_WORLD.Get_size() >= 2:
            ...
            if self.on_root:
                ...
                self.workers = Stack(reversed(xrange(size)))
...
def barrier(comm):
    ...
    try:
        ...
        sendreqs, recvreqs = [], []
        for pid in xrange(comm.Get_remote_size()):
...
def _init_get_data(options):
    ...
    vals = (None, (), {})
    data = dict((k, options.pop(k, v)) for k, v in izip(keys, vals))
    return data
...
def server_main_service():
    ...
    optdict = dict((opt[2:], val) for opt, val in optlist)
