...
class AppStore(object):
    ...
    def __init__(self, issuer_id:str, key_id:str, key:str, save_fold_name:str = 'file') -> None:
        if not issuer_id:
            die('isserId不能为空')
        ...
        self._foldPath = None
        self.save_fold_name = save_fold_name if save_fold_name else 'file'
        # 创建请求的Agent
    ...
    def foldPath(self) -> Path:
        ...
        if self._foldPath is None:
            self._foldPath = Path().resolve().joinpath(self.save_fold_name)
            # 创建文件夹
