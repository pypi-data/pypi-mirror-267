...
class Tracker:
    ...
    def __init__(self, path):
        self.items = TrackerItems(self.read_tracker(path))
    ...
    def read_tracker(self, path) -> dict[str, int]:
        """Read the item tracker"""

        if not os.path.exists(path):
            self.reset_tracker(path)
        data = helper.read_file_string(path)
        try:
    ...
    def write_tracker1(self, path):
        """Write the item tracker"""

        data = json.dumps(self.items.to_dict(), indent=4)
        helper.write_file_string(path, data)
