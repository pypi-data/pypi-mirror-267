...
class Poller:
    ...
    def start_poll(self, msg):
        ...
        bot.send_message(msg.from_user.id, text, reply_markup=self.create_keyboard())
        bot.register_next_step_handler(msg, self.next_step)
    ...
    def next_step(self, msg):
        ...
        if self.buttons.get(self.counter):
            ...
            bot.send_message(msg.from_user.id, self.bot_answers.get(self.counter))
        ...
        if filters:
            ...
            if self.bot_answers.get(self.counter):
                bot.send_message(msg.from_user.id, self.bot_answers.get(self.counter).format(msg.text))
            self.counter += 1
            self.start_poll(msg)
        else:
