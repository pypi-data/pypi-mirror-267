...
class FrogModel:
        ...
        try:
            ...
            with open(filename, "rb") as file_handle:
                for chunk in pd.read_csv(
                    ...
                    updated, inserted = self.upsert(table_name, chunk, _correlation_id=_correlation_id, activity=_activity)
            ...
            with pd.ExcelFile(filename) as xls:
                ...
                for count, sheet_name in enumerate(xls.sheet_names):
                    ...
                    updated, inserted = self.upsert(
                        table_to_upload, df, _correlation_id=_correlation_id, activity=_activity
                    )
    ...
    def _get_combined_key_columns_for_upsert(self, table_name: str, is_anura_table = False):
        pk_custom_columns = []
        if self.oc:
            pk_custom_columns = self.oc.get_pk_custom_columns_from_platform(table_name, self.model_name, self._app_key, self.log)
    ...
    def upsert(
        ...
        # If table name is abbreviated name, then table_name becomes anura table name
        if table_name in anura_abbreviated_names.keys():
        ...
        if not is_anura_table and not is_custom_table:
            # Skip it
            self.log.warning(
                "Table name not recognised as an Anura, Abbreviated name or Custom table (skipping): %s", table_name
            )
            return 0, 0
        ...
        # TODO, if PK does not exist as column in file just ignore it
        # TODO fails if combined_key_columns empty, what to do here if no PKs? just import?
        try:
            # here it fails if key column does not exist in file
            data = data.drop_duplicates(combined_key_columns)
        except KeyError as e:
            pk_missing_name = str(e).split("'")[1] # cant find a nicer way of extracting missing column name
            if activity:
                activity.update_activity(
                    ActivityStatus.FAILED,
                    last_message=f"Primary Key column {pk_missing_name} does not exist in the input data. Aborting upsert.",
                    progress=100,
                )
            return 0, 0
