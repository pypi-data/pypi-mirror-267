...
class GitCheckFileIsPushedHandler(GitHandler):
    ...
    async def get(self, path: str = ""):
        ...
        # Check the commit is pushed to specific remote branch
        ...
        try:
            current_branch = await self.git.get_current_branch(path=local_path)
        except Exception as exc:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps({"code": 1, "message": str(exc)}))
            return

        remote_commit_result = await self.git.get_remote_commit_ids(branch=current_branch, path=local_path)
