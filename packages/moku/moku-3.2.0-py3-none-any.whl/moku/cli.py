...
from moku import MOKU_CLI_PATH, MOKU_DATA_PATH, __version__
...
def _get_moku_data_url(fw=None):
    return f'https://updates.liquidinstruments.com/static/mokudata-{fw or COMPAT_FW}.tar.gz'  # noqa
...
def download(args):
    ...
    try:
        _mdf_url = _get_moku_data_url(args.fw_ver).replace('tar.gz', 'md5')
...
def list_mokus(args):
    ...
    proc = Popen([MOKU_CLI_PATH, "list"], stdout=sys.stdout,
                 stderr=sys.stderr)
    proc.wait()


parser_list = subparsers.add_parser('list',
                                    help="List mokus on the network")
parser_list.set_defaults(func=list_mokus)


def main():
    print(f"Moku Client Version {__version__}")
