...
class TimeFrequencyAnalyzer(Moku):
    ...
    def set_frontend(self, channel, impedance, coupling, range, strict=True):
        ...
        params = dict(
            ...
            range=validate_range(
                range,
                ['400mVpp', '1Vpp', '4Vpp', '10Vpp', '40Vpp', '50Vpp']))
        return self.session.post(
    ...
    def set_interpolation(self, mode="Linear", strict=True):
        ...
        params = dict(
            strict=strict,
            mode=validate_range(mode, ['None', 'Linear']),
        )
        return self.session.post(
            f"slot{self.slot}/{self.operation_group}",
            operation,
    ...
    def set_acquisition_mode(self, mode="Continuous", gate_source=None,
                             gate_threshold=None, window_length=None,
                             strict=True):
        """
    ...
    def set_interval_analyzer(self, id, start_event_id,
                              stop_event_id, enable=True,
                              strict=True):
        """
    ...
    def generate_output(self, channel, signal_type, scaling,
                        zero_point=0, output_range=None, invert=False,
                        strict=True):
        """
        generate_output
        :type strict: `boolean`
        ...
        params = dict(
            strict=strict,
            channel=channel,
            signal_type=validate_range(signal_type, ['Interval', 'Count']),
            scaling=scaling,
            zero_point=zero_point,
            output_range=validate_range(output_range, ['2Vpp', '10Vpp']),
            invert=invert,
        )
        return self.session.post(
            f"slot{self.slot}/{self.operation_group}",
    ...
    def start_logging(self, event_ids, duration=60, file_name_prefix="",
                      comments="", delay=0, quantity="EventTimestamp",
                      strict=True):
        """
        start_logging
        :type strict: `boolean`
        ...
        params = dict(
            ...
            delay=delay,
            event_ids=event_ids,
            quantity=validate_range(quantity, ['EventTimestamp']),
        )
        return self.session.post(
            f"slot{self.slot}/{self.operation_group}",
            operation,
